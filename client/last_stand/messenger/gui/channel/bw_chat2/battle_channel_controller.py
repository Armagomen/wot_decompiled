    ...
    def _formatCommand(self, command):
        ...
        if command.getCommandType() == MESSENGER_COMMAND_TYPE.BATTLE:
            ...
            if battleChatCommand.name in self._BASE_RELATED_COMMANDS_NAME:
                ...
                markerCommand2Text = {(LSMarkerComponentNames.MAGNUS, LSMarkersType.MAGNUS): {BATTLE_CHAT_COMMAND_NAMES.MOVING_TO_TARGET_POINT: (
                                                                                                                            rPath.move_to_magnus(), backport.text), 
                                                                           BATTLE_CHAT_COMMAND_NAMES.MOVE_TO_TARGET_POINT: (
                                                                                                                          rPath.attention_to_magnus(), backport.text)}, 
                   (LSMarkerComponentNames.CAMP, LSMarkersType.CAMP): {BATTLE_CHAT_COMMAND_NAMES.MOVING_TO_TARGET_POINT: (
                                                                                                                        rPath.move_to_camp(), backport.text), 
                                                                       BATTLE_CHAT_COMMAND_NAMES.MOVE_TO_TARGET_POINT: (
                                                                                                                      rPath.attention_to_camp(), getTextWithGridId)}}
                for (componentName, markerStyle), paramsData in viewitems(markerCommand2Text):
                    component = entity.dynamicComponents.get(componentName)
                    if component and component.style == markerStyle:
                        params = paramsData.get(battleChatCommand.name, None)
                        if params:
