...
class LSProgressPointsFormatter(ServiceChannelFormatter):
    _MSG_KEY = 'lsProgressPointsMessage'
...
class LSBattleResultsFormatter(BattleResultsFormatter):
    ...
    def _prepareFormatData(self, message):
        ...
        dailyQuestProgressPoints = battleResults.get('tokens', {}).get(ProgressPointsSettings.TOKEN, {}).get('count', 0)
        ctx['progressPoints'] = dailyQuestProgressPoints + battleResults.get('progressPoints', 0)
        artefacts = sum(data.get('count', 0) for token, data in viewitems(battleResults.get('tokens', {})) if ArtefactsSettings.QUEST_PREFIX in token and token != ProgressPointsSettings.TOKEN)
    ...
    def __makeBattleResultString(self, lsPhase, lsPhasesCount, isWinner, bonusType):
        ...
        return g_settings.htmlTemplates.format('LSBattleResultWaves', ctx={'curPhase': text_styles.credits(lsPhase), 'maxPhases': text_styles.credits(lsPhasesCount)})

    def __makeArtefactString(self, artefacts):
        if not artefacts:
            return ''
        if artefacts > 1:
            return g_settings.htmlTemplates.format('LSBattleResultQuests', ctx={'artefacts': artefacts})
        return g_settings.htmlTemplates.format('LSBattleResultQuest', ctx={'artefacts': artefacts})

    def __makeAchievementsAndBadgesStrings(self, battleResults):
        popUpRecords = []
        badges = []
        for vehBattleResults in viewvalues(battleResults.get('playerVehicles', {})):
