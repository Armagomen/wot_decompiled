...
def showNarrationWindowView(useQueue=False):
    from last_stand.gui.impl.lobby.narration_view import NarrationWindow
    layoutID = R.views.last_stand.mono.lobby.narration_view()
    findAndLoadWindow(useQueue, NarrationWindow, layoutID)
...
@dependency.replace_none_kwargs(lsController=ILSController)
def showInfoPage(lsController=None):
    if not lsController.isInfoPageEnabled():
        return
...
def showBattleResult(arenaUniqueID):
    from last_stand.gui.impl.lobby.states import LastStandBattleResultsState
    LastStandBattleResultsState.goTo(arenaUniqueID=arenaUniqueID)
...
def showAbilitiesIncompleteConfirm(parent=None):
    from last_stand.gui.impl.lobby.dialogs.abilities_incomplete_dialog import AbilitiesIncompleteDialog
    from gui.impl.dialogs import dialogs
    result = yield wg_await(dialogs.showSingleDialogWithResultData(layoutID=R.views.last_stand.mono.lobby.dialogs.abilities_incomplete_confirm(), wrappedViewClass=AbilitiesIncompleteDialog, parent=parent))
    raise AsyncReturn(result)
...
def showStageRewardWindow(resourceID, isReward=False, useQueue=False):
    from last_stand.gui.impl.lobby.stage_reward_view import StageRewardWindow
    findAndLoadWindow(useQueue, StageRewardWindow, resourceID, isReward)