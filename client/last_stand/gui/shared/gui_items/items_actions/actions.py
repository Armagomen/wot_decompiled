...
def getConsumablesPrice(consumables):
    result = sum([ item.getBuyPrice() for item in consumables.layout.getItems() if not item.isInInventory and item not in consumables.installed
                 ], ITEM_PRICE_ZERO)
    return result
...
class LSBuyAndInstallConsumablesProcessor(BuyAndInstallConsumablesProcessor):
    ...
    def _setupPlugins(self):
        self.addPlugins((
         ...
         proc_plugs.MoneyValidator(getConsumablesPrice(getLSConsumables(self._vehicle)).price, byCurrencyError=False),
         LSConsumablesInstallValidator(self._vehicle)))
