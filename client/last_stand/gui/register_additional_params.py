...
from last_stand.gui.ls_gui_constants import LSDailyQuestDecorationMap, LS_TOOLTIP_SET, PREBATTLE_ACTION_NAME, FUNCTIONAL_FLAG
...
def registerAdditionalParams(personality):
    ...
    registerLobbyTooltipsBuilders([
     (
      'last_stand.gui.scaleform.daapi.view.tooltips.tooltip_builders', LS_TOOLTIP_SET)])
...
def registerConditionFormatterIcons():
    ...
    BATTLE_RESULTS_KEYS.update({'ls_pickedUpSouls': 'ls_pickedUpSouls', 
       'ls_phasesCompleted': 'ls_phasesCompleted', 
       'ls_killsByTeam': CONDITION_ICON.KILL_VEHICLES})


def registerDailyQuestDecorationMap(personality):
    from constants import DailyQuestDecorationMap
    commonKeys = set(DailyQuestDecorationMap) & set(LSDailyQuestDecorationMap)
    if commonKeys:
        raise SoftException(('DailyQuestDecorationMap already has keys:{keys}. Personality: {personality}').format(keys=commonKeys, personality=personality))
    DailyQuestDecorationMap.update(LSDailyQuestDecorationMap)