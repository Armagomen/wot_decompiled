...
_PRESETS_CUSTOM_INDEXES = {AmmoPanelSwitchPreset.PRESET_1: [
                                  3, 5, 0, 2], 
...
class LSConsumablesPanel(ConsumablesPanelMeta):
    ...
    LS_BOOSTER_FACTORS = {
     BOOSTER_FACTOR_NAMES.ABILITY_DURATION, BOOSTER_FACTOR_NAMES.ABILITY_COST}
    ...
    def __init__(self):
        ...
        self._shellsCount = {}
        self._lsEquipmentDataCache = {'usageCost': {}, 'duration': {}}
        return
    ...
    def _populate(self):
        ...
        if lsBattleGuiCtrl:
            lsBattleGuiCtrl.onVehicleFactorsReady += self._updateFactors
    ...
    def _dispose(self):
        ...
        self._lsEquipmentDataCache.clear()
        lsBattleGuiCtrl = self.sessionProvider.dynamic.getControllerByID(BATTLE_CTRL_ID.LS_BATTLE_GUI_CTRL)
        if lsBattleGuiCtrl:
            lsBattleGuiCtrl.onVehicleFactorsReady -= self._updateFactors
        super(LSConsumablesPanel, self)._dispose()

    def _updateFactors(self, diff):
        if not self.LS_BOOSTER_FACTORS & diff:
            return
        eqCtrl = self.sessionProvider.shared.equipments
        if eqCtrl:
            vehicle = avatar_getter.getPlayerVehicle()
            for intCD, item in viewitems(eqCtrl.getEquipments()):
                if intCD not in self._cds or self._isSuperShell(item) or self._isLSEmptySlot(item):
                    continue
                ...
                if BOOSTER_FACTOR_NAMES.ABILITY_COST in diff:
                    newCount = self._getUsageCost(descriptor.compactDescr, variant, useCache=False) if self._hasUsageCost(item) else 0
                    self.as_updateAbilityCostS(idx, newCount)
                if BOOSTER_FACTOR_NAMES.ABILITY_DURATION in diff:
                    self._getDurationSeconds(descriptor.compactDescr, variant, useCache=False)
    ...
    def _getEquipmentIcon(self, idx, item, icon):
        ...
        if lsIconRes.isValid():
            return backport.image(lsIconRes())
        return super(LSConsumablesPanel, self)._getEquipmentIcon(idx, item, icon)

    def _updateLSEquipmentKeyHandlers(self, updateSlots=None):
        lsEquipmentRange = lrange(self._LS_EQUIPMENT_START_IDX, self._LS_EQUIPMENT_END_IDX + 1)
    ...
    def _updateEquipmentSlot(self, idx, item, forceUpdate=False):
        ...
        vehicle = avatar_getter.getPlayerVehicle()
        variant = descriptor.getVariant(avatar_getter.getPlayerVehicle().typeDescriptor) if vehicle else descriptor.fallbackVariant
        count = self._getUsageCost(descriptor.compactDescr, variant) if self._hasUsageCost(item) else 0
    ...
    def _onCurrentShellChanged(self, intCD):
        super(LSConsumablesPanel, self)._onCurrentShellChanged(intCD)
        for valueIntCD, count in self._shellsCount.items():
            self._updateShellGlow(valueIntCD, count, count)
    ...
    def _clearAllEquipmentGlow(self):
        ...
        for idx in range(self._EQUIPMENT_START_IDX, self._PANEL_MAX_LENGTH - 1):
            self.as_hideGlowS(idx)
    ...
    def _buildEquipmentSlotTooltipText(self, item):
        ...
        vehicle = avatar_getter.getPlayerVehicle()
        variant = descriptor.getVariant(vehicle.typeDescriptor) if vehicle else descriptor.fallbackVariant
        if self._hasDuration(item):
            if int(variant.durationSeconds) > 0:
                body = ('\n').join((
                 body,
                 backport.text(R.strings.last_stand_battle.consumablesPanel.tooltip.durationTime(), time=str(int(self._getDurationSeconds(descriptor.compactDescr, variant))))))
        ...
        if self._hasUsageCost(item):
            usageCost = self._getUsageCost(descriptor.compactDescr, variant)
            if usageCost:
    ...
    def _getUsageCost(self, intCD, variant, useCache=True):
        if useCache and intCD in self._lsEquipmentDataCache['usageCost']:
            return self._lsEquipmentDataCache['usageCost'][intCD]
        factorsComponent = getVehicleBoosterFactorsComponent(avatar_getter.getPlayerVehicle())
        cost = max(0, int(factorsComponent.applyFactors(variant.usageCost, BOOSTER_FACTOR_NAMES.ABILITY_COST))) if factorsComponent else variant.usageCost
        self._lsEquipmentDataCache['usageCost'][intCD] = cost
        return cost

    def _getDurationSeconds(self, intCD, variant, useCache=True):
        if useCache and intCD in self._lsEquipmentDataCache['duration']:
            return self._lsEquipmentDataCache['duration'][intCD]
        duration = int(variant.durationSeconds)
        if duration > 0:
            factorsComponent = getVehicleBoosterFactorsComponent(avatar_getter.getPlayerVehicle())
            applyFactor = ceil(factorsComponent.applyFactors(duration, BOOSTER_FACTOR_NAMES.ABILITY_DURATION)) if factorsComponent else duration
            duration = max(1, int(applyFactor))
        self._lsEquipmentDataCache['duration'][intCD] = duration
        return duration
