...
def _lsActionRibbonFormatterWithRole(ribbon, arenaDP, updater):
    if ribbon.isShowActionSource:
        vInfo = arenaDP.getVehicleInfo(ribbon.vehicleID)
        ...
        role = getVehicleRole(vTypeInfoVO)
        if role is not None:
            vehicleClassTag = role
    else:
        vehicleName, vehicleClassTag = ('', '')
    updater(ribbonID=ribbon.getID(), ribbonType=ribbon.getType(), leftFieldStr=ribbon.actionValue, vehName=vehicleName, vehType=vehicleClassTag)
    return
...
class _LSIncomingRepairRibbon(_LSGameplayActionRibbon):
    ...
    def getType(self):
        if self.isSelfHeal:
            return LS_BATTLE_EFFICIENCY_TYPES.LS_REPAIR
        return LS_BATTLE_EFFICIENCY_TYPES.LS_REPAIR_BY_OTHER
...
@LSGameplayActionRibbonsFactory.registerActionRibbon(LSGameplayActionID.HEAL_BY_OBELISK)
class _LSHealthByObelisk(_LSGameplayActionRibbon):
    BATTLE_EFFICIENCY_TYPE = LS_BATTLE_EFFICIENCY_TYPES.LS_REPAIR_VEHICLE_BY_OBELISK
    ...
    def getFormatter(self):
        return _lsActionRibbonFormatterWithRole
...
class LSRibbonsAggregator(RibbonsAggregator):
    ...
    def _aggregateRibbons(self, ribbons):
        copyMergedRibbons = []
        for destEffType, sourceClasses in self.RIBBON_TYPES_COPY_TO.items():
...
def createRibbonsAggregator():
    if BattleReplay.g_replayCtrl.isPlaying:
        return LSRibbonsAggregatorPlayer()
    return LSRibbonsAggregator()