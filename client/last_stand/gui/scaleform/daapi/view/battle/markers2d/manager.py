...
class LSAreaMarkerPlugin(AreaMarkerPlugin):
    ...
    def getMarkerSubtype(self, targetID):
        ...
        marker = findFirst(lambda item: item.getTargetID() == targetID, viewvalues(self.markers))
        if not marker:
            return INVALID_MARKER_SUBTYPE
    ...
    def _getComponent(self, targetID, markerType):
        return findFirst(lambda item: item[1].getTargetID() == targetID and item[1].getBCMarkerType() == markerType, viewitems(self.markers), (0,
                                                                                                                                               None))
