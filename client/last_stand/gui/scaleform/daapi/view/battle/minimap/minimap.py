...
class LSAreaMarkerEntriesPlugin(LSMinimapScaleMixin, BaseAreaMarkerEntriesPlugin):
    ...
    def _getMarkerFromTargetID(self, targetID, markerType):
        return findFirst(lambda item: item.getTargetID() == targetID and item.getBCMarkerType() == markerType, viewvalues(self._entries))
...
class LSMinimapVehiclesPlugin(LSMinimapScaleMixin, ArenaVehiclesPlugin):
    ...
    def _getDisplayedVehicleClassTag(self, vInfo):
        ...
        if role is not None:
            ...
            return super(LSMinimapVehiclesPlugin, self)._getDisplayedVehicleClassTag(vInfo)
...
class LSEquipmentsPlugin(LSMinimapScaleMixin, EquipmentsPlugin):
    ...
    def _getSymbolFromMarker(self, marker):
        if marker in self.LS_EQ_MARKER_TO_SYMBOL:
            return self.LS_EQ_MARKER_TO_SYMBOL[marker]
        return super(LSEquipmentsPlugin, self)._getSymbolFromMarker(marker)
