...
import BattleReplay, BigWorld, logging
...
class LSBattleGUIControllerBase(IArenaVehiclesController):
    ...
    def __init__(self):
        ...
        self.onVehicleFactorsReady = Event(self._eManager)
        self.onBuffUpdate = Event(self._eManager)
    ...
    def startControl(self, battleCtx, arenaVisitor):
        ...
        LSArenaPhasesComponent.onCleanBCMarkersRequested += self._removeBattleCommunicationMarkers
    ...
    def onFactorsReady(self, vehicleID, diff):
        if self._isPlayerVehicle(vehicleID):
            self.onVehicleFactorsReady(diff)

    def applyBuff(self, data, vehicleID):
        self.guiSessionProvider.invalidateVehicleState(VEHICLE_VIEW_STATE.LS_BUFF, data, vehicleID)
        self._updateBuff(True, data)

    def unApplyBuff(self, data):
        self._updateBuff(False, data)

    def _updateBuff(self, isActive, data):
        self.onBuffUpdate(isActive, data)
    ...
    def _removeBattleCommunicationMarkers(self):
        pass