...
class LSVehicleInventoryPresenter(ViewComponent[VehiclesInventoryModel], IGlobalListener):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.mono.hangar.vehicle_tooltip():
            return LSVehicleCarouselTooltipView(event.getArgument('inventoryId'))
        return super(LSVehicleInventoryPresenter, self).createToolTipContent(event=event, contentID=contentID)
