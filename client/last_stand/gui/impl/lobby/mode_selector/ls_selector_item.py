...
class LSSelectorItem(ModeSelectorLegacyItem):
    ...
    def calendarTooltipText(self):
        ...
        if timeValue >= time_utils.ONE_DAY:
            return backport.text(R.strings.mode_selector.mode.last_stand.calendar(), date=self.__getEndDate())
        return backport.text(R.strings.mode_selector.mode.last_stand.calendarDay(), date=self.__getEndDate())
    ...
    def handleInfoPageClick(self):
        if not self.lsCtrl.isInfoPageEnabled():
            return
    ...
    def _isInfoIconVisible(self):
        return GUI_SETTINGS.lookup(LS_INFO_PAGE_KEY) is not None and self.lsCtrl.isInfoPageEnabled()
