...
import BigWorld
from last_stand.gui.prb_control.entities import vehicleAbilitiesCheck
...
class LastStandEntity(PreQueueEntity, VehicleSwitcher):
    ...
    @vehicleAbilitiesCheck
    def queue(self, ctx, callback=None):
        super(LastStandEntity, self).queue(ctx, callback=callback)

    def doSelectAction(self, action):
        if action.actionName == PREBATTLE_ACTION_NAME.LAST_STAND:
            return SelectResult(True)
        return super(LastStandEntity, self).doSelectAction(action)
