...
from last_stand.gui.ls_gui_constants import DifficultyLevel, PREBATTLE_ACTION_NAME, FUNCTIONAL_FLAG
...
class LastStandSquadEntity(SquadEntity, VehicleSwitcher):
    ...
    def init(self, ctx=None):
        ...
        initCtx = ctx.getInitCtx() if ctx is not None else None
        if isinstance(initCtx, LastStandSquadSettingsCtx):
    ...
    def unit_onUnitExtraChanged(self, extras):
        ...
        for item in self.__difficultyLevelCtrl.getLevelsInfo():
            if item.level == levelFromQueue:
                newLevel = getFirstNewStatusUnlockLevel()
    ...
    def _goToHangar(self):
        ...
        elif self._arenaUniqueID is None:
            showHangar()
        return
    ...
    def __createUnitLeaveMeta(self, unlockCtx, isSwitching=False):
        ...
        if (unlockCtx.hasFlags(FUNCTIONAL_FLAG.SWITCH) or isSwitching) and not isLSEntityCtx or unlockCtx.hasFlags(FUNCTIONAL_FLAG.EXIT) and isLSEntityCtx:
            return self.__getLeaveSquadBuilder(R.strings.last_stand_lobby.squad.leave)
        return self.__getLeaveSquadBuilder(R.strings.last_stand_lobby.squad.goToAnother)
    ...
    def __getLeaveSquadBuilder(self, key):
        ...
        builder.setIcon(R.images.last_stand.gui.maps.icons.platoon.logo_180(), backgrounds=[
