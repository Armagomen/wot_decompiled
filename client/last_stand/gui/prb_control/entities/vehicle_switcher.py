...
class VehicleSwitcher(object):
    ...
    def _getFavoriteVehInvID(self):
        filteredVehicles = self._lsCtrl.getSuitableVehicles()
        if g_currentVehicle.intCD in filteredVehicles:
            ...
            if vehInvID and self._itemsCache.items.getVehicle(vehInvID) is None:
                vehInvID = None
            if not vehInvID:
                vehicles = sorted(filteredVehicles.values(), key=vehicleComparisonKey)
                if vehicles:
