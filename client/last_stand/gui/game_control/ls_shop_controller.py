...
from last_stand_common.last_stand_constants import ShopSettings, ProgressPointsSettings
...
class LSShopController(ILSShopController):
    ...
    def keyBundles(self):
        return sorted(viewvalues(self._bundles), key=cmp_to_key(lambda b1, b2: cmp(b1.bundleID, b2.bundleID)))
    ...
    def getProgressPointsInBundle(self, bundleID):
        ...
        for bonus in bundle.bonuses:
            ...
            for token in viewvalues(tokens):
                if token.id != ProgressPointsSettings.TOKEN:
                    continue
                return token.count
    ...
    def _getBonuses(self, bundleID):
        ...
        for bonusType, bonusValue in viewitems(bonuses):
