...
class Generator(DynamicScriptComponent):
    ...
    def set_activationProgress(self, prev):
        ...
        if activation.progress > 0:
            ...
            if ctrl is not None:
                ctrl.onGeneratorCapture(self.__getIndex(), activation.progress, activation.timeLeft,
                                        activation.numInvaders)
            if gameObject.findComponentByType(ArenaComponents.GeneratorProgressComponent) is None:
                gameObject.createComponent(ArenaComponents.GeneratorProgressComponent,
                                           lambda: self.activationProgress.progress,
                                           lambda: self.activationProgress.timeLeft)
            if prev is None:
            ...
            gameObject.removeComponentByType(ArenaComponents.GeneratorProgressComponent)
        return
