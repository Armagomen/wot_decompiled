...
from last_stand.gui.shared.utils import checkAbilities
...
class ClientLastStandBattleMode(last_stand_constants.LastStandBattleMode):
    ...
    def _client_gameControllers(self):
        ...
        return (
         (
          ILSController, LSController, False),
         (
    ...
    def _client_awardControllerHandlers(self):
        from last_stand.gui.game_control.award_handlers import LSAwardWindowHandler
        return (LSAwardWindowHandler,)
    ...
    def _client_platoonLayouts(self):
        ...
        return [
         (
          EPlatoonLayout.MEMBER, PlatoonLayout(R.views.last_stand.lobby.MembersWindow(), MembersWindow))]
    ...
    def _client_battleEntry(self):
        from gui.impl.gen import R
        return R.entries.last_stand.battle()
    ...
    def _client_LobbyContextMenuOptions(self):
        ...
        return (
         (
          CREATE_LS_SQUAD, lsSquadOptionBuilder, createLSSquadHandler),)
    ...
    def _client_tokenQuestsSubFormatters(self):
        ...
        return (
         LSStarterBundleFormatter(), LSBattlePassPointsFormatter(), LSKingRewardFormatter())

    @property
    def _client_bonusTokens(self):
        from last_stand.gui.server_events.bonuses import boosterTokenChecker, BoosterTokenBonus, progressPointTokenChecker, ProgressPointTokenBonus
        return [
         (
          boosterTokenChecker, BoosterTokenBonus), (progressPointTokenChecker, ProgressPointTokenBonus)]
    ...
    def _client_battleChannelController(self):
        ...
        return (
         BATTLE_CHANNEL.TEAM, LSTeamChannelController)
    ...
    def _client_messengerServerFormatters(self):
        from last_stand.messenger.formatters.service_channel import LSProgressPointsFormatter, LSBattleResultsFormatter, LSAutoMaintenanceFormatter
        from messenger.formatters.service_channel import InvoiceReceivedFormatter
        return {_SM_TYPE.lsProgressPointsMessage.index(): LSProgressPointsFormatter(), 
    ...
    def _client_controlModes(self):
        return {CTRL_MODE_NAME.ARCADE: (
                                 LSArcadeControlMode, 'arcadeMode', CTRL_TYPE.USUAL), 
           CTRL_MODE_NAME.SNIPER: (
                                 LSSniperControlMode, 'sniperMode', CTRL_TYPE.USUAL)}
    ...
    def _client_equipmentTriggers(self):
        ...
        return [
         (
          'LSartillery_deathzone', _EventArtilleryItem, _ReplayEventArtilleryItem),
         (
          'LSdetonator_deathzone', _EventArtilleryItem, _ReplayEventArtilleryItem)]

    @property
    def _client_equipmentItems(self):
        from last_stand.gui.battle_control.controllers.equipment_items import _HpRepairAndCrewHeal, LSEquipmentReplayItem, LSEquipmentItem, LSSituationalEquipmentItem, _FastReload, _LSInstantShot, _NitroEquipmentReplayItem, _NitroEquipmentItem, _AbilityWithDuration, LSDamageShield
        return [
         (
          'LS_selfRepair', _HpRepairAndCrewHeal, LSEquipmentReplayItem),
         (
          'LS_teamRepair', LSEquipmentItem, LSEquipmentReplayItem),
         (
          'LS_damageShield', LSDamageShield, LSEquipmentReplayItem),
         (
    ...
    def _client_equipmentItemsTooltipMovies(self):
        ...
        return (
         moduleMovies, advancedTooltipMovies)
    ...
    def _client_hangarDynamicGuiProvider(self):
        from last_stand.gui.hangar_presets.ls_dynamic_gui_providers import LSHangarDynamicGuiProvider
        return LSHangarDynamicGuiProvider
...
class ClientLastStandMediumBattleMode(ClientLastStandBattleMode):
    ...
    @property
    def _client_hangarDynamicGuiProvider(self):
        from last_stand.gui.hangar_presets.ls_dynamic_gui_providers import LSMediumHangarDynamicGuiProvider
        return LSMediumHangarDynamicGuiProvider
...
class ClientLastStandHardBattleMode(ClientLastStandBattleMode):
    ...
    @property
    def _client_hangarDynamicGuiProvider(self):
        from last_stand.gui.hangar_presets.ls_dynamic_gui_providers import LSHardHangarDynamicGuiProvider
        return LSHardHangarDynamicGuiProvider
...
def init():
    ...
    g_overrideScaleFormViewsConfig.initExtensionLobbyPackages(__name__, [
     'last_stand.gui.scaleform.daapi.view.lobby',
    ...
    BATTLE_PACKAGES = ('last_stand.gui.scaleform.daapi.view.battle.shared', 'gui.Scaleform.daapi.view.battle.shared.vehicle_mechanics',
    ...
    g_overrideScaleFormViewsConfig.initExtensionBattlePackages(__name__, [
     'last_stand.gui.scaleform.daapi.view.battle'], last_stand_constants.ARENA_GUI_TYPE.LAST_STAND)
