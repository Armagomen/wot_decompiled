...
class EpicVehiclesWatcher(BaseVehiclesWatcher):
    ...
    def _getUnsuitableVehicles(self, onClear=False):
        ...
        vehs = self.itemsCache.items.getVehicles(
            REQ_CRITERIA.INVENTORY | ~REQ_CRITERIA.VEHICLE.LEVELS(config.validVehicleLevels)).itervalues()
        eventVehs = self.itemsCache.items.getVehicles(
            REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.EVENT_BATTLE ^ REQ_CRITERIA.VEHICLE.CLAN_WARS).itervalues()
