...
class EventBattleEntity(PreQueueEntity):
    ...
    def __init__(self):
        super(EventBattleEntity, self).__init__(FUNCTIONAL_FLAG.EVENT, QUEUE_TYPE.EVENT_BATTLES,
                                                EventBattleSubscriber())
        self.__watcher = None
    ...
    def doSelectAction(self, action):
        return SelectResult(True) if action.actionName in (PREBATTLE_ACTION_NAME.EVENT_BATTLE,) else super(
            EventBattleEntity, self).doSelectAction(action)

    def isInQueue(self):
        return prb_getters.isInEventBattlesQueue()
