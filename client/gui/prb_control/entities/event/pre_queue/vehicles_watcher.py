...
class EventBattlesVehiclesWatcher(BaseVehiclesWatcher):
    ...
    def _getUnsuitableVehicles(self, onClear=False):
        vehs = self.itemsCache.items.getVehicles(
            REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_NO_TAGS(_TAGS.EVENT_VEHS)).values()
        return vehs

    def __getTicketsShortageVehicles(self):
        vehs = self.itemsCache.items.getVehicles(REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS(
            {_TAGS.EVENT_BOSS}) | REQ_CRITERIA.VEHICLE.HAS_NO_TAGS({_TAGS.EVENT_SPECIAL_BOSS})).values()
        return vehs
