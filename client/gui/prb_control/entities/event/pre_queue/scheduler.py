...
class EventScheduler(BaseScheduler):
    ...
    def __show(self, status, isInit=False):
        ...
        if not self.__isConfigured:
            SystemMessages.pushMessage(text=backport.text(strRes.notSet()), type=SystemMessages.SM_TYPE.Warning,
                                       priority=NotificationPriorityLevel.HIGH)
        elif not self.__isPrimeTime and status != PrimeTimeStatus.FROZEN:
            SystemMessages.pushMessage(text=backport.text(strRes.primeTime.notAvailable.body()),
                                       messageData={'header': backport.text(strRes.primeTime.notAvailable.header())},
