...
class _EventBattleVehiclesValidator(SquadVehiclesValidator):
    ...
    def _isVehicleSuitableForMode(self, vehicle):
        return ValidationResult(False, UNIT_RESTRICTION.EVENT_VEHICLE_NOT_SELECTED) if not self._isValidMode(
            vehicle) else None
...
class _EventStateValidator(UnitStateValidator):
    ...
    def _validate(self):
        ...
        return ValidationResult(False,
                                UNIT_RESTRICTION.UNIT_INACTIVE_PERIPHERY_UNDEF) if status != PrimeTimeStatus.AVAILABLE else super(
            _EventStateValidator, self)._validate()
...
class EventSquadSlotsValidator(CommanderValidator):
    ...
    def _validate(self):
        ...
        return ValidationResult(False,
                                UNIT_RESTRICTION.COMMANDER_VEHICLE_NOT_SELECTED) if hasEmptySlots or not pInfo.isReady else None
