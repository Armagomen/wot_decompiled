...
def _getLeftTime(currentCycleEnd):
    ...
    return text_styles.stats(backport.getTillTimeStringByRClass(_correctLeftTime(timeLeft),
...
class EventBattlesSelectorTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args):
        ...
        timeTableBlocks.append(
            self._packTimeBlock(message=text_styles.main(backport.text(_STR_PATH.tooltips.selector.timeTable.today())),
                                timeStr=text_styles.bonusPreviewText(todayStr)))
        tomorrowStr = self._packPeriods(tomorrowPeriods)
        timeTableBlocks.append(self._packTimeBlock(
            message=text_styles.main(backport.text(_STR_PATH.tooltips.selector.timeTable.tomorrow())),
            timeStr=text_styles.stats(tomorrowStr)))
        items.append(formatters.packBuildUpBlockData(timeTableBlocks, 7,
    ...
    def _packHeaderBlock(self):
        return formatters.packTitleDescBlock(
            title=text_styles.highTitle(backport.text(_STR_PATH.tooltips.selector.title())),
            desc=text_styles.main(backport.text(_STR_PATH.tooltips.selector.desc())))

    def _packTimeTableHeaderBlock(self):
        return formatters.packImageTextBlockData(
            title=text_styles.stats(backport.text(_STR_PATH.tooltips.selector.timeTable.title())),
            img=backport.image(R.images.gui.maps.icons.buttons.calendar()), imgPadding=formatters.packPadding(top=2),
    ...
    def __formatTillEndBlock(timeLeft):
        return formatters.packTextBlockData(
            text_styles.main(backport.text(_STR_PATH.tooltips.selector.timeTable.tillEnd())) + ' ' + _getLeftTime(
                timeLeft))
...
class EventBattlesCalendar(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if self.__gameEventController.isModeActive():
            blocks.append(formatters.packBuildUpBlockData(
                [self.__packHeader(), packTimeTableHeaderBlock(SELECTOR_BATTLE_TYPES.EVENT),
                 formatters.packBuildUpBlockData(
                     packCalendarBlock(self.__gameEventController, time_utils.getCurrentTimestamp(),
                                       SELECTOR_BATTLE_TYPES.EVENT))]))
        return blocks
    ...
    def __packHeader(self):
        ...
        if cycleEndTime is None:
            ...
            return formatters.packTextBlockData(
                text_styles.highlightText(backport.text(_STR_PATH.selectorTooltip.timeTable.tillEnd(), time=timeLeft)),
...
class EventQuestsTooltipData(BlocksTooltipData):
    ...
    def __packHeader(self, quests, vehicle):
        ...
        if diff > 0 or not isPrimeTimeLeft or isLastSeasonDay or allBossCompleted:
            desc = text_styles.concatStylesToSingleLine(icons.inProgress(), text_styles.main(
                backport.text(R.strings.event.tooltips.header.quests.header.description.till_end(),
                              time=text_styles.stats(
                                  getDaysLeftFormatted(gameEventController=self.__gameEventController)))))
        elif isPrimeTimeLeft:
            desc = text_styles.concatStylesToSingleLine(icons.clockSh(), text_styles.tutorial(
                backport.text(R.strings.event.tooltips.header.quests.header.description.till_upd(),
                              time=getTillTimeByResource(EventInfoModel.getDailyProgressResetTimeDelta(),
                                                         R.strings.menu.Time.timeLeftShort))))
        return formatters.packImageTextBlockData(title=text_styles.highTitle(title), desc=desc, img=backport.image(
            R.images.gui.maps.icons.wtevent.quests.dyn('{}_header'.format(vehicle.eventType))()),
                                                 txtPadding=formatters.packPadding(top=18),
                                                 descPadding=formatters.packPadding(top=15, left=-4), txtOffset=20)

    def __packHeaderUnavailable(self):
        self._setWidth(385)
        return formatters.packTitleDescBlock(
            title=text_styles.highTitle(backport.text(R.strings.event.tooltips.header.quests.header.unavailable())),
            desc=text_styles.standard(
                backport.text(R.strings.event.tooltips.header.quests.header.description.unavailable())),
            padding=formatters.packPadding(top=23, left=19, bottom=15))
    ...
    def __packQuestInfo(self, quest):
        ...
        return formatters.packTitleDescBlock(title=name, desc=text_styles.main(quest.getDescription()),
                                             padding=selfPadding, descPadding=descPadding)
    ...
    def __packQuestReward(self, quest):
        ...
        return formatters.packBuildUpBlockData(blocks, layout=BLOCKS_TOOLTIP_TYPES.LAYOUT_HORIZONTAL,
                                               padding=formatters.packPadding(left=20, right=20),
                                               align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER) if blocks else None
    ...
    def __packBonus(self, bonus):
        ...
        iconBlock = formatters.packImageBlockData(img=bonus.getImage(AWARDS_SIZES.SMALL),
                                                  align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)
        textBlock = formatters.packAlignedTextBlockData(text=bonus.getFormattedLabel(), align=align, padding=padding)
        return formatters.packBuildUpBlockData(blocks=[iconBlock, textBlock], blockWidth=72,
                                               align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER,
                                               padding=formatters.packPadding(top=-8, bottom=-6))

    def __packBottom(self, count):
        return formatters.packTextBlockData(
            text=makeHtmlString('html_templates:lobby/textStyle', 'alignText', {'align': 'center',
                                                                                'message': text_styles.main(
                                                                                    backport.text(
                                                                                        R.strings.event.tooltips.header.quests.bottom(),
                                                                                        count=count))}),
