...
class TotalKillItemPacker(KillItemPacker):
    ...
    def pack(self, data):
        ...
        if deathReasons:
            packer = formatters.packTextParameterBlockData
            blocks = [
                packer(value=str(value), name=backport.text(R.strings.postbattle_screen.tooltip.kill.num(reason)())) for
                reason, value in deathReasons.iteritems()]
            blocks.append(packer(value=str(sum(deathReasons.values())),
                                 name=backport.text(R.strings.postbattle_screen.tooltip.kill.totalKilled(),
                                                    vals=getTooltipParamsStyle())))
            blockToInsert = formatters.packBuildUpBlockData(blocks)
            items.append(blockToInsert)
        return items
...
class TotalDetectionItemPacker(DetectionItemPacker):
    ...
    def pack(self, data):
        ...
        if value:
            packer = formatters.packTextParameterBlockData
            blocks = [packer(value=backport.getIntegralFormat(value),
                             name=backport.text(R.strings.postbattle_screen.tooltip.spotted.totalTanks()))]
            blockToInsert = formatters.packBuildUpBlockData(blocks)
            items.append(blockToInsert)
        return items
