...
from gui.server_events.bonuses import mergeBonuses, getSplitBonusFunction, RandomCrewbook, HunterCollection, \
    RandomEventStyle3d, RandomEventStyle2d, RandomEventDecal, LootBoxTokensBonus, TmanTemplateTokensBonus
...
from gui.shared.missions.packers.bonus import BaseBonusUIPacker, SimpleBonusUIPacker, GoodiesBonusUIPacker, \
    BonusUIPacker, ItemBonusUIPacker, CrewBookBonusUIPacker, VehiclesBonusUIPacker
...
_NON_BONUS_TOKENS = (
'wtevent:hunter_collection', 'wtevent:event_collection', TmanTemplateTokensBonus.TANKMAN_RECEVIED_TOKEN_PREFIX)
...
def _getBonusStatus(bonusType, bonusGroup, boxCtrl=None):
    ...
    if not bonus:
        ...
        return _Status.COLLECTED if all(
            [bonus.getC11nItem(item).fullCount() > 0 for item in bonus.getValue()]) else _Status.NOT_COLLECTED
...
class SimpleEventGoldPacker(SimpleEventCountablePacker):
    ...
    def _getToolTip(cls, bonus):
        return [createTooltipData(tooltip=makeTooltip(backport.text(R.strings.event.bonusTooltip.gold.header()),
                                                      backport.text(R.strings.event.bonusTooltip.gold.body())))]
...
class SimpleEventGoodiesPacker(GoodiesBonusUIPacker):
    ...
    def _getBoostersToolTip(cls, tooltipData, booster):
        ...
        tooltipData.append(createTooltipData(
            tooltip=makeTooltip(backport.text(R.strings.event.bonusTooltip.dyn(boosterName).header()),
                                backport.text(R.strings.event.bonusTooltip.dyn(boosterName).body()))))
...
class SimpleEventRandomCrewBookPacker(BaseBonusUIPacker):
    ...
    @classmethod
    def _getToolTip(cls, bonus):
        return [createTooltipData(tooltip=makeTooltip(backport.text(R.strings.event.bonusTooltip.crewBooks.header()),
                                                      backport.text(R.strings.event.bonusTooltip.crewBooks.body(),
                                                                    exp=bonus.getXP())))]
