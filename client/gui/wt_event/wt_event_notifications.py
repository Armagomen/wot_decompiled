...
class WTEventNotifications(IWTEventNotifications):
    ...
    def __onTokensUpdate(self, diff):
        ...
        if config.ticketToken in diff:
            SystemMessages.pushMessage(text=backport.text(self._STR_RES.ticketToken.received.body(),
                                                          ticketsCount=str(self.__gameEventCtrl.getTicketCount())),
                                       messageData={
                                           'header': backport.text(self._STR_RES.ticketToken.received.header())},
                                       type=SystemMessages.SM_TYPE.WarningHeader,
    ...
    def __onServerSettingsChange(self, diff):
        ...
        if IS_LOOT_BOXES_ENABLED in diff:
            if self.__lobbyContext.getServerSettings().isLootBoxesEnabled():
                SystemMessages.pushMessage(text=backport.text(self._STR_RES.lootboxes.switchOn.body()),
                                           type=SystemMessages.SM_TYPE.WTEventSwitchOnLootboxes)
            else:
                SystemMessages.pushMessage(text=backport.text(self._STR_RES.lootboxes.switchOff.body()),
                                           type=SystemMessages.SM_TYPE.WarningHeader,
                                           priority=NotificationPriorityLevel.HIGH,
                                           messageData={'header': backport.text(self._STR_RES.lootboxes.header())})

    def __onPrimeTimeStatusUpdate(self, status):
        if self.__curStatus == status:
            return
        if status == PrimeTimeStatus.FROZEN:
            SystemMessages.pushMessage(text=backport.text(self._STR_RES.switchOff.body()),
                                       messageData={'header': backport.text(self._STR_RES.switchOff.header())},
                                       type=SystemMessages.SM_TYPE.ErrorHeader, priority=NotificationPriorityLevel.HIGH)
        elif self.__curStatus == PrimeTimeStatus.FROZEN:
            SystemMessages.pushMessage(text=backport.text(self._STR_RES.switchOn.body()),
                                       messageData={'header': backport.text(self._STR_RES.switchOn.header())},
                                       type=SystemMessages.SM_TYPE.InformationHeader,
                                       priority=NotificationPriorityLevel.HIGH)
        elif status == PrimeTimeStatus.AVAILABLE:
            if self.__isFirstPrimeTime():
                SystemMessages.pushMessage(text=backport.text(self._STR_RES.eventStart.body()),
                                           type=SystemMessages.SM_TYPE.WTEventStart)
            else:
                SystemMessages.pushMessage(text=backport.text(self._STR_RES.primeTime.available.body()), messageData={
                    'header': backport.text(self._STR_RES.primeTime.available.header())},
                                           type=SystemMessages.SM_TYPE.WarningHeader)
            ...
            SystemMessages.pushMessage(text=backport.text(self._STR_RES.eventEnd.body()),
