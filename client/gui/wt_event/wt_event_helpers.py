...
def getAlertStatusVO(gameEventCtrl=None, connectionMgr=None):
    ...
    return CalendarStatusVO(
        alertIcon=backport.image(R.images.gui.maps.icons.library.alertBigIcon()) if showPrimeTimeAlert else None,
        buttonIcon='', buttonLabel=backport.text(R.strings.event.serverAlertMessage.button()),
        buttonVisible=showPrimeTimeAlert and gameEventCtrl.hasAvailablePrimeTimeServers(), buttonTooltip=None,
        statusText=text_styles.vehicleStatusCriticalText(errorStr), popoverAlias=None, bgVisible=True,
        shadowFilterVisible=showPrimeTimeAlert, tooltip=TOOLTIPS_CONSTANTS.EVENT_BATTLES_CALENDAR,
...
def getDaysLeftFormatted(gameEventController=None):
    ...
    if season is None:
        ...
        return backport.text(
            R.strings.event.status.timeLeft.lessHour()) if timeLeft < time_utils.ONE_HOUR else backport.getTillTimeStringByRClass(
            timeLeft, R.strings.event.status.timeLeft)
        ...
        def wrapper(self, event):
            if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
                ...
                if tooltipData.specialAlias == TOOLTIPS_CONSTANTS.EVENT_LOOTBOX:
                    ...
                    window = DecoratedTooltipWindow(
                        WtEventLootBoxTooltipView(isHunterLootBox=lootboxType == EventLootBoxes.WT_HUNTER),
                        parent=self.getParentWindow(), useDecorator=False)
                    ...
                    window = DecoratedTooltipWindow(VehicleCompensationTooltipContent(_COMP_TOOLTIP,
                                                                                      viewModelClazz=LootBoxVehicleCompensationTooltipModel,
                                                                                      **tooltipData.specialArgs))
                    window.move(event.mouse.positionX, event.mouse.positionY)
                else:
