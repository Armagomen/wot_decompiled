...
class _CarouselFilter(object):
    ...
    def isDefault(self, keys=None):
        ...
        if keys is None:
            keys = filterDefaults.keys()
        for key in keys:
            if self._filters[key] != filterDefaults[key]:
                return False
    ...
    def reset(self, keys=None, exceptions=None, save=True):
        filterDefaults = self.filterDefaults
        keys = keys if keys is not None else filterDefaults.iterkeys()
        exceptions = exceptions or []
        keys = [ key for key in keys if key not in exceptions ]
        filtersForReset = _filterDict(filterDefaults, keys)
        hasChanges = bool(filtersForReset)
        self.update(filtersForReset, save and hasChanges)
        return
...
class CustomizationCriteriesGroup(RoleCriteriesGroup):
    ...
    def _setOwn3DStyleCriteria(self, filters):
        if filters.get(FILTER_KEYS.OWN_3D_STYLE):
