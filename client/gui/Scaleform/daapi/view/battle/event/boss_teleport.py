...
if typing.TYPE_CHECKING:
    from gui.battle_control.controllers.spawn_ctrl import WtSpawnController
...
class EventBossTeleportView(EventBossTeleportViewMeta, ISpawnListener):
    ...
    @property
    def _spawnCtrl(self):
        return self._sessionProvider.dynamic.spawn
    ...
    def _handleKeyEvent(self, event):
        ...
        if cmdMap.isFired(CommandMapping.CMD_AMMO_CHOICE_4,
                          event.key) and event.isKeyDown() and not event.isRepeatedEvent():
            self.onCancel()
