...
class EventBattleTimer(EventBattleTimerMeta, IBattleHintView):
    ...
    def showHint(self, hint, data):
        hintName = hint.name
        if hintName == 'wtTimeRemaining_hunter':
        ...
        elif hintName == 'wtTimeRemaining_boss':
            message = backport.text(R.strings.event.battleTimer.timeRemaining.boss.messageText())
            self.as_showMessageS(message, isOverTime=False)
        elif hintName == 'wtOvertime_hunter' or hintName == 'wtOvertime_boss':
            message = backport.text(R.strings.event.battleTimer.overtime.messageText())
