...
from gui.Scaleform.daapi.view.battle.battle_royale import StatusNotificationTimerPanel
...
from gui.Scaleform.daapi.view.battle.battle_royale.status_notifications import sn_items
...
class EventStatusNotificationTimerPanel(StatusNotificationTimerPanel):
    ...
    def _generateNotificationTimerSettings(self):
        ...
        self._addNotificationTimerSetting(data, _TIMER_STATES.STUN, BATTLE_NOTIFICATIONS_TIMER_LINKAGES.WT_STUN_ICON,
                                          link, BATTLE_NOTIFICATIONS_TIMER_COLORS.ORANGE, True, False,
                                          EVENT.STUN_INDICATOR, replace=True)
        self._addNotificationTimerSetting(data, _TIMER_STATES.OVERTURNED,
                                          BATTLE_NOTIFICATIONS_TIMER_LINKAGES.HALF_OVERTURNED_ICON,
                                          BATTLE_NOTIFICATIONS_TIMER_LINKAGES.BATTLE_ROYALE_DESTROY_TIMER_UI,
                                          noiseVisible=False, pulseVisible=False, iconOffsetY=-10)
        ...
        self._addNotificationTimerSetting(data, _TIMER_STATES.WT_BOMB_CARRY,
                                          BATTLE_NOTIFICATIONS_TIMER_LINKAGES.WT_BOMB_CARRY, link,
                                          BATTLE_NOTIFICATIONS_TIMER_COLORS.GREEN, True, False, iconOffsetY=11,
                                          tipKeyCode=sfKey)
        link = BATTLE_NOTIFICATIONS_TIMER_LINKAGES.BATTLE_ROYALE_TIMER_UI
        self._addNotificationTimerSetting(data, _TIMER_STATES.WT_BOMB_CAPTURE,
                                          BATTLE_NOTIFICATIONS_TIMER_LINKAGES.WT_BOMB_CAPTURE, link,
                                          BATTLE_NOTIFICATIONS_TIMER_COLORS.BLUE, True, False)
        self._addNotificationTimerSetting(data, _TIMER_STATES.WT_BOMB_DEPLOY,
                                          BATTLE_NOTIFICATIONS_TIMER_LINKAGES.WT_BOMB_DEPLOY, link,
                                          BATTLE_NOTIFICATIONS_TIMER_COLORS.BLUE, True, False)
        self._addNotificationTimerSetting(data, _TIMER_STATES.WT_BOMB_ABSORB,
                                          BATTLE_NOTIFICATIONS_TIMER_LINKAGES.WT_BOMB_ABSORB, link,
                                          BATTLE_NOTIFICATIONS_TIMER_COLORS.BLUE, True, False)
        return data

    def _addNotificationTimerSetting(self, data, typeId, iconName, linkage, color='', noiseVisible=False,
                                     pulseVisible=False, text='', countdownVisible=True, iconOffsetY=0, replace=False,
                                     tipKeyCode=None):
        if replace:
            for item in data:
