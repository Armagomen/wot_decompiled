...
class EventVehicleInfoComponent(vehicle.VehicleInfoComponent):
    ...
    def addVehicleInfo(self, vInfoVO, overrides):
        if not _isEventVehicle(vInfoVO):
            ...
            return self._data.update(
                {'vehicleType': 'boss' if isBoss(vInfoVO.vehicleType.tags) else vInfoVO.vehicleType.getClassName(),
    ...
    def __getBombData(self, vSpecific):
        ...
        return {EventKeys.BOMB_INDEX.value: vSpecific.getValue(EventKeys.BOMB_GUI_INDEX.value) if vSpecific else 0,
                ...
                EventKeys.BOMB_TIME_TOTAL.value: vSpecific.getValue(
                    EventKeys.BOMB_TIME_TOTAL.value) if vSpecific else 0,
    ...
    def __updateResurrect(self, vInfoVO):
        ...
        self._data.update({EventKeys.RESURRECT_TIME_LEFT.value: vSpecific.getValue(EventKeys.RESURRECT_TIME_LEFT.value),
                           EventKeys.RESURRECT_TIME_TOTAL.value: vSpecific.getValue(
                               EventKeys.RESURRECT_TIME_TOTAL.value)})
...
class EventStatisticsDataController(ClassicStatisticsDataController):
    ...
    def _createExchangeBroker(self, exchangeCtx):
        ...
        exchangeBroker.setVehiclesInfoExchange(
            vehicle.VehiclesExchangeBlock(EventVehicleInfoComponent(), positionComposer=broker.BiDirectionComposer(),
                                          idsComposers=(
                                          vehicle.TeamsSortedIDsComposer(), vehicle.TeamsCorrelationIDsComposer()),
                                          statsComposers=None))
        exchangeBroker.setVehiclesStatsExchange(
            vehicle.VehiclesExchangeBlock(EventVehicleStatsComponent(), positionComposer=broker.BiDirectionComposer(),
                                          idsComposers=None, statsComposers=(vehicle.TotalStatsComposer(),)))
        exchangeBroker.setVehicleStatusExchange(EventVehicleStatusComponent(
            idsComposers=(vehicle.TeamsSortedIDsComposer(), vehicle.TeamsCorrelationIDsComposer()),
