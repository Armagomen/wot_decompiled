...
class World2DMarkerComponent(_IMarkerComponentBase):
    ...
    def _createMarker(self):
        gui = self._gui()
        if gui and not self._isMarkerExists:
            self._isMarkerExists = gui.createMarker(self._componentID, self._symbol, self._matrixProduct,
                                                    self._isVisible)
    ...
    def _setupMarker(self, gui):
        gui.setupMarker(self._componentID, self._config.get('shape', 'arrow'), self._config.get('min-distance', 0),
                        self._config.get('max-distance', 0), self._distance,
                        self._config.get('distanceFieldColor', 'yellow'))
...
class MinimapMarkerComponent(_IMarkerComponentBase):
    ...
    def _createMarker(self):
        ...
        if gui and not self._isMarkerExists:
            ...
            self._isMarkerExists = gui.createMarker(self._componentID, self._config.get('symbol', ''),
                                                    self._config.get('container', ''), matrix=matrix,
                                                    active=self._isVisible)
            if self._isMarkerExists:
                self._setupMarker(gui)
