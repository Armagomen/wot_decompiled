...
class VehicleMarkerPlugin(MarkerPlugin, ChatCommunicationComponent, IArenaVehiclesController):
    ...
    __slots__ = (
    '_markers', '_markersStates', '_clazz', '__playerVehicleID', '_isSquadIndicatorEnabled', '__showDamageIcon',
    '_markerTimers', '__callbackIDs', '__targetedTankMarkerID', '__targetedMarkerFromCppID', '__equipmentCtrl',
    '__erased')
    ...
    def start(self):
        ...
        if ctrl is not None:
            ...
            ctrl.onVehicleMarkerErased += self.__onVehicleMarkerErased
    ...
    def addVehicleInfo(self, vInfo, arenaDP):
        if vInfo.isObserver():
            ...
            if marker is None:
                return
            self._setVehicleInfo(marker, vInfo, ctx.getPlayerGuiProps(vehicleID, vInfo.team),
                                 ctx.getPlayerFullNameParts(vehicleID))
            self._setMarkerInitialState(marker, accountDBID=vInfo.player.accountDBID)
            return
    ...
    def _setVehicleInfo(self, marker, vInfo, guiProps, nameParts):
        ...
        self._invokeMarker(markerID, 'setVehicleInfo', vType.classTag, vType.iconPath, nameParts.vehicleName,
                           vType.level, nameParts.playerFullName, nameParts.playerName, nameParts.clanAbbrev,
                           nameParts.regionCode, vType.maxHealth, guiPropsName, hunting, squadIndex,
                           backport.text(R.strings.ingame_gui.stun.seconds()))
    ...
    def _onVehicleFeedbackReceived(self, eventID, vehicleID, value):
        ...
        if eventID in MARKER_HIT_STATE and self.__showDamageIcon:
    ...
    def __updateInspireMarker(self, vehicleID, handle, isSourceVehicle, isInactivation, endTime, duration, primary=True, animated=True, equipmentID=None):
        if equipmentID is not None and isInactivation is not None:
            ...
            if isBerserk:
                self._updateMarkerTimer(vehicleID, handle, duration, BATTLE_MARKER_STATES.BERSERKER_STATE,
                                        showCountdown=False)
                return
    ...
    def __needsMarker(self, vInfo):
        return vInfo.isAlive() or not isSpawnedBot(
            vInfo.vehicleType.tags) and VEHICLE_TAGS.EVENT_BOT not in vInfo.vehicleType.tags and vInfo.vehicleID not in self.__erased
    ...
    def __onVehicleMarkerAdded(self, vProxy, vInfo, guiProps):
        if not self.__needsMarker(vInfo):
            ...
            if vehicleID in self._markers:
                ...
                if marker is None:
                    return
                self._setVehicleInfo(marker, vInfo, guiProps,
                                     self.sessionProvider.getCtx().getPlayerFullNameParts(vehicleID))
                self._setMarkerInitialState(marker, accountDBID=accountDBID)
            return
    ...
    def __onVehicleMarkerErased(self, vehicleID):
        self.__erased.append(vehicleID)
        self._hideVehicleMarker(vehicleID)
