...
class ConsumablesPanel(ConsumablesPanelMeta, BattleGUIKeyHandler, CallbackDelayer):
    ...
    def __resetStorages(self, storageRange, storageMask, clearKeys=False):
        for idx in storageRange:
            self._cds[idx] = None
            if clearKeys:
                if self.__extraKeys.has_key(idx):
                    del self.__extraKeys[idx]
                keyCode, _ = self.__genKey(idx)
                if self.__keys.has_key(keyCode):
                    del self.__keys[keyCode]
    ...
    def _addEquipmentSlot(self, idx, intCD, item):
        ...
        if item is None:
            ...
            self.as_addEquipmentSlotS(idx=idx, keyCode=bwKey, sfKeyCode=sfKey, tag=None, quantity=0, timeRemaining=0,
                                      reloadingTime=0, iconPath='', tooltipText=EMPTY_EQUIPMENT_TOOLTIP,
                                      animation=ANIMATION_TYPES.NONE)
            ...
            if self._isNeedsExtendTooltipBody(item):
                additionalStr = self._getAdditionalTooltipBodyString(item)
            ...
            self.as_addEquipmentSlotS(idx=idx, keyCode=bwKey, sfKeyCode=sfKey, tag=next(iter(tags), None),
                                      quantity=quantity, timeRemaining=timeRemaining, reloadingTime=reloadingTime,
                                      iconPath=iconPath, tooltipText=toolTip, animation=animationType)
        return
    ...
    def _removeListeners(self):
        g_eventBus.removeListener(GameEvent.CHOICE_CONSUMABLE, self.__handleConsumableChoice,
    ...
    def __handleEquipmentExpanded(self, intCD):
        ...
        if ctrl is None:
            ...
            if errorType == IgnoreEntitySelection:
                return
            elif not result and errorType not in (NoneType, NeedEntitySelection):
                ctrl = self.sessionProvider.shared.messages
                if ctrl is not None:
                    ctrl.showVehicleError(error.key, error.ctx)
                return
            elif errorType == InCooldownError:
                return
            elif intCD not in self._cds:
