...
class BattleHintData(namedtuple('_HintData', (
'name', 'componentAlias', 'duration', 'maxWaitTime', 'priority', 'hintType', 'soundFx', 'soundNotification',
'descriptionKey1', 'descriptionKey2', 'soundNotificationNewbie', 'backgroundColor'))):
...
def makeHintsData():
    ...
    if battleHintsConfig:
        for hint in battleHintsConfig.values():
            hints.append(BattleHintData(name=hint['name'].asString,
                                        componentAlias=_xml.readTupleOfStrings(_XML_CTX, hint, 'component'),
                                        hintType=HintType(hint['hintType'].asString),
                                        descriptionKey1=hint['descriptionKey1'].asString if hint.has_key(
                                            'descriptionKey1') else None,
                                        descriptionKey2=hint['descriptionKey2'].asString if hint.has_key(
                                            'descriptionKey2') else None,
                                        duration=hint['duration'].asFloat if hint.has_key('duration') else None,
                                        maxWaitTime=hint['maxWaitTime'].asFloat if hint.has_key('maxWaitTime') else 10,
                                        priority=hint['priority'].asInt if hint.has_key('priority') else 0,
                                        soundFx=hint['soundFx'].asString if hint.has_key('soundFx') else None,
                                        soundNotification=hint['soundNotification'].asString if hint.has_key(
                                            'soundNotification') else None, soundNotificationNewbie=hint[
                    'soundNotificationNewbie'].asString if hint.has_key('soundNotificationNewbie') else None,
                                        backgroundColor=hint['backgroundColor'].asString if hint.has_key(
                                            'backgroundColor') else None))
