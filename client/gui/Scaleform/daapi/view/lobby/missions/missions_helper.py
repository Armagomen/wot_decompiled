...
from gui.server_events.awards_formatters import AWARDS_SIZES, EPIC_AWARD_SIZE, getEpicAwardFormatter, \
    getEventAwardFormatter
...
from gui.server_events.cond_formatters.requirements import AccountRequirementsFormatter, TQAccountRequirementsFormatter, \
    WtAccountRequirementsFormatter
...
def _getWeeklyLimitedStatusKey(isWeekly):
    return QUESTS.MISSIONDETAILS_MISSIONSCOMPLETE_WEEKLY if isWeekly else QUESTS.MISSIONDETAILS_MISSIONSCOMPLETE
...
class _EventDailyMissionInfo(_MissionInfo):
    ...
    def _getCompleteStatusTooltipHeader(self):
        return backport.text(R.strings.quests.missionDetails.status.notAvailable()) if not self._controller.isDailyQuestsRefreshAvailable() else super(_EventDailyMissionInfo, self)._getCompleteStatusTooltipHeader()
...
class _EpicBattleDailyMissionInfo(_EventDailyMissionInfo):
    ...
    def _getCompleteKey(self):
        return R.strings.epic_battle.questsTooltip.epicBattle.timeLeft() if not self._controller.isDailyQuestsRefreshAvailable() else super(_EpicBattleDailyMissionInfo, self)._getCompleteKey()
...
class _PremiumMissionInfo(_MissionInfo):
    ...
    def _getUnavailableStatusFields(self, errorMsg):
        if errorMsg == 'requirements':
            ...
            clockIcon = _getClockIconTag()
            statusMsg = text_styles.concatStylesWithSpace(clockIcon,
                                                          text_styles.error(QUESTS.MISSIONDETAILS_STATUS_NOTAVAILABLE))
        return {'statusLabel': statusMsg,
                'status': MISSIONS_STATES.NOT_AVAILABLE}

    def _getRegularStatusFields(self, isLimited, bonusCount, bonusLimit):
        statusLabel = text_styles.concatStylesWithSpace(icons.inProgress(), text_styles.neutral(
            backport.text(R.strings.quests.personalMission.status.inProgress())))
        return {'statusLabel': statusLabel,
                'status': MISSIONS_STATES.NONE}
...
class _WTMissionInfo(_MissionInfo):
    ...
    def _getUnavailableStatusFields(self, errorMsg):
        ...
        if errorMsg == WT_QUEST_UNAVAILABLE_NOT_ENOUGH_TICKETS_REASON:
            notAvailableIcon = _getNotAvailableIconTag()
            statusMsg = text_styles.concatStylesWithSpace(notAvailableIcon,
                                                          text_styles.error(QUESTS.MISSIONDETAILS_STATUS_NOTAVAILABLE))
    ...
    def _getCompleteDailyStatus(self, completeKey):
        return text_styles.main(backport.text(R.strings.event.tooltips.header.quests.header.description.till_end(),
                                              time=text_styles.stats(getDaysLeftFormatted(
                                                  gameEventController=self.__eventController)))) if self.__eventController.isLastSeasonDay() else super(
            _WTMissionInfo, self)._getCompleteDailyStatus(completeKey)

    def _getCompleteStatusTooltipHeader(self):
        isAnyPrimeNow = self.__eventController.hasAvailablePrimeTimeServers()
        return backport.text(
            R.strings.quests.missionDetails.status.notAvailable()) if not isAnyPrimeNow else backport.text(
            R.strings.tooltips.quests.unavailable.time.statusTooltip())

    def _getCompleteKey(self):
        return R.strings.event.tooltips.header.quests.header.description.till_end() if self.__eventController.isLastSeasonDay() else super(
            _WTMissionInfo, self)._getCompleteKey()
...
class _DetailedPersonalMissionInfo(_MissionInfo):
    ...
    def __isPawnAvailable(self, quest):
        return self.eventsCache.getPersonalMissions().getFreeTokensCount(
            quest.getPMType().branch) >= quest.getPawnCost()
...
class _DetailedWTMissionInfo(_DetailedMissionInfo):
    ...
    def _getCompletedDateLabel(self):
        dateLabel = text_styles.concatStylesWithSpace(_getClockIconTag(), text_styles.error(
            backport.text(R.strings.quests.missionDetails.status.notAvailable())), self.__getLabelBody())
        return dateLabel

    def __getLabelBody(self):
        return text_styles.main(backport.text(R.strings.event.tooltips.header.quests.header.description.till_end(),
                                              time=text_styles.stats(getDaysLeftFormatted(
                                                  gameEventController=self.__eventController)))) if self.__eventController.isLastSeasonDay() else self._getCompleteDailyStatus(
            self._getCompleteKey())
...
def getMissionInfoData(event):
    ...
    elif event.getGroupID() == EPIC_BATTLE_GROUPS_ID:
