...
from gui.sounds.ambients import BattlePassSoundEnv, LobbySubViewEnv, MarathonPageSoundEnv, MissionsCategoriesSoundEnv, \
    MissionsEventsSoundEnv, MissionsPremiumSoundEnv, BattleMattersSoundEnv
from helpers import dependency
from helpers.i18n import makeString as _ms
from items import getTypeOfCompactDescr
from skeletons.gui.app_loader import GuiGlobalSpaceID, IAppLoader
from skeletons.gui.event_boards_controllers import IEventBoardController
from skeletons.gui.game_control import IBattlePassController, IBattleRoyaleController, IGameSessionController, \
    IMapboxController, IMarathonEventsController, IRankedBattlesController, IUISpamController
...
TabData = namedtuple('TabData', ('alias',
                                 ...
                                 'prefix'))
TABS_DATA_ORDERED = [
    TabData(QUESTS_ALIASES.MISSIONS_EVENT_BOARDS_VIEW_PY_ALIAS, QUESTS_ALIASES.MISSIONS_EVENT_BOARDS_VIEW_LINKAGE,
            QUESTS.MISSIONS_TAB_EVENTBOARDS, QUESTS.MISSIONS_TAB_EVENTBOARDS_DISABLED,
            _ms(QUESTS.MISSIONS_TAB_LABEL_EVENTBOARDS), None),
    TabData(QUESTS_ALIASES.MISSIONS_GROUPED_VIEW_PY_ALIAS, QUESTS_ALIASES.MISSIONS_GROUPED_VIEW_LINKAGE,
            QUESTS.MISSIONS_TAB_MARATHONS, QUESTS.MISSIONS_TAB_MARATHONS, _ms(QUESTS.MISSIONS_TAB_LABEL_MARATHON),
            None),
    TabData(QUESTS_ALIASES.BATTLE_MATTERS_VIEW_PY_ALIAS, QUESTS_ALIASES.BATTLE_MATTERS_VIEW_LINKAGE,
            QUESTS.MISSIONS_TAB_BATTLEMATTERS, QUESTS.MISSIONS_TAB_BATTLEMATTERS,
            backport.text(R.strings.battle_matters.battleMattersTab()), None),
    TabData(QUESTS_ALIASES.MAPBOX_VIEW_PY_ALIAS, QUESTS_ALIASES.MAPBOX_VIEW_LINKAGE, QUESTS.MISSIONS_TAB_MAPBOX,
            QUESTS.MISSIONS_TAB_MAPBOX, backport.text(R.strings.mapbox.mapboxTab()), None),
    TabData(QUESTS_ALIASES.BATTLE_PASS_MISSIONS_VIEW_PY_ALIAS, QUESTS_ALIASES.BATTLE_PASS_MISSIONS_VIEW_LINKAGE,
            QUESTS.MISSIONS_TAB_BATTLE_PASS, QUESTS.MISSIONS_TAB_BATTLE_PASS, _ms(BATTLE_PASS.BATTLEPASSTAB), None),
    TabData(QUESTS_ALIASES.MISSIONS_CATEGORIES_VIEW_PY_ALIAS, QUESTS_ALIASES.MISSIONS_CATEGORIES_VIEW_LINKAGE,
            QUESTS.MISSIONS_TAB_CATEGORIES, QUESTS.MISSIONS_TAB_CATEGORIES, _ms(QUESTS.MISSIONS_TAB_LABEL_CATEGORIES),
            None),
    TabData(QUESTS_ALIASES.MISSIONS_PREMIUM_VIEW_PY_ALIAS, QUESTS_ALIASES.MISSIONS_PREMIUM_VIEW_LINKAGE,
            QUESTS.MISSIONS_TAB_DAILY, QUESTS.MISSIONS_TAB_DAILY, _ms(QUESTS.MISSIONS_TAB_LABEL_DAILY), None)]
...
class MissionsPage(LobbySubView, MissionsPageMeta):
    ...
    backport.sound(R.sounds.ev_mapbox_enter()), backport.sound(R.sounds.ev_mapbox_exit())),
                     QUESTS_ALIASES.BATTLE_MATTERS_VIEW_PY_ALIAS: (
                     backport.sound(R.sounds.bm_enter()), backport.sound(R.sounds.bm_exit()))}
    ...
    def _populate(self):
        ...
        if self.marathonsCtrl.isAnyActive():
            TABS_DATA_ORDERED.insert(MARATHONS_START_TAB_INDEX, TabData(QUESTS_ALIASES.MISSIONS_GROUPED_VIEW_PY_ALIAS,
                                                                        QUESTS_ALIASES.MISSIONS_GROUPED_VIEW_LINKAGE,
                                                                        QUESTS.MISSIONS_TAB_MARATHONS,
                                                                        QUESTS.MISSIONS_TAB_MARATHONS,
                                                                        _ms(QUESTS.MISSIONS_TAB_LABEL_MARATHON), None))
        self.__updateHeader()
    ...
    def __getHeaderTabData(self, tabData):
        ...
        if alias == QUESTS_ALIASES.MISSIONS_EVENT_BOARDS_VIEW_PY_ALIAS and not self.__elenHasEvents() or alias == QUESTS_ALIASES.MISSIONS_MARATHON_VIEW_PY_ALIAS and not (
                marathonEvent and marathonEvent.isEnabled()) or alias == QUESTS_ALIASES.MISSIONS_GROUPED_VIEW_PY_ALIAS and (
                self.marathonsCtrl.doesShowAnyMissionsTab() or self.__mapboxCtrl.isEnabled() and self.__mapboxCtrl.getCurrentCycleID() is not None) or alias == QUESTS_ALIASES.BATTLE_PASS_MISSIONS_VIEW_PY_ALIAS and self.battlePass.isDisabled() or alias == QUESTS_ALIASES.MAPBOX_VIEW_PY_ALIAS and (
                not self.__mapboxCtrl.isEnabled() or self.__mapboxCtrl.getCurrentCycleID() is None) or alias == QUESTS_ALIASES.BATTLE_MATTERS_VIEW_PY_ALIAS and not self.__battleMattersTabIsEnabled():
             ...
             QUESTS_ALIASES.MAPBOX_VIEW_PY_ALIAS):
                ...
                if alias == QUESTS_ALIASES.MISSIONS_PREMIUM_VIEW_PY_ALIAS:
                    ...
                    if alias == QUESTS_ALIASES.MISSIONS_CATEGORIES_VIEW_PY_ALIAS:
                        from gui.Scaleform.daapi.view.lobby.missions.regular.missions_views import \
                            MissionsCategoriesView
                        advisableQuests = self.eventsCache.getAdvisableQuests(
                            MissionsCategoriesView.getViewQuestFilter())
                        advisableQuests.update(
                            {q.getID(): q for q in self.__getSuitableEvents(self.getComponent(alias))})
                    else:
                        advisableQuests = self.eventsCache.getAdvisableQuests()
    ...
    def __getSuitableEvents(tab):
        if not tab:
            return []
        return [quest for quest in tab.getSuitableEvents() if
                not isBattleMattersQuestID(quest.getGroupID()) or quest.isAvailable().isValid]
    ...
    def __showFilter(self):
        self.as_showFilterS(self.__currentTabAlias not in (QUESTS_ALIASES.MISSIONS_EVENT_BOARDS_VIEW_PY_ALIAS,
                                                           ...
                                                           QUESTS_ALIASES.MAPBOX_VIEW_PY_ALIAS,
                                                           QUESTS_ALIASES.BATTLE_MATTERS_VIEW_PY_ALIAS),
                            self.__currentTabAlias not in NON_FLASH_TABS)
