...
class EventBattleQueue(EventBattleQueueMeta):
    __eventController = dependency.instance(IGameEventController)
    ...
    def __onTokensUpdate(self, diff):
        ...
        if config.quickBossTicketToken in diff:
            if self.__eventController.getQuickTicketCount():
                self.__showQuickStartPanel(self.__eventController.getQuickTicketExpiryTime(),
                                           self.__packBossQStartPanelData())
            else:
            ...
            if self.__eventController.getQuickHunterTicketCount():
                self.__showQuickStartPanel(self.__eventController.getQuickHunterTicketExpiryTime(),
                                           self.__packHunterQStartPanelData())
            else:
                self.__hideQuickStartPanel(cancelHideQuickStartPanelCallback=True)
    ...
    def __packBossQStartPanelData(self):
        ...
        vehicles = self.__itemsCache.items.getVehicles(
            REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS({WT_TAGS.BOSS}))
        if not vehicles:
    ...
    def __packHunterQStartPanelData(self):
        vehicles = self.__itemsCache.items.getVehicles(
            REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS({WT_TAGS.HUNTER}))
        hunters = []
