...
from gui.server_events.events_dispatcher import showPersonalMission, showMissionsElen, showMissionsMarathon, \
    ...
    showMissionsBattlePassCommonProgression, showMissionsMapboxProgression
...
from skeletons.gui.game_control import IMarathonEventsController, IFestivityController, IRankedBattlesController, \
    IQuestsController, IBattleRoyaleController, IMapboxController, IEpicBattleMetaGameController, IGameEventController
...
class HangarHeader(HangarHeaderMeta, IGlobalListener, IEventBoardsListener):
    ...
    def _makeHeaderVO(self):
        ...
        return {'isVisible': True,
                'quests': self._getCommonQuestsToHeaderVO(
                    self._currentVehicle.item)} if self._currentVehicle.isPresent() else emptyHeaderVO
    ...
    def __getEventQuestVO(self, vehicle):
        ...
        questsVO = [
            self._headerQuestFormaterVo(enable=isEnabled, icon=backport.image(icon), label=label, stateIcon=stateIcon,
                                        questType=HANGAR_HEADER_QUESTS.QUEST_TYPE_EVENT_BATTLES,
                                        flag=backport.image(flag),
                                        tooltip=TOOLTIPS_CONSTANTS.EVENT_BATTLES_QUESTS_PREVIEW, isTooltipSpecial=True)]
    ...
    def __updateBPWidget(self):
        ...
        isValidBattleType = self.prbDispatcher and self.prbDispatcher.getEntity() and self.__battlePassController.isValidBattleType(
