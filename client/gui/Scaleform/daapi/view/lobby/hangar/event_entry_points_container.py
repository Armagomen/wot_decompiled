...
class _EntryPointData(object):
    ...
    def getIsEnabledByValidator(self):
        ...
        if self.id in _ENTRY_POINT_ENABLED_VALIDATOR:
            ...
            _logger.error(
                'Entry point "%s" validator error. You should add None object or function for config ' + 'check in _ENTRY_POINT_ENABLED_VALIDATOR.',
                self.id)
        return result
...
class EventEntryPointsContainer(EventEntryPointsContainerMeta, Notifiable, IGlobalListener):
    ...
    def _isRandomBattleSelected(self):
        return self.prbDispatcher.getFunctionalState().isQueueSelected(
            QUEUE_TYPE.RANDOMS) if self.prbDispatcher is not None else False
