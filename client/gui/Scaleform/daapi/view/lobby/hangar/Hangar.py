...
from gui.shared.event_dispatcher import showRankedPrimeTimeWindow, showAmmunitionSetupView, \
    showEpicBattlesPrimeTimeWindow, showEventBattlesPrimeTimeWindow
...
from skeletons.gui.game_control import IRankedBattlesController, IEpicBattleMetaGameController, IPromoController, \
    IIGRController, IBattlePassController, IBattleRoyaleController, IBootcampController, IMapboxController, \
...
class Hangar(LobbySelectableView, HangarMeta, IGlobalListener):
        ...
        if self.prbDispatcher is not None:
            if self.prbDispatcher.getFunctionalState().isInPreQueue(QUEUE_TYPE.RANKED):
                newCarouselAlias = HANGAR_ALIASES.RANKED_TANK_CAROUSEL
                    ...
                    QUEUE_TYPE.EPIC) or self.prbDispatcher.getFunctionalState().isInUnit(PREBATTLE_TYPE.EPIC):
            ...
            elif self.battlePassController.isVisible() and self.battlePassController.isValidBattleType(
                ...
                self.tankMenXpPanel.as_setTankmenXpPanelS(False, False)
    ...
    def __updateState(self):
        ...
        if isC11nEnabled:
            customizationTooltip = makeTooltip(_ms(backport.text(R.strings.tooltips.hangar.tuning.header())),
                                               _ms(backport.text(R.strings.tooltips.hangar.tuning.body())))
        else:
            customizationTooltip = makeTooltip(_ms(backport.text(R.strings.tooltips.hangar.tuning.disabled.header())),
                                               _ms(backport.text(R.strings.tooltips.hangar.tuning.disabled.body())))
    ...
    def __updateLootBoxesState(self):
        if self.prbDispatcher:
            ...
            isVisible = not self.bootcampController.isInBootcamp() and self.gameEventController.isEnabled() and (
                        state.isInPreQueue(QUEUE_TYPE.RANDOMS) or state.isInUnit(
                    PREBATTLE_TYPE.SQUAD) or state.isInLegacy(PREBATTLE_TYPE.TRAINING) or state.isInPreQueue(
                    QUEUE_TYPE.EVENT_BATTLES) or state.isInUnit(PREBATTLE_TYPE.EVENT))
        else:
            isVisible = False
