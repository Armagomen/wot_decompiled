...
from account_helpers.AccountSettings import NEW_LOBBY_TAB_COUNTER, RECRUIT_NOTIFICATIONS, NEW_SHOP_TABS, \
    LAST_SHOP_ACTION_COUNTER_MODIFICATION, OVERRIDEN_HEADER_COUNTER_ACTION_ALIASES
...
from skeletons.gui.game_control import IAnonymizerController, IBadgesController, IBoostersController, \
    IBootcampController, IChinaController, IEpicBattleMetaGameController, IGameSessionController, IIGRController, \
    IRankedBattlesController, IServerStatsController, IWalletController, IClanNotificationController, \
    IBattleRoyaleController, IUISpamController, IPlatoonController, IMapboxController, IMapsTrainingController, \
    IGameEventController
...
class LobbyHeader(LobbyHeaderMeta, ClanEmblemsHelper, IGlobalListener):
    ...
    uiModeSelectorButtonLogger = ModeSelectorEntryPointLogger()
    ...
    def onPrbEntitySwitched(self):
        self._updatePrebattleControls()
        self.uiModeSelectorButtonLogger.onPrbEntitySwitched()
    ...
    def closeFullscreenBattleSelector(self):
        self._closeFullScreenBattleSelector(LOG_CLOSE_DETAILS.SELECTOR)

    def _closeFullScreenBattleSelector(self, details):
        from gui.impl.lobby.mode_selector.mode_selector_view import ModeSelectorView
        view = self.gui.windowsManager.getViewByLayoutID(ModeSelectorView.layoutID)
        if view is not None:
            ModeSelectorView.uiLogger.log(LOG_ACTIONS.CLOSED, details=details)
            view.close()
        return
    ...
    def _dispose(self):
        ...
        if tooltipMgr:
            tooltipMgr.onShow -= self.__onShowTooltip
            tooltipMgr.onHide -= self.__onHideTooltip
        self.uiModeSelectorButtonLogger.reset()
    ...
    def __updateAccountCompletionStatus(self):
        if not account_completion.isEnabled() or self.bootcampController.isInBootcamp():
            return
        response = yield future_async.await(self.wgnpController.getEmailStatus())
    ...
    def __getEventTooltipData(self, restriction):
        rClass = R.strings.tooltips.hangar.startBtn
        rSubClass = rClass.primeNotAvailable
        if restriction == UNIT_RESTRICTION.COMMANDER_VEHICLE_NOT_SELECTED:
            rSubClass = R.strings.event.hangar.startBtn.eventSquadNotReady.wrongVehicleCount
        elif restriction == UNIT_RESTRICTION.EVENT_VEHICLE_NOT_SELECTED:
            rSubClass = rClass.squadNotReady
        elif restriction == PREBATTLE_RESTRICTION.TICKETS_SHORTAGE:
            rSubClass = rClass.noTicket
        elif restriction == PREBATTLE_RESTRICTION.VEHICLE_IN_BATTLE:
            rSubClass = R.strings.tooltips.redButton.disabled.vehicle.inBattle
        elif restriction == UNIT_RESTRICTION.VEHICLE_IS_IN_BATTLE:
            rSubClass = R.strings.tooltips.redButton.disabled.vehicle.inBattle
        elif restriction == UNIT_RESTRICTION.IS_IN_ARENA:
    ...
    def __getMapsTrainingTooltipData(self):
    ...
    def __unitMgr_onUnitLeft(self, _, __):
        self._updatePrebattleControls()
    ...
    def isEventEnabled(self):
        ...
        isEventAvailable = self.gameEventController.isEnabled()
        mapboxAvailable = self.__mapboxCtrl.isActive() and self.__mapboxCtrl.isInPrimeTime()
        return not self.bootcampController.isInBootcamp() and (
                    self.rankedController.isAvailable() or isEpicBattleAvailabe or brAvailable or mapboxAvailable or isEventAvailable)
    ...
    def _updatePrebattleControls(self, *_):
        ...
        isEvent = state.isInPreQueue(constants.QUEUE_TYPE.EVENT_BATTLES) or state.isInUnit(
        ...
        if self.__isHeaderButtonPresent(LobbyHeader.BUTTONS.SQUAD):
            ...
            self.as_updateSquadS(isInSquad, tooltip, TOOLTIP_TYPES.COMPLEX, False, squadSelected.squadIcon,
                                 hasSearchSupport, extendedSquadInfoVo._asdict())
        ...
        if self.__isFightBtnDisabled and not state.hasLockedState:
            ...
            elif g_currentVehicle.isOnlyForEpicBattles() and (
                    g_currentVehicle.isUnsuitableToQueue() or g_currentVehicle.isDisabledInRent()):
    ...
    def __onHideTooltip(self, tooltip):
        if tooltip == self.__SELECTOR_TOOLTIP_TYPE:
            self.uiModeSelectorButtonLogger.tooltipClosed(SELECTOR_BUTTON_TOOLTIP_LOG_ID)
    ...
    def __processMMActiveTestConfirm(self, prbEntity, callback):
        ...
        if toShow:
            result = yield future_async.await(showActiveTestConfirmDialog(config.get('startTime', 0.0), config.get('finishTime', 0.0), config.get('link', '')))
            if result:
