...
from account_helpers.AccountSettings import AccountSettings, QUESTS, QUEST_DELTAS, QUEST_DELTAS_COMPLETION, \
    ACTIVE_TEST_PARTICIPATION_CONFIRMED
...
from gui.Scaleform.daapi.view.lobby.header.fight_btn_tooltips import getRankedFightBtnTooltipData, \
    getEpicFightBtnTooltipData, getSandboxTooltipData, getSquadFightBtnTooltipData, getMapsTrainingTooltipData, \
    getUnsuitableToQueueTooltipData, getEventTooltipData, getPreviewTooltipData, getRandomTooltipData, \
    getMapboxFightBtnTooltipData
...
from gui.shared.event_dispatcher import showShop, showStorage, hideWebBrowserOverlay, showModeSelectorWindow, \
    showActiveTestConfirmDialog
...
from skeletons.gui.game_control import IAnonymizerController, IBadgesController, IBoostersController, \
    ...
    ISteamCompletionController, IEventBattlesController
...
class LobbyHeader(LobbyHeaderMeta, ClanEmblemsHelper, IGlobalListener):
    ...
    def __updateAccountCompletionStatus(self):
        if not self.steamRegistrationCtrl.isSteamAccount or self.bootcampController.isInBootcamp():
            return
        status = yield future_async.await(self.wgnpSteamAccCtrl.getEmailStatus())
    ...
    def isEventEnabled(self):
        ...
        return not self.bootcampController.isInBootcamp() and (
                    self.rankedController.isAvailable() or isEpicBattleAvailabe or brAvailable or mapboxAvailable or isEventBattlesAvailable)
        ...
        if self._isLobbyHeaderControlsDisabled:
            ...
            if self.__isFightBtnDisabled and not state.hasLockedState:
                if isSandbox:
                    ...
                    tooltipData = getUnsuitableToQueueTooltipData(result)
