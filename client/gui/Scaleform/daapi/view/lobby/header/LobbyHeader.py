...
from gui.impl.lobby.common.main_menu_utils import TABS as _TABS
from gui.impl.lobby.common.lobby_header_utils import HeaderMenuVisibilityState, HEADER_BUTTONS_COUNTERS_CHANGED_EVENT, findExtensionTooltip, LobbyHeaderVisibilityAction
...
from skeletons.gui.game_control import IAnonymizerController, IBadgesController, IBattleRoyaleController, IBoostersController, IChinaController, IClanNotificationController, IEpicBattleMetaGameController, IGameSessionController, IIGRController, ILimitedUIController, IMapboxController, IMapsTrainingController, IPlatoonController, IRankedBattlesController, IServerStatsController, ISteamCompletionController, IWalletController, IAchievements20Controller, IAchievementsController, IHangarGuiController, IExchangeRatesWithDiscountsProvider
...
LobbyHeaderTabInfo = namedtuple('LobbyHeaderTabInfo', (
 'wulfAlias', 'label', 'tooltip', 'showFunction', 'conditionFunction'))
...
class LobbyHeader(LobbyHeaderMeta, ClanEmblemsHelper, IGlobalListener):
    ...
    def __init__(self):
        ...
        self.__externalTabs = collectLobbyHeaderTabs()
        self.__externalTabsWulfToTab = self.__getExternalTabsWulfToTab(self.__externalTabs)
        return
    ...
    def _addListeners(self):
        ...
        self.addListener(events.LobbyHeaderMenuEvent.DESELECT_HEADER_BUTTONS, self.__deselectHeaderButtons, scope=EVENT_BUS_SCOPE.LOBBY)
    ...
    def __getExternalTabsWulfToTab(self, externalTabs):
        return {tabInfo.wulfAlias:alias for alias, tabInfo in externalTabs.items()}
    ...
    def _removeListeners(self):
        ...
        self.removeListener(events.LobbyHeaderMenuEvent.UPDATE_PREBATTLE_CONTROLS, self._updatePrebattleControls, scope=EVENT_BUS_SCOPE.LOBBY)
        ...
        self.removeListener(events.LobbyHeaderMenuEvent.DESELECT_HEADER_BUTTONS, self.__deselectHeaderButtons, scope=EVENT_BUS_SCOPE.LOBBY)
        self.anonymizerController.onStateChanged -= self.__updateAnonymizedState
        self.gameSession.onPremiumNotify -= self.__onPremiumTimeChanged
        self.gameSession.onPremiumTypeChanged -= self._onPremiumTypeChanged
        self.wallet.onWalletStatusChanged -= self.__onWalletChanged
    ...
    def __onViewAddedToContainer(self, _, pyEntity):
        if pyEntity.layer is WindowLayer.SUB_VIEW:
            pyAlias = pyEntity.alias
            alias = self._WULF_TO_TAB.get(pyAlias, self.__externalTabsWulfToTab.get(pyAlias, pyAlias))
            if alias in self.TABS.ALL() or alias in self.__externalTabs:
                self.__setCurrentScreen(alias)
    ...
    def _updatePrebattleControls(self, *_):
        if self._isLobbyHeaderControlsDisabled:
            ...
            if prbDispatcher is None or prbEntity is None:
                return
    ...
    def __deselectHeaderButtons(self, *_):
    ...
    def _getTournamentsSelectorData(self):
        ...
        if isTournamentEnabled():
            result = {'label': MENU.HEADERBUTTONS_TOURNAMENT, 'value': VIEW_ALIAS.LOBBY_TOURNAMENTS, 
    ...
    def _getHangarMenuItemDataProvider(self):
        ...
        for alias, tabInfo in self.__externalTabs.items():
            if tabInfo.conditionFunction(self):
                tabDataProvider.extend([
                 {'label': backport.text(tabInfo.label), 
                    'value': alias, 
    ...
    def __updateTabCounter(self, alias, counter=None):
        if alias not in self.TABS.ALL() and alias not in self.__externalTabs:
            return
        else:
    ...
    def __updateModeControls(self, pFuncState, isNewbie):
        items = battle_selector_items.getItems()
        selectedItem = items.update(pFuncState)
    ...
    def __updateSquadControls(self, controlsHelper, pFuncState, pValidation, isNewbie):
        selectedSquadItem = battle_selector_items.getSquadItems().update(pFuncState)
        isInSquad = any(pFuncState.isInUnit(prbType) for prbType in PREBATTLE_TYPE.SQUAD_PREBATTLES)
        isSquadEnabled = isInSquad or self.platoonCtrl.getPermissions().canCreateSquad()
        self.as_doDisableHeaderButtonS(self.BUTTONS.SQUAD, isEnabled=isSquadEnabled)
        if selectedSquadItem.isDisabled():
        ...
        if pFuncState.isNavigationDisabled():
            ...
            tooltipType = TOOLTIP_TYPES.COMPLEX
        self.as_updateSquadS(isInSquad=isInSquad, tooltip=tooltip, tooltipType=tooltipType, isEvent=controlsHelper is not None and controlsHelper.isEventSquadEnable(), icon=selectedSquadItem.getSmallIcon(), hasPopover=self.platoonCtrl.hasSelectorPopover(), eventBgLinkage=selectedSquadItem.getHighlightLinkage(isNewbie), data=extendedSquadInfoVo._asdict())
        return isInSquad

    def __updateFightControls(self, controlsHelper, pFuncState, pValidation, selectedItem, isInSquad, isNewbie, pInfo):
        self.__isFightBtnDisabled = self._checkFightButtonDisabled(pValidation.isValid, selectedItem.isLocked())
        ...
        if not tooltip and controlsHelper is not None:
            tooltip, isSpecial = controlsHelper.getFightControlTooltipData(pValidation, isInSquad, self.__isFightBtnDisabled, isNavigationEnabled)
        self.as_setFightBtnTooltipS(tooltip, isSpecial)
        self.as_setFightButtonS(selectedItem.getFightButtonLabel(pFuncState, pInfo))
        self.as_setFightButtonHighlightS(selectedItem.getFightButtonHighlight(isNewbie))
        return
