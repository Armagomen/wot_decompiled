...
class _EventSquadItem(_SpecialSquadItem):
    ...
    def _update(self, state):
        ...
        self._isSelected = state.isQueueSelected(QUEUE_TYPE.EVENT_BATTLES)
        primeTimeStatus, _, _ = self.__gameEventCtrl.getPrimeTimeStatus()
        self._isVisible = self.__gameEventCtrl.isEnabled() and self.__gameEventCtrl.isInPrimeTime() and state.isInPreQueue(
...
def _addEpicTrainingBattleType(items, settings=None):
    ...
    items.append(
        _EpicTrainingItem(backport.text(_R_BATTLE_TYPES.epicTraining()), PREBATTLE_ACTION_NAME.EPIC_TRAINING_LIST, 11,
                          SELECTOR_BATTLE_TYPES.EPIC, isVisible=visible))
    return


def _addEventBattlesType(items):
    items.append(_EventBattlesItem(backport.text(_R_EVENT_BATTLE.title()), PREBATTLE_ACTION_NAME.EVENT_BATTLE, 2,
                                   SELECTOR_BATTLE_TYPES.EVENT))


def _addSimpleSquadType(items):
    items.append(_SimpleSquadItem(text_styles.middleTitle(backport.text(_R_BATTLE_TYPES.simpleSquad())),
                                  PREBATTLE_ACTION_NAME.SQUAD, 0))
