...
class BadgesSuffixItem(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        return [
         formatters.packBuildUpBlockData([self.packHeader(), self.packBody()], gap=-2)]
...
class BadgesSuffixRankedItem(BadgesSuffixItem):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        items.append(formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text_styles.main(backport.text(R.strings.tooltips.badgepage.ranked.suffixItem.duration())))], padding=formatters.packPadding(top=-34), layout=BLOCKS_TOOLTIP_TYPES.LAYOUT_HORIZONTAL))
        return items

    def packBody(self):
        items = [
         formatters.packTextBlockData(text_styles.main(backport.text(R.strings.tooltips.badgepage.ranked.suffixItem.position())), padding=formatters.packPadding(bottom=4))]
