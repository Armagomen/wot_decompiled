    ...
    def _buildTooltip(self, peripheryID):
        if not self.getTimeLeft():
            tooltipStr = text_styles.expText(
                backport.text(R.strings.event.primeTime.endOfCycle(), server=self.getName()))
        else:
            timeStr = text_styles.neutral(backport.getTillTimeStringByRClass(self.getTimeLeft(),
                                                                             R.strings.menu.Time.timeValueShort.noLeadingZeroes))
            if self._getIsAvailable():
                tooltipStr = text_styles.expText(
                    backport.text(R.strings.event.primeTime.serverTooltip(), server=self.getName(), time=timeStr))
            else:
                tooltipStr = text_styles.expText(
                    backport.text(R.strings.event.primeTime.serverUnavailableTooltip(), server=self.getName(),
                                  time=timeStr))
    ...
    def __getStatusTitle(self):
        ...
        if status == PrimeTimeStatus.NOT_AVAILABLE:
            ...
            if timeLeft < time_utils.ONE_DAY:
                ...
                startTime = time_formatters.formatDate('%d.%m.%Y',
                                                       time_utils.getCurrentLocalServerTimestamp() + timeLeft)
            return text_styles.grandTitle(
                backport.text(R.strings.event.primeTime.status.untill(), startTime=startTime, server=currServerName))
        return text_styles.grandTitle(
            backport.text(R.strings.event.primeTime.status.disableFirst(), server=currServerName)) if status in (
        PrimeTimeStatus.FROZEN, PrimeTimeStatus.NOT_SET) else text_styles.grandTitle(
            backport.text(R.strings.event.primeTime.status.allServersDisabled()))
    ...
    def __getTimeText(self, serverInfo):
        if serverInfo is None:
            ...
            if currentSeason and not timeLeft:
                return text_styles.grandTitle(backport.text(R.strings.event.primeTime.status.allServersDisabled()))
            timeLeftStr = backport.getTillTimeStringByRClass(timeLeft,
                                                             R.strings.menu.Time.timeValueShort.noLeadingZeroes)
            if isAvailable:
