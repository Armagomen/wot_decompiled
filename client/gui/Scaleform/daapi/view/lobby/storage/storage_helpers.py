...
_HANDLERS_MAP = {GUI_ITEM_TYPE.OPTIONALDEVICE: (
                                CONTEXT_MENU_HANDLER_TYPE.STORAGE_OPTIONAL_DEVICE_ITEM,
                                CONTEXT_MENU_HANDLER_TYPE.STORAGE_OPTIONAL_MODERNIZED_DEVICE_ITEM), 
   GUI_ITEM_TYPE.EQUIPMENT: (
                           CONTEXT_MENU_HANDLER_TYPE.STORAGE_EQUIPMENT_ITEM,), 
   GUI_ITEM_TYPE.BATTLE_BOOSTER: (
                                CONTEXT_MENU_HANDLER_TYPE.STORAGE_BONS_ITEM,), 
   GUI_ITEM_TYPE.TURRET: (
                        CONTEXT_MENU_HANDLER_TYPE.STORAGE_MODULES_SHELLS_ITEM,), 
   ...
   GUI_ITEM_TYPE.CREW_BOOKS: (
                            CONTEXT_MENU_HANDLER_TYPE.STORAGE_CREW_BOOKS_NO_SALE_ITEM,
                            CONTEXT_MENU_HANDLER_TYPE.STORAGE_MODULES_SHELLS_ITEM), 
   GUI_ITEM_TYPE.DEMOUNT_KIT: (
                             CONTEXT_MENU_HANDLER_TYPE.STORAGE_DEMOUNT_KIT_ITEM,), 
   GUI_ITEM_TYPE.RECERTIFICATION_FORM: (
                                      CONTEXT_MENU_HANDLER_TYPE.STORAGE_RECERTIFICATION_FORM_ITEM,), 
   GUI_ITEM_TYPE.MENTORING_LICENSE: (
                                   CONTEXT_MENU_HANDLER_TYPE.STORAGE_MENTORING_LICENSE_ITEM,)}
...
def _getContextMenuHandlerID(item):
    ...
    if len(handlers) > 1:
        return handlers[item.isForSale]
    return handlers[0]
...
def getStorageItemDescr(item):
    ...
    if itemType in GUI_ITEM_TYPE.VEHICLE_MODULES:
        return _generateDescr(item, i18n.makeString(STORAGE.MODULECOMPATIBLE_LABEL), STORAGE.MODULECOMPATIBLE_MOREVEHICLES, ('vehicles',
                                                                                                                             'clipVehicles',
                                                                                                                             'uniChargedVehicles'), _MAX_COMPATIBLE_VEHS_COUNT)
    else:
        if itemType == GUI_ITEM_TYPE.SHELL:
        ...
        if itemType == GUI_ITEM_TYPE.BATTLE_BOOSTER:
...
def getVehicleName(vehicle):
    return (' ').join((
     ...
     text_styles.neutral(int2roman(vehicle.level)),
...
def _getVehicleDescription(vehicle):
    return (' ').join((
     ...
     backport.getIntegralFormat(_calculateVehicleMaxAdditionalPrice(vehicle)),
     icons.credits()))
...
def _generateDescr(item, label, moreLabel, paramsNames, maxItemsCount):
    ...
    descrItems = [
     label,
     (', ').join(compatiblesSet[:maxItemsCount])]
...
def getItemExtraParams(item):
    if item.itemTypeID == GUI_ITEM_TYPE.OPTIONALDEVICE:
        return OptDeviceBonusesDescriptionBuilder().getDescriptionByKpiStrings(item)
    return ()
