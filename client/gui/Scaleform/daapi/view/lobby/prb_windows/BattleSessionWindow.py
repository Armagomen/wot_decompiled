...
class BattleSessionWindow(BattleSessionWindowMeta):
    ...
    def onSettingUpdated(self, entity, settingName, settingValue):
        if settingName == PREBATTLE_SETTING_NAME.ARENA_TYPE_ID:
            ...
            self.as_setInfoS(self.__isTurnamentBattle, self.__battlesWinsString, self.__arenaName, self.__firstTeam,
                             self.__secondTeam, self.prbEntity.getProps().getBattlesScore(), self.__eventName,
                             self.__sessionName)
    ...
    def __doRequestToUnassignMember(self, pID):
        yield self.prbDispatcher.sendPrbRequest(
            AssignLegacyCtx(pID, self._getPlayerTeam() | PREBATTLE_ROSTER.UNASSIGNED, 'prebattle/assign'))
    ...
    def _setRosterList(self, rosters):
        ...
        self.as_setRosterListS(playerTeam, True, self._makeAccountsData(rosters[playerTeam | PREBATTLE_ROSTER.ASSIGNED],
                                                                        assignedComparator))
        self.as_setRosterListS(playerTeam, False,
                               self._makeAccountsData(rosters[playerTeam | PREBATTLE_ROSTER.UNASSIGNED],
                                                      unassignedComparator))
    ...
    def __updateCommonRequirements(self, teamLimits, rosters):
        ...
        self.as_setPlayersCountTextS(
            makeHtmlString('html_templates:lobby/prebattle', key, {'membersCount': playersCount,
