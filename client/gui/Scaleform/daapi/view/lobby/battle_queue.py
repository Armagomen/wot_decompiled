...
from gui.shared.gui_items.Vehicle import VEHICLE_CLASS_NAME, getTypeBigIconPath, VEHICLE_TAGS
...
class _EventQueueProvider(_RandomQueueProvider):
    EVENT_TYPES_ORDERED = (VEHICLE_TAGS.EVENT_BOSS, VEHICLE_TAGS.EVENT_HUNTER)
    ...
    def processQueueInfo(self, qInfo):
        ...
        uiData = []
        counts = {VEHICLE_TAGS.EVENT_BOSS: bosses,
                  VEHICLE_TAGS.EVENT_HUNTER: hunters}
        for vTypeName in self.EVENT_TYPES_ORDERED:
            uiData.append({'type': backport.text(R.strings.event.vehicle.tags.dyn(vTypeName).name()),
        ...
        self._proxy.as_setAverageTimeS(
            i18n.makeString(backport.text(R.strings.event.battleQueue.avgWaitTime.label(), vehName=vehicle.userName)),
            _timeLabel(avgWaitTime))
...
class BattleQueue(BattleQueueMeta, LobbySubView):
    ...
    def _dispose(self):
        self.__stopUpdateScreen()
        g_playerEvents.onArenaCreated -= self.onStartBattle
        self.removeListener(events.GameEvent.SHOW_EXTERNAL_COMPONENTS, self._onShowExternals,
                            scope=EVENT_BUS_SCOPE.GLOBAL)
        self.removeListener(events.GameEvent.HIDE_EXTERNAL_COMPONENTS, self._onHideExternals,
                            scope=EVENT_BUS_SCOPE.GLOBAL)
        self._blur.fini()
        self.fireEvent(events.HangarVehicleEvent(events.HangarVehicleEvent.HERO_TANK_MARKER, ctx={'isDisable': False}),
