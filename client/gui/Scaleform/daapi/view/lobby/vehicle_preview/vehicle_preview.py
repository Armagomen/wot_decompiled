...
from skeletons.gui.game_control import IHeroTankController, IVehicleComparisonBasket, IGameEventController
...
class VehiclePreview(LobbySelectableView, VehiclePreviewMeta):
    ...
    def _populate(self):
        ...
        g_eventBus.addListener(events.HangarSimpleEvent.EVENT_PORTAL_SELECTED, self.__onCloseView,
                               EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.addListener(events.HangarSimpleEvent.EVENT_VEHICLE_SELECTED, self.__onCloseView,
                               EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.addListener(events.HangarSimpleEvent.PORTAL_MANAGER_ACTIVATED, self.__onCloseView,
                               EVENT_BUS_SCOPE.LOBBY)
        self.setBottomPanel()
        ...
        g_eventBus.handleEvent(events.HangarSimpleEvent(events.HangarSimpleEvent.VEHICLE_PREVIEW_LOADED),
                               scope=EVENT_BUS_SCOPE.LOBBY)
        return

    def _dispose(self):
        g_eventBus.handleEvent(events.HangarSimpleEvent(events.HangarSimpleEvent.VEHICLE_PREVIEW_UNLOADED),
                               scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.HangarSimpleEvent.EVENT_PORTAL_SELECTED, self.__onCloseView,
                                  EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.HangarSimpleEvent.EVENT_VEHICLE_SELECTED, self.__onCloseView,
                                  EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.HangarSimpleEvent.PORTAL_MANAGER_ACTIVATED, self.__onCloseView,
                                  EVENT_BUS_SCOPE.LOBBY)
        specialData = getHeroTankPreviewParams() if self.__isHeroTank else None
    ...
    def __getInfoPanelListDescription(vehicle):
        ...
        hasEquipBonuses = any(itertools.chain(vehicle.optDevices.installed, vehicle.battleAbilities.installed,
                                              vehicle.battleBoosters.installed,
                                              (rCons and rCons.kpi for rCons in vehicle.consumables.installed),
