...
class WtEventVehiclePreview(ConfigurableVehiclePreview):
    ...
    def _populate(self):
        ...
        g_eventBus.handleEvent(events.LobbySimpleEvent(events.HangarSimpleEvent.EVENT_VEHICLE_PREVIEW_OPEN),
                               scope=EVENT_BUS_SCOPE.LOBBY)
        self.__eventCtrl.onUpdated += self.__eventUpdated

    def _dispose(self):
        g_eventBus.handleEvent(events.LobbySimpleEvent(events.HangarSimpleEvent.EVENT_VEHICLE_PREVIEW_CLOSE),
                               scope=EVENT_BUS_SCOPE.LOBBY)
        self.__eventCtrl.onUpdated -= self.__eventUpdated
