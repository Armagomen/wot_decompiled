...
class VehiclePreviewTopPanelTabsView(ViewImpl):
    ...
    def __onTabChanged(self, *args, **kwargs):
        ...
        if callable(command):
            command(self.__parentCtx.get('itemCD'), style=self.__style if self.__currentTabID == TabID.STYLE else None, topPanelData=self.__makeTopPanelData(), itemsPack=self.__parentCtx.get('itemsPack'))
