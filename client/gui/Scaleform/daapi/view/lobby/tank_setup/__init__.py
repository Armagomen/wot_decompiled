...
def getContextMenuHandlers():
    return (
     (
      CONTEXT_MENU_HANDLER_TYPE.TANK_SETUP_OPTIONAL_DEVICE_ITEM, OptDeviceItemContextMenu),
     (
...
def getViewSettings():
    ...
    return (
     ViewSettings(VIEW_ALIAS.AMMUNITION_SETUP_VIEW, AmmunitionSetupView, 'ammunitionSetupView.swf', WindowLayer.TOP_SUB_VIEW, VIEW_ALIAS.AMMUNITION_SETUP_VIEW, ScopeTemplates.LOBBY_SUB_SCOPE, True, isModal=True),
     ComponentSettings(HANGAR_ALIASES.AMMUNITION_SETUP_VIEW_INJECT, AmmunitionSetupViewAdaptor, ScopeTemplates.DEFAULT_SCOPE),
     ComponentSettings(HANGAR_ALIASES.AMMUNITION_SETUP_VIEW_VEHICLE_PARAMS, AmmunitionSetupViewVehicleParams, ScopeTemplates.DEFAULT_SCOPE))


def getBusinessHandlers():
    return (
     _TankSetupBusinessHandler(),)
