...
class ConsumablesPanelMeta(BaseDAAPIComponent):
    ...
    def as_setItemTimeQuantityInSlotS(self, idx, quantity, timeRemaining, maxTime, animation, stage):
        return self.flashObject.as_setItemTimeQuantityInSlot(idx, quantity, timeRemaining, maxTime, animation,
                                                             stage) if self._isDAAPIInited() else None

    def as_setCoolDownTimeS(self, idx, duration, baseTime, startTime):
        return self.flashObject.as_setCoolDownTime(idx, duration, baseTime,
                                                   startTime) if self._isDAAPIInited() else None
    ...
    def as_setCoolDownTimeSnapshotS(self, idx, time, isBaseTime, isFlash):
        return self.flashObject.as_setCoolDownTimeSnapshot(idx, time, isBaseTime,
                                                           isFlash) if self._isDAAPIInited() else None

    def as_addShellSlotS(self, idx, keyCode, sfKeyCode, quantity, clipCapacity, shellIconPath, noShellIconPath,
                         tooltipText):
        return self.flashObject.as_addShellSlot(idx, keyCode, sfKeyCode, quantity, clipCapacity, shellIconPath,
                                                noShellIconPath, tooltipText) if self._isDAAPIInited() else None
    ...
    def as_addEquipmentSlotS(self, idx, keyCode, sfKeyCode, tag, quantity, timeRemaining, reloadingTime, iconPath,
                             tooltipText, animation):
        return self.flashObject.as_addEquipmentSlot(idx, keyCode, sfKeyCode, tag, quantity, timeRemaining,
                                                    reloadingTime, iconPath, tooltipText,
                                                    animation) if self._isDAAPIInited() else None
