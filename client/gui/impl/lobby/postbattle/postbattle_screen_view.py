...
from gui.impl.gen.view_models.views.lobby.postbattle.detailed_personal_efficiency_model import \
    DetailedPersonalEfficiencyModel
...
class PostbattleScreenView(ViewImpl):
    __slots__ = ('__arenaUniqueID', '__tooltipParametersCreator', '__tooltipContentCreator', '__eventPlugin',
                 '__questBnsTooltipData')
    ...
    def createContextMenu(self, event):
        if event.contentID == R.views.common.BackportContextMenu():
            ...
            window = BackportContextMenuWindow(
                createContextMenuData(CONTEXT_MENU_HANDLER_TYPE.BATTLE_RESULTS_USER, args), self.getParentWindow())
            window.load()
            return window
        else:
    ...
    def _initialize(self, *args, **kwargs):
        ...
        self.__addListeners()
        g_eventBus.handleEvent(LobbyHeaderMenuEvent(LobbyHeaderMenuEvent.TOGGLE_VISIBILITY,
    ...
    def _finalize(self):
        ...
        g_eventBus.handleEvent(
            LobbyHeaderMenuEvent(LobbyHeaderMenuEvent.TOGGLE_VISIBILITY, ctx={'state': HeaderMenuVisibilityState.ALL}),
            EVENT_BUS_SCOPE.LOBBY)
    ...
    def __getQuestBonusTooltipData(self, tooltipID):
        ...
        if len(ids) != 2:
            ...
            return questData.get(int(tIdx))

    def __getTooltipContentCreator(self):
        creatorMap = {R.views.lobby.postbattle.tooltips.ProgressiveReward(): self.__getProgressiveRewardTooltipContent,
    ...
    def __getAchievementTooltipParameters(self, event):
        ...
        args = self.__battleResults.presenter.getAchievementTooltipData(self.__arenaUniqueID, achievementID,
                                                                        achievementName, isPersonal)
        return createTooltipData(isSpecial=True, specialAlias=getAchievementTooltipType(achievementName),
                                 specialArgs=args)
    ...
    def __getEfficiencyTooltipParameters(self, event):
        ...
        args = [self.__battleResults.presenter.getEfficiencyTooltipData(self.__arenaUniqueID, parameterName,
                                                                        enemyVehicleID=enemyVehicleID)]
    ...
    def __getTotalEfficiencyTooltipParameters(self, event):
        args = [self.__battleResults.presenter.getTotalEfficiencyTooltipData(self.__arenaUniqueID,
                                                                             event.getArgument('type'))]
        return createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.TOTAL_EFFICIENCY_PARAM,
                                 specialArgs=args)
