...
class TankmanInfoAdapter(TankmanAdapter):
    ...
    def getSkillsLabel(self):
        return backport.text(
            R.strings.tooltips.vehiclePreview.tankman.skillsTitle()) if self._tankmanInfo.skills else ''
...
class WTTankmanInfoAdapter(TankmanInfoAdapter):
    ...
    def getLabel(self):
        ...
        return backport.text(_STR_PATH.status.boss()) if isBoss(vehicleType.tags) else backport.text(
            _STR_PATH.status.hunter(), vehicle=vehicleType.userString)
    ...
    def getSkillsLabel(self):
        return backport.text(_STR_PATH.skillsTitle()) if self._tankmanInfo.skills else ''
