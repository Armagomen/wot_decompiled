...
class SquadMembersView(ViewImpl, CallbackDelayer):
    ...
    def _addListeners(self):
        ...
        g_eventBus.addListener(events.CoolDownEvent.PREBATTLE, self.__handleSetPrebattleCoolDown,
                               scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.addListener(events.FightButtonEvent.FIGHT_BUTTON_UPDATE, self._updateReadyButton,
                               scope=EVENT_BUS_SCOPE.LOBBY)
    ...
    def _removeListeners(self):
        ...
        if unitMgr and unitMgr.unit:
            unitMgr.unit.onUnitEstimateInQueueChanged -= self._updateMembers
        g_eventBus.removeListener(events.CoolDownEvent.PREBATTLE, self.__handleSetPrebattleCoolDown,
                                  scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.FightButtonEvent.FIGHT_BUTTON_UPDATE, self._updateReadyButton,
                                  scope=EVENT_BUS_SCOPE.LOBBY)
        self.__lobbyContext.getServerSettings().onServerSettingsChange -= self.__onServerSettingsChange
        self.__platoonCtrl.onAvailableTiersForSearchChanged -= self.__onAvailableTiersForSearchChanged
        self.__platoonCtrl.onAutoSearchCooldownChanged -= self._updateFindPlayersButton
...
class EventMembersView(SquadMembersView):
    ...
    def _getTitle(self):
        title = ''.join((i18n.makeString(backport.text(R.strings.platoon.event_squad())),
