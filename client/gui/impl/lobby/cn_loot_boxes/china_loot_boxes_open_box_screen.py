...
from account_helpers.AccountSettings import LOOT_BOXES_OPEN_ANIMATION_STATE
...
from gui.impl.gen.view_models.views.lobby.cn_loot_boxes.china_loot_boxes_open_box_screen_model import BoxType, \
    ChinaLootBoxesOpenBoxScreenModel, OSBit
from gui.impl.gen.view_models.views.lobby.cn_loot_boxes.vehicle_model import VehicleType
from gui.impl.lobby.cn_loot_boxes.china_loot_boxes_sound import enterLootBoxState, exitLootBoxState, playStorageClosed, \
    playStorageOpened
from gui.impl.lobby.cn_loot_boxes.tooltips.china_loot_boxes_compensation_tooltip import \
    ChinaLootBoxesCompensationTooltip
...
class ChinaLootBoxesOpenBoxScreen(ViewImpl):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        with self.viewModel.transaction() as model:
            ...
            model.setUseExternalShop(self.__cnLootBoxes.useExternalShop())
            self.__setRewards(model=model)
        enterLootBoxState()

    def _onLoaded(self, *args, **kwargs):
        self.__inBoxesFlowEnter()
        if not AccountSettings.getSettings(LOOT_BOXES_OPEN_ANIMATION_STATE):
            self.__onHideWaiting()
        self.__updateBoxesCount()
    ...
    def __changeAnimationState(self):
        isAnimationActive = AccountSettings.getSettings(LOOT_BOXES_OPEN_ANIMATION_STATE)
        AccountSettings.setSettings(LOOT_BOXES_OPEN_ANIMATION_STATE, not isAnimationActive)
        if self.__osBit == OSBit.X64:
            self.__updateAnimationState()

    @replaceNoneKwargsModel
    def __updateAnimationState(self, model=None):
        isAnimationActive = AccountSettings.getSettings(LOOT_BOXES_OPEN_ANIMATION_STATE) and self.__osBit == OSBit.X64
