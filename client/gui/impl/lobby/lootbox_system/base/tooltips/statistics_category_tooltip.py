...
class StatisticsCategoryTooltipView(ViewImpl):
    __slots__ = ('__bonusesCategory', '__eventName')
    ...
    def __setBonuses(self, model=None):
        ...
        rewards, _ = self.__lootBoxes.getStatistics(self.__eventName)
...
def _packCustomization(rewards, model):
    for customization, count in _iterCustomizations(rewards):
        bonusModel = StatisticsCategoryTooltipBonusModel()
...
def _packAttachments(rewards, model):
    for attachment, count in _iterAttachments(rewards):
        bonusModel = StatisticsCategoryTooltipBonusModel()
        bonusModel.setName(attachment.itemTypeName)
        bonusModel.setIcon(attachment.rarity)
        bonusModel.setCount(count)
...
@dependency.replace_none_kwargs(customization=ICustomizationService)
def _iterAttachments(rewards, customization=None):
    return ((customization.getItemByID(getItemTypeID(cData['custType']), cData['id']), cData['value']) for cData in (c for c in rewards.get(Type.CUSTOMIZATIONS.value, {})) if cData['custType'] == GUI_ITEM_TYPE_NAMES[GUI_ITEM_TYPE.ATTACHMENT])
