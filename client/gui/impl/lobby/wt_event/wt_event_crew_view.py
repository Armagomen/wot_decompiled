...
class WTEventCrewView(ViewImpl, IGlobalListener):
    ...
    def __init__(self, *args, **kwargs):
        settings = ViewSettings(layoutID=R.views.lobby.wt_event.WTEventCrew(), flags=ViewFlags.COMPONENT,
                                model=WtEventCrewModel())
        settings.args = args
    ...
    def __getBackportTooltipData(self, event):
        ...
        if not tooltipId:
            ...
            return TooltipData(tooltip=tooltipId, isSpecial=True, specialAlias=tooltipId, specialArgs=[_SIXTH_SENSE,
                                                                                                       int(event.getArgument(
                                                                                                           'invID'))]) if tooltipId == WtEventCrewModel.SKILL_TOOLTIP else None
