...
from gui.impl.gen.view_models.views.lobby.wt_event.wt_event_multiopen_renderer_model import \
    WtEventMultiopenRendererModel
...
from gui.wt_event.wt_event_models_helper import setLootBoxesCount, setGuaranteedAward, fillAdditionalAwards, \
    fillVehicleAward
...
class WtEventPortalAwards(WtEventBasePortalAwards):
    ...
    def createToolTipContent(self, event, contentID):
        return WtGuaranteedRewardTooltipView() if contentID == R.views.lobby.wt_event.tooltips.WtGuaranteedRewardTooltipView() else super(
            WtEventPortalAwards, self).createToolTipContent(event, contentID)
    ...
    def _goToPreview(self, args):
        ...
        if intCD == 0:
            ...
            if itemType == GUI_ITEM_TYPE.VEHICLE:
                ...
                event_dispatcher.showStylePreview(vehicleCD, item, item.getDescription(),
                                                  partial(_backToAwardView, self.__lootBoxType, self._awards),
                                                  backBtnDescrLabel=backport.text(
                                                      R.strings.event.awardView.backToAwards()))
            return

    def _goToStorage(self):
        playLootboxBackToPortalsExitEvent()
        g_eventBus.handleEvent(events.WtEventPortalsEvent(events.WtEventPortalsEvent.ON_PORTAL_AWARD_VIEW_CLOSED),
                               scope=EVENT_BUS_SCOPE.LOBBY)
        event_dispatcher.showEventStorageWindow()

    def __goToPortal(self):
        g_eventBus.handleEvent(events.WtEventPortalsEvent(events.WtEventPortalsEvent.ON_BACK_TO_PORTAL),
                               scope=EVENT_BUS_SCOPE.LOBBY)
    ...
    def __animationEnded(self, args):
        ...
        if isAnimationEnded:
            event_dispatcher.showVehicleAwardWindow(vehicleSource=SpecialVehicleSource.MULTIPLE_OPENING,
                                                    parent=self.getParentWindow())

    def __openMore(self):
        parent = self.getParentWindow()
        self._boxesCtrl.requestLootBoxOpen(self.__lootBoxType, self.__count, parentWindow=parent,
                                           callbackUpdate=self.__updateData)
...
class WtEventPortalAwardsWindow(LobbyWindow):
    ...
    def __init__(self, lootBoxType, awards, count, openedCount, parent=None):
        super(WtEventPortalAwardsWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN,
                                                        content=WtEventPortalAwards(lootBoxType=lootBoxType,
                                                                                    awards=awards, count=count,
...
def _fillMainAwards(lootBoxType, model, bonuses, tooltipItems):
    ...
    packBonusModelAndTooltipData(sorted(bonuses, key=lambda bonus: sortBonuses(bonus, order)), model, tooltipItems,
                                 getWtEventBonusPacker)
