...
class WTEventCarouselView(ViewImpl, IGlobalListener):
    ...
    def __init__(self, *args, **kwargs):
        settings = ViewSettings(layoutID=R.views.lobby.wt_event.WTEventCarousel(), flags=ViewFlags.COMPONENT,
                                model=WtEventCarouselViewModel())
        settings.args = args
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            tooltipData = createTooltipData(isSpecial=True, specialAlias=event.getArgument('tooltipId'),
                                            specialArgs=(event.getArgument('id'), 1))
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return WtEventAmmunitionTooltipView(intCD=event.getArgument(
            'id')) if contentID == R.views.lobby.wt_event.tooltips.WtEventAmmunitionTooltipView() else super(
            WTEventCarouselView, self).createToolTipContent(event, contentID)
    ...
    def __addListeners(self):
        ...
        g_eventBus.addListener(events.HangarSimpleEvent.UPDATE_CAROUSEL_VEHICLE_STATES, self.__updateStates,
    ...
    def __removeListeners(self):
        ...
        self.viewModel.status.onBuyTicket -= self.__onBuyTicketClick
        self.viewModel.status.onOpenTasks -= self.__onOpenMissionsClick
        g_eventBus.removeListener(events.HangarSimpleEvent.UPDATE_CAROUSEL_VEHICLE_STATES, self.__updateStates,
                                  EVENT_BUS_SCOPE.LOBBY)
    ...
    def __update(self):
        ...
        if self.__isInBattle != vehicle.isInBattle:
            ...
            g_eventBus.handleEvent(events.FightButtonEvent(events.FightButtonEvent.FIGHT_BUTTON_UPDATE),
...
def _fillVehicles(array, item, itemsCache=None, gameEventCtrl=None):
    vehicles = itemsCache.items.getVehicles(
        REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_ANY_TAGS(_TAGS.EVENT_VEHS))
    array.clearItems()
