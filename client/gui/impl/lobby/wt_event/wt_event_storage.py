...
from gui.wt_event.wt_event_models_helper import setLootBoxesCount, setVehicleExchangeCost, setSpecialTankAvailability, \
    setGuaranteedAward
...
class WTEventStorageView(WtEventBasePortalsView):
    ...
    def _addListeners(self):
        ...
        g_eventBus.addListener(events.WtEventPortalsEvent.ON_PORTAL_VIEW_CLOSED, self.__onPortalViewClose,
                               EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.addListener(events.WtEventPortalsEvent.ON_ALL_PORTAL_VIEWS_CLOSED, self.__goToHangar,
                               EVENT_BUS_SCOPE.LOBBY)

    def _removeListeners(self):
        g_eventBus.removeListener(events.WtEventPortalsEvent.ON_PORTAL_VIEW_CLOSED, self.__onPortalViewClose,
                                  EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.WtEventPortalsEvent.ON_ALL_PORTAL_VIEWS_CLOSED, self.__goToHangar,
                                  EVENT_BUS_SCOPE.LOBBY)
        self.viewModel.onPortalClick -= self.__goToPortal
    ...
    def _updateModel(self):
        ...
        with self.viewModel.transaction() as model:
            ...
            self.__setPortalInfo(model.hunterPortal, EventLootBoxes.WT_HUNTER)
            self.__setPortalInfo(model.bossPortal, EventLootBoxes.WT_BOSS)

    def _onEventUpdated(self):
        with self.viewModel.transaction() as model:
            setVehicleExchangeCost(model.tankPortal)
    ...
    def __setPortalInfo(self, model, lootBoxType):
        setLootBoxesCount(model, lootBoxType)
        model.setAttemptPrice(getPortalCost(lootBoxType))
    ...
    def __canOpenPortal(self, portalType):
        if portalType == PortalType.TANK:
            ...
            if vehicle is not None:
                return not vehicle.isInInventory and not vehicle.isRestorePossible()
            return True
        else:
...
class WtEventStorageWindow(LobbyWindow):
    ...
    def __init__(self, parent=None):
        super(WtEventStorageWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN,
                                                   content=WTEventStorageView(R.views.lobby.wt_event.WtEventPortal()),
