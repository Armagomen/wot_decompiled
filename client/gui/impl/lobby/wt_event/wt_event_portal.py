...
from gui.wt_event.wt_event_models_helper import fillVehicleModel, setLootBoxesCount, setVehicleExchangeCost, \
    setBonusVehicles, setSpecialTankAvailability, setGuaranteedAward
from gui.wt_event.wt_event_simple_bonus_packers import getWtEventSimpleBonusPacker, fillPortalRewardModel, sortBonuses, \
    packSpecialVehicleBonus, HUNTER_BONUSES_ORDER, BOSS_BONUSES_ORDER
...
class WTEventPortalView(WtEventBasePortalsView):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return WtGuaranteedRewardTooltipView() if contentID == R.views.lobby.wt_event.tooltips.WtGuaranteedRewardTooltipView() else super(
            WTEventPortalView, self).createToolTipContent(event, contentID)
    ...
    def _addListeners(self):
        ...
        g_eventBus.addListener(events.WtEventPortalsEvent.ON_PORTAL_AWARD_VIEW_CLOSED, self._onPortalAwardsViewClose,
                               EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.addListener(events.WtEventPortalsEvent.ON_BACK_TO_PORTAL, self.__onPortalAwardsViewClosed,
                               EVENT_BUS_SCOPE.LOBBY)
    ...
    def _removeListeners(self):
        ...
        self.viewModel.onRunPortalClick -= self.__onRunPortal
        self.viewModel.onPreviewTankClick -= self.__onShowPreview
        g_eventBus.removeListener(events.WtEventPortalsEvent.ON_PORTAL_AWARD_VIEW_CLOSED, self._onPortalAwardsViewClose,
                                  EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.WtEventPortalsEvent.ON_BACK_TO_PORTAL, self.__onPortalAwardsViewClosed,
                                  EVENT_BUS_SCOPE.LOBBY)
    ...
    def _onClosedByUser(self):
        ...
        g_eventBus.handleEvent(events.WtEventPortalsEvent(events.WtEventPortalsEvent.ON_PORTAL_VIEW_CLOSED),
    ...
    def __onRunPortal(self, args=None):
        if self.__portalType == PortalType.TANK:
            ...
            openCount = int(args.get('runPortalTimes',
                                     _DEFAULT_RUN_PORTAL_TIMES)) if args is not None else _DEFAULT_RUN_PORTAL_TIMES
            parent = self.getParentWindow()
    ...
    def __setHighProbBonuses(self, lootBoxType, bonuses, model):
        ...
        if bonusGroup is None:
            ...
            packBonusModelAndTooltipData(
                bonuses=sorted(bonusGroup.bonuses, key=lambda bonus: sortBonuses(bonus, order)),
                packer=getWtEventSimpleBonusPacker(), model=bonusesListModel, tooltipData=self.__tooltipItems)
...
class WtEventPortalWindow(LobbyWindow):
    ...
    def __init__(self, portalType, defaultRunPortalTimes, parent=None):
        super(WtEventPortalWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN,
                                                  content=WTEventPortalView(portalType, defaultRunPortalTimes),
                                                  parent=parent, layer=WindowLayer.FULLSCREEN_WINDOW)
