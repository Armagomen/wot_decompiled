...
from gui.shared.event_dispatcher import showBrowserOverlayView, showEventCollectionWindow
...
class WTEventWelcomeView(ViewImpl):
    ...
    def __init__(self, *args, **kwargs):
        settings = ViewSettings(layoutID=R.views.lobby.wt_event.WTEventWelcome(), flags=ViewFlags.LOBBY_TOP_SUB_VIEW,
                                model=WtEventWelcomeModel())
        settings.args = args
    ...
    def __onCloseHandler(self):
        showEventCollectionWindow(fromWelcome=True)
        self.destroyWindow()
