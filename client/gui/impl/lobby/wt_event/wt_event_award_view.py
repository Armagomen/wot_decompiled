...
class WTEventAwardView(ViewImpl):
    ...
    def createToolTipContent(self, event, contentID):
        return WtEventLootBoxTooltipView(isHunterLootBox=event.getArgument(
            'isHunterLootBox')) if contentID == R.views.lobby.wt_event.tooltips.WtEventLootBoxTooltipView() else super(
            WTEventAwardView, self).createToolTipContent(event, contentID)
    ...
    def _onLoading(self, questId, *args, **kwargs):
        ...
        if isPostBattle:
            ...
            isFinalReward = elementsCount == self.__gameController.getCollectionSize()
            if isFinalReward:
                statusText = backport.text(rAwardsView.status.all())
                WTEventAwardsScreenSound.playCollectionDoneSound()
            else:
                statusText = backport.text(rAwardsView.status.part(), elements=elementsCount)
                WTEventAwardsScreenSound.playCollectionProgressSound()
    ...
    def _finalize(self):
        ...
        WTEventAwardsScreenSound.playCollectionClosed()
...
class WTEventAwardWindow(LobbyNotificationWindow):
    ...
    def __init__(self, questId, parent=None):
        super(WTEventAwardWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN,
                                                 content=WTEventAwardView(questId=questId), parent=parent)
...
class WTEventSpecialAwardWindow(LobbyWindow):
    ...
    def __init__(self, questId, parent=None):
        super(WTEventSpecialAwardWindow, self).__init__(wndFlags=WindowFlags.WINDOW_FULLSCREEN,
                                                        content=WTEventAwardView(questId=questId),
                                                        layer=WindowLayer.OVERLAY, parent=parent)
