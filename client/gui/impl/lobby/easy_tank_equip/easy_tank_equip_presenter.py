...
class EasyTankEquipPresenter(ViewComponent[EasyTankEquipViewModel]):
    __appLoader = dependency.descriptor(IAppLoader)
    __easyTankEquipCtrl = dependency.descriptor(IEasyTankEquipController)
    __itemsCache = dependency.descriptor(IItemsCache)

    def __init__(self):
        super(EasyTankEquipPresenter, self).__init__(model=EasyTankEquipViewModel, enabled=False)
        ...
        self.__isApplyBtnClicked = False
        self.__uiLogger = EasyTankEquipLogger()
        return

    @property
    def viewModel(self):
        return super(EasyTankEquipPresenter, self).getViewModel()
    ...
    def onEntered(self):
        ...
        if self.copyVehicle:
            ...
            self._registerChild(R.aliases.hangar.shared.ETEVehicleParams(), self.__vehicleParamsView)
            self._subscribe()

    def onExit(self):
        self._unsubscribe()
        uid = self._childrenUidByPosition.get(R.aliases.hangar.shared.ETEVehicleParams())
        if uid:
            self._unregisterChild(uid, True)
        self.__uiLogger.onViewClose(self.__isApplyBtnClicked, self.__getCardsLogInfo())
        if self.__isApplyBtnClicked:
            self.__isApplyBtnClicked = False
        self.__clearProposal()
