...
from gui.impl.gen.view_models.views.battle.battle_page.prebattle_shell_ammunition_slot import \
    PrebattleShellAmmunitionSlot, ShellBattleState
...
class PrebattleAmmunitionBlocksController(AmmunitionBlocksController):
    ...
    def _updateShells(self, viewModel, isFirst=False):
        if self.__ammunitionPanelState == State.PREBATTLE:
            PrebattleShellsBlock(self._vehicle, self._currentSection, self.__nextShellIntCD,
                                 self.__currentShellIntCD).adapt(viewModel, isFirst)
        else:
