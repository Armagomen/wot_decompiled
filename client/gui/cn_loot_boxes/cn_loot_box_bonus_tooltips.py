...
class CNLootBoxVehicleBlueprintFragmentTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, vehicleCDs):
        ...
        titleBlock = formatters.packTitleDescBlock(title=text_styles.highTitle(
            backport.text(R.strings.tooltips.blueprint.BlueprintFragmentTooltip.fragmentHeader())),
                                                   padding=formatters.packPadding(top=12))
    ...
    def __packVehicleFragmentBlock(self, vehicleCD):
        ...
        iconSource = R.images.gui.maps.icons.blueprints.fragment.medium.dyn(
            'vehicle' if not isFinal else 'vehicle_complete', default=R.invalid)()
        return formatters.packImageTextBlockData(title=self.__getVehicleDescrStr(vehicle),
                                                 img=backport.image(iconSource),
                                                 txtPadding=formatters.packPadding(left=21), titleAtMiddle=True)
    ...
    def __getVehicleDescrStr(vehicle):
        ...
        if vehicle.isElite:
            vehTypeIcon = icons.makeImageTag(
                source=backport.image(R.images.gui.maps.icons.vehicleTypes.elite.dyn(vehicleType, default=R.invalid)()),
                width=28, height=28, vSpace=-6)
        else:
            vehTypeIcon = icons.makeImageTag(
                source=backport.image(R.images.gui.maps.icons.vehicleTypes.dyn(vehicleType, default=R.invalid)()))
        return text_styles.concatStylesToSingleLine(text_styles.mainBig(int2roman(vehicle.level)), vehTypeIcon,
                                                    text_styles.mainBig(vehicle.userName))
