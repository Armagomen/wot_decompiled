...
from gui.shared.missions.packers.bonus import BACKPORT_TOOLTIP_CONTENT_ID, BlueprintBonusUIPacker, BonusUIPacker, \
    CrewBookBonusUIPacker, CustomizationBonusUIPacker, GoodiesBonusUIPacker, ItemBonusUIPacker, SimpleBonusUIPacker, \
    TokenBonusUIPacker, VEHICLE_RENT_ICON_POSTFIX, VehiclesBonusUIPacker, getDefaultBonusPackersMap
...
class TmanTemplateBonusPacker(SimpleBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        for tokenID in bonus.getTokens().iterkeys():
            if tokenID.startswith(RECRUIT_TMAN_TOKEN_PREFIX):
                tooltipData.append(
                    TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.TANKMAN_NOT_RECRUITED,
                                specialArgs=[tokenID]))
...
class CNLootBoxVehiclesBonusUIPacker(VehiclesBonusUIPacker):
    ...
    def _packVehicleBonusModel(cls, bonus, vehInfo, isRent, vehicle):
        ...
        if bonus.isCompensation():
            model.setIconSource(R.images.gui.maps.icons.cnLootBoxes.rewards.s600x450.gold())
        return model
...
class VehicleBlueprintsPacker(object):
    ...
    def getTooltip(bonuses):
        ...
        return TooltipData(tooltip=None, isSpecial=True,
                           specialAlias=TOOLTIPS_CONSTANTS.CN_LOOT_BOXES_VEHICLE_BLUEPRINT_FRAGMENT,
                           specialArgs=[fragmentCDs])
...
class InfoTypeTmanTemplateBonusPacker(InfoTypeRewardPacker):
    ...
    def __packTmanTemplateToken(cls, tokenID, bonus):
        ...
        if recruit is None:
            ...
            cls._packCommon(bonus, model)
            model.setIcon(
                _R_BONUS_ICONS.dyn(AWARDS_SIZES.SMALL).dyn('tankwoman' if recruit.isFemale() else 'tankman')())
            return model
...
def packBonusModelAndTooltipData(bonuses, rewardModels, tooltipData, vehicleModel):
    ...
    if len(universalBlueprints) > 1:
        ...
        bonuses.append(IntelligenceBlueprintBonus('blueprints', (
        universalBlueprints[0].getValue()[0], sum((b.getCount() for b in universalBlueprints)))))
...
def _isVehicleBlueprintBonus(bonus):
    return isinstance(bonus, VehicleBlueprintBonus) and bonus.getBlueprintName() in (
    BlueprintsBonusSubtypes.FINAL_FRAGMENT, BlueprintsBonusSubtypes.VEHICLE_FRAGMENT)
