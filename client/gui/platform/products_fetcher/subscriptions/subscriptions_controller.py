...
from constants import PRIME_GAMING_SUBSCRIPTION_PRODUCT
...
from gui.platform.products_fetcher.subscriptions.subscription_descriptors import PrimeGamingDescriptor, SubscriptionDescriptor, WotPlusDescriptor, WotPlusProDescriptor
...
WOT_PLUS_TIER_TO_DESCRIPTOR_MAP = {WotPlusTier.CORE: WotPlusDescriptor, WotPlusTier.PRO: WotPlusProDescriptor}
...
class SubscriptionProductsFetchController(ProductsFetchController):
    ...
    def _updateDescriptorsMap(self):
        self.productIDToDescriptor = {PRIME_GAMING_SUBSCRIPTION_PRODUCT: PrimeGamingDescriptor}
        for tier in WotPlusTier.ALL:
            productCodes = self._wotPlusCtrl.getSettingsStorage().getProductCodesForTier(tier)
            for productCode in productCodes:
                self.productIDToDescriptor[productCode] = WOT_PLUS_TIER_TO_DESCRIPTOR_MAP[tier]
