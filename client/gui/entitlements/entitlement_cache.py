...
class ShopEntitlement(object):
    ...
    def createFromResponseData(cls, data, onHold=False):
        return ShopEntitlement(data.get('code', ''), data.get('amount', 0), expiresAt=data.get('expires_at', None),
                               onHold=onHold)

    def __repr__(self):
        return '<{}(name={}, amount={}, expiresAt={}, onHold={})>'.format(self.__class__.__name__, self.__name,
                                                                          self.__amount, self.__expiresAt,
                                                                          self.__isOnHold)
...
class ShopEntitlementsCache(object):
    ...
    def __getDictFromResponseList(self, responseList, onHold=False):
        return {balanceEntitlement.get('code', ''): ShopEntitlement.createFromResponseData(balanceEntitlement, onHold)
                for balanceEntitlement in responseList}
