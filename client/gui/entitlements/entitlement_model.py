...
class AgateEntitlement(object):
    ...
    def createFromResponseData(cls, data, onHold=False):
        return AgateEntitlement(data.get('code', ''), data.get('amount', 0), expiresAt=data.get('expires_at', None),
                                onHold=onHold)

    def __repr__(self):
        return '<AgateEntitlement(name={}, amount={}, expiresAt={}, onHold={})>'.format(self.__name, self.__amount,
                                                                                        self.__expiresAt,
                                                                                        self.__isOnHold)
...
class AgateEntitlementsModel(object):
    ...
    def __getDictFromResponseList(self, responseList, onHold=False):
        return {balanceEntitlement.get('code', ''): AgateEntitlement.createFromResponseData(balanceEntitlement, onHold)
