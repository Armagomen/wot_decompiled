...
_EventSettings = namedtuple('_EventSettings',
                            ('vehicleCharacteristics', 'portals', 'hunterCollection', 'bossCollection'))
...
def _readEventSettings():
    ...
    result = _EventSettings(_readVehicleCharacteristics(section['vehicleCharacteristics']),
                            _readPortals(section['portals']), _readCollection(section['hunterCollection']),
                            _readCollection(section['bossCollection']))
...
def _readVehicleCharacteristics(section):
    ...
    for subsection in section['vehicles'].values():
        ...
        result[vehicle] = _VehicleCharacteristics(_readProperties(subsection['pros'], properties),
                                                  _readProperties(subsection['cons'], properties),
                                                  role=subsection['role'].asString)
