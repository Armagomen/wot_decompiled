...
@ReprInjector.simple(('_tags', 'tags'), ('_quantity', 'quantity'), ('_stage', 'stage'), ('_prevStage', 'prevStage'),
                     ('_timeRemaining', 'timeRemaining'), ('_totalTime', 'totalTime'),
                     ('_animationType', 'animationType'))
class _EquipmentItem(object):
    __slots__ = (
    '_tags', '_descriptor', '_quantity', '_stage', '_prevStage', '_timeRemaining', '_prevQuantity', '_totalTime',
    '_animationType', '_serverPrevStage')
...
class _WtAfterburningItem(_AfterburningItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        return (False, _ActivationError('equipmentAlreadyActivated', {
            'name': self._descriptor.userString})) if self._stage == EQUIPMENT_STAGES.ACTIVE else super(
            _WtAfterburningItem, self).canActivate(entityName, avatar)
        ...
        if not result:
            ...
            if vehicleID is not None:
                ...
                if vehicle is not None:
                    ...
                    if component:
                        res, keyError = component.tryActivate()
                        return (
                        res, _ActivationError(keyError, {'name': self._descriptor.userString}) if keyError else None)
            return (True, None)


class _ChargedShotItem(_ComponentEquipment):

    def update(self, quantity, stage, timeRemaining, totalTime):
        super(_ChargedShotItem, self).update(quantity, stage, timeRemaining, totalTime)
...
class _TeleportItem(_TriggerItem):
    ...
    def update(self, quantity, stage, timeRemaining, totalTime):
        ...
        if stage == EQUIPMENT_STAGES.DEPLOYING:
            ...
            self._totalTime = 0
        spawn = self._sessionProvider.dynamic.spawn
        if stage != EQUIPMENT_STAGES.PREPARING and spawn.isSpawnPointsVisible:
            spawn.closeSpawnPoints()
    ...
    def activate(self, entityName=None, avatar=None):
        ...
        if spawn is not None:
            points = [{'guid': udo.guid,
                       ...
                       'index': udo.positionNumber} for udo in BigWorld.userDataObjects.values() if
                      isinstance(udo, TeleportKeyPoint)]
            spawn.setEquipment(weakref.proxy(self))
            spawn.showSpawnPoints(points)
...
def _eventItemFactory(descriptor, quantity, stage, timeRemaining, totalTime, tag=None):
    ...
    return _ChargedShotItem(descriptor, quantity, stage, timeRemaining, totalTime,
                            tag) if descriptor.name.lower().endswith('chargedshot_wt') else _EventItem(descriptor,
                                                                                                       quantity, stage,
                                                                                                       timeRemaining,
                                                                                                       totalTime, tag)
...
class EquipmentsController(MethodsRules, IBattleController):
    ...
    def __init__(self, setup):
        ...
        self.onEquipmentStateLocked = Event.Event(self.__eManager)
        self.onEquipmentStateUnlocked = Event.Event(self.__eManager)
        self._order = []
        self._equipments = {}
    ...
    def setEquipment(self, intCD, quantity, stage, timeRemaining, totalTime):
        ...
        if not intCD:
            ...
            if descriptor.equipmentType in (EQUIPMENT_TYPES.regular, EQUIPMENT_TYPES.battleAbilities):
                if 'hidden' not in descriptor.tags:
    ...
    def lockEquipmentState(self, intCD):
        self.onEquipmentStateLocked(intCD)

    def unlockEquipmentState(self, intCD):
        self.onEquipmentStateUnlocked(intCD)
...
def isWtEventItem(item):
    return isinstance(item, (_EventItem, _WtAfterburningItem, _TeleportItem))
