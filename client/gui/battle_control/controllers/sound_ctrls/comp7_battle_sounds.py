...
from gui.battle_control.controllers.sound_ctrls.common import SoundPlayersBattleController, VehicleStateSoundPlayer, \
    SoundPlayer
...
class _EquipmentStateSoundPlayer(VehicleStateSoundPlayer):
    ...
    def __play2dFromMappingDelayed(self, soundsMapping, item):
        ...
        if delayedSoundParams is not None:
            self.__callbackDelayer.delayCallback(item.getTimeRemaining() - delayedSoundParams.timeDelta, _play2d,
                                                 delayedSoundParams.soundName)
        return
...
class _ArtillerySoundPlayer(SoundPlayer):
    ...
    def __updateAttack(self):
        ...
        if vehicle is not None:
            affectAreas = [area for area in self.__artilleryAreas if
                           vehicle.position.flatDistTo(area.position) < area.radius]
