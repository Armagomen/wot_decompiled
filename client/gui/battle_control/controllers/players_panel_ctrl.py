...
class PlayersPanelController(IPlayersPanelController):
    ...
    def __processCamp(self, params):
        ...
        if params.isAlive:
            arenaDP = self.__sessionProvider.getArenaDP()
            vInfos = [vInfo for vInfo in arenaDP.getVehiclesInfoIterator() if
                      isBossBot(vInfo=vInfo) and vInfo.gameModeSpecific.getValue(
                          EventKeys.CAMP.value) == campUdo and vInfo.vehicleStatus & VEHICLE_STATUS.IS_ALIVE]
            for component in self._viewComponents:
