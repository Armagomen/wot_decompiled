...
class EventBattleDescription(ArenaWithLabelDescription):
    ...
    def getWinString(self, isInBattle=True):
        return backport.text(R.strings.event.loading.winText.boss()) if isBossTeam(self._team) else backport.text(
            R.strings.event.loading.winText.hunters())

    def getTeamName(self, team):
        return backport.text(R.strings.event.stats.team.boss()) if isBossTeam(team) else backport.text(
            R.strings.event.stats.team.hunters())
