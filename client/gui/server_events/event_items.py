...
from gui.server_events.parsers import AccountRequirements, VehicleRequirements, TokenQuestAccountRequirements, \
    PreBattleConditions, PostBattleConditions, BonusConditions
...
class Quest(ServerEventAbstract):
    ...
    def shouldBeShown(self):
        return self.isAvailable().isValid and self.lobbyContext.getServerSettings().isMapsTrainingEnabled() if events_helpers.isMapsTraining(
...
class WtQuest(Quest):
    ...
    def isAvailable(self):
        return ValidationResult(False,
                                WT_QUEST_UNAVAILABLE_NOT_ENOUGH_TICKETS_REASON) if self.isBossQuest and not self.gameEventController.hasEnoughTickets() and not self.gameEventController.hasSpecialBoss() else super(
            WtQuest, self).isAvailable()
