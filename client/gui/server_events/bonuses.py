...
from gui.impl.gen.view_models.views.lobby.wt_event.portal_reward import TooltipType
...
class SimpleBonus(object):
    ...
    def _format(self, styleSubset):
        ...
        if self._name is not None and formattedValue is not None:
            text = makeHtmlString('html_templates:lobby/quests/{}'.format(styleSubset), self._name,
    ...
    def getWrappedEpicBonusList(self):
        ...
        for tID, tokenRecord in self.getTokens().iteritems():
            ...
            result.append({'id': tID,
                           ...
                           'icon': {AWARDS_SIZES.SMALL: backport.image(
                               R.images.gui.maps.icons.quests.bonuses.dyn(AWARDS_SIZES.SMALL).dyn(bonusImageName)()),
                                    AWARDS_SIZES.BIG: backport.image(
                                        R.images.gui.maps.icons.quests.bonuses.dyn(AWARDS_SIZES.BIG).dyn(
                                            bonusImageName)())},
...
def tokensFactory(name, value, isCompensation=False, ctx=None):
    ...
    for tID, tValue in value.iteritems():
        ...
        if tID == dependency.instance(IGameEventController).getConfig().ticketToken:
...
class VehiclesBonus(SimpleBonus):
    ...
    def formattedList(self):
        ...
        for item, vehInfo in self.getVehicles():
            ...
            if vInfoLabels:
                result.append(text_styles.standard(
                    i18n.makeString('#quests:bonuses/vehicles/name', name=text_styles.main(item.userName),
    ...
    def getRentInfo(self, vehInfo):
        for rentType, getter in ((RentType.TIME_RENT, self.getRentDays), (RentType.BATTLES_RENT, self.getRentBattles),
...
class UniversalCrewbook(SimpleBonus):
    ...
    def getItems(self):
        return [self.__ITEM(item['itemName'], item['count'], self.__getOptionsBonus(item['options'])) for item in
                self._value]
...
class HunterCollection(SimpleBonus):

    def getIcon(self):
        return R.images.gui.maps.icons.wtevent.rewards.hunter_collection()

    def getTooltip(self):
        return TooltipType.HUNTER_COLLECTION


class RandomEventStyle3d(CustomizationsBonus):

    def getIcon(self):
        return R.images.gui.maps.icons.wtevent.rewards.event_style3d()

    def getTooltip(self):
        return TooltipType.STYLE_3D
...
class RandomEventStyle2d(CustomizationsBonus):
    ...
    def getIcon(self):
        return R.images.gui.maps.icons.wtevent.rewards.event_style2d()

    def getTooltip(self):
        return makeTooltip(backport.text(self._TOOLTIP_PATH.title()), backport.text(self._TOOLTIP_PATH.description()))
...
class RandomEventDecal(CustomizationsBonus):
    ...
    def getIcon(self):
        return R.images.gui.maps.icons.wtevent.rewards.event_decal()
