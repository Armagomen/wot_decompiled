...
def packBonusGroups(bonuses, groupModels, groupsLayout, eventName, tooltipsData=None, packer=None):
    ...
    for group in groupsLayout:
        ...
        AwardsManager.sortBonuses(eventName, groupBonuses, reverse=True)
        if not groupBonuses:
            continue
        groupModel = RewardRowModel()
        bonusModelsList = groupModel.rewards
        groupModel.setLabel(backport.text(group.name))
        bonusesCount = packBonusModelAndTooltipData(groupBonuses, bonusModelsList, tooltipData=tooltipsData, merge=False, packer=packer, eventName=eventName)
        if not bonusesCount:
            continue
