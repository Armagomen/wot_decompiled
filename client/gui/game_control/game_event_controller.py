...
PERFORMANCE_GROUP_LIMITS = {WtPerfProblems.HIGH_RISK: [{WtLimitType.SYSTEM_DATA: {'clientBit': 1}},
                                                       {WtLimitType.SYSTEM_DATA: {'osBit': 1,
                                                                                  'graphicsEngine': 0}},
                                                       {WtLimitType.HARDWARE_PARAMS: {
                                                           HARDWARE_SCORE_PARAMS.PARAM_GPU_MEMORY: 490}},
                                                       {WtLimitType.SYSTEM_DATA: {'graphicsEngine': 0},
                                                        WtLimitType.HARDWARE_PARAMS: {
                                                            HARDWARE_SCORE_PARAMS.PARAM_RAM: 2900}}],
                            WtPerfProblems.MEDIUM_RISK: [
                                {WtLimitType.HARDWARE_PARAMS: {HARDWARE_SCORE_PARAMS.PARAM_GPU_SCORE: 150}},
                                {WtLimitType.HARDWARE_PARAMS: {HARDWARE_SCORE_PARAMS.PARAM_CPU_SCORE: 50000}}]}
...
class GameEventController(IGameEventController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def onLobbyInited(self, ctx):
        ...
        g_eventBus.addListener(events.HangarSimpleEvent.VEHICLE_PREVIEW_LOADED, self.__onViewLoaded,
                               scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.addListener(events.HangarSimpleEvent.VEHICLE_PREVIEW_UNLOADED, self.__onViewUnLoaded,
                               scope=EVENT_BUS_SCOPE.LOBBY)
    ...
    def __clear(self):
        ...
        if self.__serverSettings is not None:
            self.__serverSettings.onServerSettingsChange -= self.__updateEventBattlesSettings
        self.__serverSettings = None
        g_eventBus.removeListener(events.HangarSimpleEvent.HANGAR_LOADED, self.__onViewLoaded,
                                  scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.HangarSimpleEvent.HANGAR_UNLOADED, self.__onViewUnLoaded,
                                  scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.HangarSimpleEvent.VEHICLE_PREVIEW_LOADED, self.__onViewLoaded,
                                  scope=EVENT_BUS_SCOPE.LOBBY)
        g_eventBus.removeListener(events.HangarSimpleEvent.VEHICLE_PREVIEW_UNLOADED, self.__onViewUnLoaded,
                                  scope=EVENT_BUS_SCOPE.LOBBY)
        self.__hangarsSpace.onSpaceChangedByAction -= self.__onSpaceChanged
        self.__hangarsSpace.onSpaceChanged -= self.__onSpaceChanged
        self.__hangarsSpace.onSpaceCreate -= self.__onSpaceCreate
