...
ProgressionData = namedtuple('ProgressionData', ('surveys', 'rewards', 'minRank', 'totalBattles'))
    ...
    def getEventEndTimestamp(self):
        if self.hasPrimeTimesLeftForCurrentCycle() or self.isInPrimeTime():
            ...
            lastPrimeTimeEnd = max([period[1] for primeTime in self.getPrimeTimes().values() for period in
                                    primeTime.getPeriodsBetween(int(currServerTime), actualCycle.endDate, True)])
