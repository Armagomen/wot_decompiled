...
def _sortDevices(vehicle, loadout):
    ...
    del loadout.devices[:]
    loadout.devices.extend(sortedDevices)
...
class _OptDevicesAssistLocalDataProvider(_IOptDeviceAssistDataProvider):
    ...
    def start(self):
        if not self.__initialized:
            ...
            optionalDevicesConfig = {}
            if self._wotPlusController.getSettingsStorage().isRenewableSubscriptionEnabled():
                optionalDevicesConfig = self._lobbyContext.getServerSettings().getOptionalDevicesUsageConfig()
            self._syncServerSettings(convertServerDiffToRichTypes(optionalDevicesConfig))
...
class OptionalDevicesAssistantCtrl(object):
    ...
    def getPopularOptDevicesPresets(self, vehicle):
        ...
        if self._remoteDataProvider.initiated:
            ...
            self._sortOptDevicesPresets(vehicle, commonPreset, legendPreset)
            return (
             commonPreset, legendPreset)
        common, legendary = self._localDataProvider.getPresets(vehicle)
        self._sortOptDevicesPresets(vehicle, common, legendary)
        return (common, legendary)

    def _sortOptDevicesPresets(self, vehicle, common, legendary):
    ...
    def __isDataRetrievingAllowed(self):
        if not self._wotPlusCtrl.getSettingsStorage().isOptionalDevicesAssistantAvailable():
            return False
