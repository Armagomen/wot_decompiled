...
from account_helpers.AccountSettings import LOOT_BOXES_INTRO_WAS_SHOWN
...
class CNLootBoxesController(ICNLootBoxesController):
    ...
    def isIntroWasShown(self):
        return AccountSettings.getSettings(LOOT_BOXES_INTRO_WAS_SHOWN)

    def useExternalShop(self):
        return IS_CHINA

    def setIntroWasShown(self, value):
        AccountSettings.setSettings(LOOT_BOXES_INTRO_WAS_SHOWN, value)
    ...
    def getStoreInfo(self):
        return {lb.getType(): lb for lb in self.__itemsCache.items.tokens.getLootBoxes().values() if
                lb.getCategory() == CHINA_LOOT_BOXES_CATEGORIES}

    def getBoxesIDs(self):
        return set((lb.getID() for lb in self.__itemsCache.items.tokens.getLootBoxes().values() if
                    lb.getCategory() == CHINA_LOOT_BOXES_CATEGORIES))
    ...
    def __getBoxesCount(self):
        return sum((lb.getInventoryCount() for lb in self.__itemsCache.items.tokens.getLootBoxes().values() if
                    lb.getCategory() == CHINA_LOOT_BOXES_CATEGORIES))
    ...
    def __updateBoxInfo(self):
        boxes = [lb for lb in self.__itemsCache.items.tokens.getLootBoxes().values() if
                 lb.getCategory() == CHINA_LOOT_BOXES_CATEGORIES]
        boxInfoData = {}
        result = None
        if boxes:
