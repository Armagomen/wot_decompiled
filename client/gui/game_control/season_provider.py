...
class SeasonProvider(ISeasonProvider):
    ...
    def hasPrimeTimesLeftForCurrentCycle(self):
        ...
        return findFirst(
            lambda primeTime: primeTime.getNextPeriodStart(currentTime, currentCycleEndTime, includeBeginning=True),
    ...
    def hasPrimeTimesPassedForCurrentCycle(self):
        ...
        if season is not None:
            if season.hasActiveCycle(time_utils.getCurrentLocalServerTimestamp()):
                ...
                currentTime = time_utils.getCurrentLocalServerTimestamp()
                return findFirst(
                    lambda primeTime: bool(primeTime.getPeriodsBetween(startDate, currentTime, includeEnd=False)),
                    primeTimes.values(), default=False)
        return False
