...
def _createCalendarWebApiCollection():
    return webApiCollection(_CloseWindowWebApi, BirthdayCalendarWebApi, OpenTabWebApi, OpenWindowWebApi, ShopWebApi,
                            SoundWebApi, UtilWebApi)
...
class BirthdayCalendarController(GameWindowController, IBirthdayCalendarController):
    ...
    def hideWindow(self):
        if self.__browserID is None:
            ...
            if app is not None and app.containerManager is not None:
                browserWindow = app.containerManager.getView(WindowLayer.WINDOW, criteria={
                    POP_UP_CRITERIA.UNIQUE_NAME: VIEW_ALIAS.BIRTHDAY_CALENDAR})
    ...
    def _showIntro(self):
        self.__introWindow = SFWindow(SFViewLoadParams(VIEW_ALIAS.BROWSER_OVERLAY_INTRO), EVENT_BUS_SCOPE.LOBBY,
                                      ctx={'url': self.__introUrl,
    ...
    def __getBrowserView(self):
        app = self.appLoader.getApp()
        if app is not None and app.containerManager is not None:
            browserView = app.containerManager.getView(WindowLayer.WINDOW, criteria={
                POP_UP_CRITERIA.VIEW_ALIAS: VIEW_ALIAS.BIRTHDAY_CALENDAR})
            return browserView
        else:
            return
    ...
    def __loadBrowser(self, url, browserSize):
        ...
        def showBrowser():
            ctx = {'size': browserSize,
                   ...
                   'showWaiting': True,
                   'showActionBtn': False}
            g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.BIRTHDAY_CALENDAR), ctx=ctx),
                                   scope=EVENT_BUS_SCOPE.LOBBY)

        self.__browserID = yield self.browserCtrl.load(url=url, browserSize=browserSize, isAsync=False,
                                                       useBrowserWindow=False, showBrowserCallback=showBrowser,
                                                       showCreateWaiting=True, title=title)
        browser = self.browserCtrl.getBrowser(self.__browserID)
        if browser:
