...
class EmptyLoadoutField(fields.UniCapList):
    ...
    def _convert(self, incoming, onlyPublic, method):
        if not incoming:
            return {}
        return super(EmptyLoadoutField, self)._convert(incoming, onlyPublic, method)
...
class BaseOptDeviceLoadoutModel(models.Model):
    __slots__ = ('linkedVehicleID', 'equipmentArchetype1', 'equipmentArchetype2', 'equipmentArchetype3',
                 'usagePercentage')
...
class VehicleOptDeviceLoadoutsModel(models.Model):
    ...
    def _parse(self, loadoutModel):
        ...
        for loadout in loadoutModel:
            vehicleLoadout = VehicleLoadout([loadout.equipmentArchetype1,
             loadout.equipmentArchetype2,
             loadout.equipmentArchetype3], loadout.usagePercentage)
