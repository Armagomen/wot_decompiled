...
class LootBoxesController(ILootBoxesController):
    ...
    def getLastViewedCount(self):
        if self.__settingsCore.isReady:
            self.__hunterLastViewedCount = self.__settingsCore.serverSettings.getSectionSettings(
                SETTINGS_SECTIONS.LOOT_BOX_VIEWED, WTLootBoxesViewedKeys.HUNTER_LAST_VIEWED, 0)
            self.__bossLastViewedCount = self.__settingsCore.serverSettings.getSectionSettings(
                SETTINGS_SECTIONS.LOOT_BOX_VIEWED, WTLootBoxesViewedKeys.BOSS_LAST_VIEWED, 0)
    ...
    def getLootBoxesByType(self, lootBoxType=None):
        ...
        return [lootBox for lootBox in self.__itemsCache.items.tokens.getLootBoxes().itervalues() if
                lootBox.getType() in lootBoxTypes]
    ...
    def getCollectionType(self, itemID):
        if self.isCollectionElement(itemID, self.__gameCtrl.getHunterCollection()):
            ...
            return EventCollections.BOSS if self.isCollectionElement(itemID,
                                                                     self.__gameCtrl.getBossCollection()) else None
    ...
    def __packLootBoxes(self):
        ...
        if not config:
            ...
            for box in config.itervalues():
                ...
                if boxType not in (
                EventLootBoxes.WT_HUNTER, EventLootBoxes.WT_BOSS) or boxType in self.__lootBoxesConfig:
                    continue
    ...
    def __saveHunterLastViewedCount(self):
        if self.__settingsCore.isReady:
            self.__settingsCore.serverSettings.setSectionSettings(SETTINGS_SECTIONS.LOOT_BOX_VIEWED, {
                WTLootBoxesViewedKeys.HUNTER_LAST_VIEWED: self.__hunterLastViewedCount})

    def __saveBossLastViewedCount(self):
        if self.__settingsCore.isReady:
            self.__settingsCore.serverSettings.setSectionSettings(SETTINGS_SECTIONS.LOOT_BOX_VIEWED, {
                WTLootBoxesViewedKeys.BOSS_LAST_VIEWED: self.__bossLastViewedCount})
