...
def getCrystalsRecords(reusable):
    ...
    return None if not crystalsRecords or not reusable.economics.haveCrystalsChanged() else _FinancialRecords(
        main=crystalsRecords, additional=additionalRecords, alternative=None)
...
def getXpRecords(reusable):
    ...
    return _FinancialRecords(main=_XpRecords(xp=xpRecord, freeXP=freeXpRecord), additional=None,
                             alternative=_XpRecords(xp=alternativeXpRecord, freeXP=alternativeFreeXpRecord))
...
def getTotalCreditsToShow(reusable, records, hasPremium):
    return reusable.economics.getCreditsToShow()[0] if hasPremium else records.getRecord('credits',
...
def _createPremiumBonusModel(reusable):
    return None if reusable.economics.isPremiumPlus or reusable.economics.isActivePremiumPlus() else getCurrencies(
        (reusable.economics.getCreditsDiff(), reusable.economics.getXPDiff()),
        (CurrencyModel.CREDITS, CurrencyModel.XP))
