...
from gui.battle_results.reusable.shared import makeAchievement, makeMarkOfMasteryFromPersonal, AchievementSimpleData, \
    VehicleSummarizeInfo
...
_AchievementData = namedtuple('_AchievementData',
                              ('achievementID', 'name', 'isEpic', 'iconName', 'groupID', 'isPersonal'))
...
def setPersonalAchievements(model, reusable, result):
    ...
    achievements = chain([_processAchievement(item, AchievementModel.ACHIEVEMENT_LEFT_BLOCK) for item in left],
                         [_processAchievement(item, AchievementModel.ACHIEVEMENT_RIGHT_BLOCK) for item in right])
    _setAchievements(achievements, model)


def setTeamStatsAchievements(model, info):
    rawAchievements = info.getAchievements()
    processedAchievements = [_processAchievement(item, AchievementModel.ACHIEVEMENT_RIGHT_BLOCK) for item in
                             rawAchievements]
...
def _processAchievement(item, groupID):
    ...
    return _AchievementData(achievementID=achievementID, name=achievement.getName(), isEpic=achievement.hasRibbon(),
                            iconName=achievement.getIconName(), groupID=groupID, isPersonal=isPersonal)
