...
class CrystalsField(CurrencyField):
    ...
    def __init__(self, stringID, blockIdx, recordItems, tags=None, isShown=False, tooltipString=R.invalid()):
        super(CrystalsField, self).__init__(stringID, blockIdx, recordItems, CurrencyModel.CRYSTALS, tags, isShown,
                                            tooltipString)
...
class CrystalsTotalField(CurrencyField):
    ...
    def __init__(self, stringID, blockIdx, recordItems, recordSubtractItems, tags=None, isShown=False,
        ...
        super(CrystalsTotalField, self).__init__(stringID, blockIdx, recordItems, CurrencyModel.CRYSTALS, tags, isShown,
                                                 tooltipString)
...
def getCrystalFields():
    return DetailedStatsCrystalFields(earned=_getEarnedCrystal(), earnings=_getCrystalEarnings(),
                                      expenses=_getCrystalExpenses(), total=_getTotalCrystal())


def _getEarnedCrystal():
    return CrystalsField(blockIdx=_CrystalBlockIndexes.EARNED, stringID=_CRYSTAL_STRINGS.earned(),
                         tooltipString=_CRYSTAL_STRINGS.earned.descr(), tags=(DetailsRecordModel.EARNED,),
                         recordItems=('originalCrystal',), isShown=True)


def _getDetailedCrystalExpenses():
    return DetailedStatsCrystalExpenses(autoBooster=CrystalsField(blockIdx=_CrystalExpensesBlockIndexes.EXPENSES,
                                                                  stringID=_CRYSTAL_STRINGS.expenses.autoBooster(),
                                                                  tooltipString=_CRYSTAL_STRINGS.expenses.autoBooster.descr(),
                                                                  recordItems=('autoBoostersCrystal',), isShown=True))


def _getTotalCrystalExpenses():
    return CrystalsField(blockIdx=_CrystalExpensesBlockIndexes.TOTAL_EXPENSES,
                         stringID=_CRYSTAL_STRINGS.expenses.total(),
                         tooltipString=_CRYSTAL_STRINGS.expenses.total.descr(),
                         tags=(DetailsRecordModel.SUBGROUP_TOTAL,), recordItems=('autoBoostersCrystal',), isShown=True)
...
def _getDetailedCrystalEarnings():
    return DetailedStatsCrystalEarnings(achievement=EventField(blockIdx=_CrystalEarningsBlockIndexes.ACHIEVEMENTS,
                                                               stringID=_CRYSTAL_STRINGS.achievement(),
                                                               tooltipString=_CRYSTAL_STRINGS.achievement.descr(),
                                                               recordItems=('eventCrystalList',),
                                                               valueType=CurrencyModel.CRYSTALS, isShown=True))


def _getTotalCrystalEarnings():
    return CrystalsField(blockIdx=_CrystalEarningsBlockIndexes.TOTAL_EARNED, stringID=_CRYSTAL_STRINGS.totalEarnings(),
                         tooltipString=_CRYSTAL_STRINGS.totalEarnings.descr(),
                         tags=(DetailsRecordModel.SUBGROUP_TOTAL,), recordItems=('crystal',), isShown=True)


def _getTotalCrystal():
    return CrystalsTotalField(blockIdx=_CrystalBlockIndexes.TOTAL, stringID=_CRYSTAL_STRINGS.total(),
                              tooltipString=_CRYSTAL_STRINGS.total.descr(), tags=(DetailsRecordModel.TOTAL,),
                              recordItems=('crystal',), recordSubtractItems=('autoBoostersCrystal',), isShown=True)
