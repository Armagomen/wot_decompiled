...
import typing
from constants import FAIRPLAY_VIOLATIONS_NAMES
...
if typing.TYPE_CHECKING:
    ...
    from gui.battle_results.presenter.getter.detailed_stats_credits import CreditEarningsField, CreditsTotalField, \
        CreditsEarnedField
...
def createFieldsGetter():
    return PostbattleFieldsGetter(creditFields=getCreditFields(), crystalFields=getCrystalFields(),
                                  xpFields=getXPFields(), teamStatsFields=getTeamStats(),
                                  premiumBenefitFields=getPremiumBenefits())
...
class _CreditsGetter(object):
    ...
    def getCreditEarnings(self, reusable):
        creditEarningsFields = self.__detailedStatsCredits.earnings.detailed.bonuses + _processFinesFields(reusable,
                                                                                                           *self.__detailedStatsCredits.earnings.detailed.fines) + (
                               self.__detailedStatsCredits.earnings.total,)
    ...
    def getCreditExpenses(self):
        creditExpensesFields = self.__detailedStatsCredits.expenses.detailed + (
        self.__detailedStatsCredits.expenses.total,)
...
class _CrystalsGetter(object):
    ...
    def getCrystalsEarnings(self):
        crystalEarningsFields = self.__detailedStatsCrystals.earnings.detailed + (
        self.__detailedStatsCrystals.earnings.total,)
    ...
    def getCrystalsExpenses(self):
        crystalExpensesFields = self.__detailedStatsCrystals.expenses.detailed + (
        self.__detailedStatsCrystals.expenses.total,)
...
class _XPGetter(object):
    ...
    def getXP(self, reusable, isRecord):
        ...
        xpFields = (earned,) + self.__detailedStatsXP.earnings.bonuses + _processFinesFields(reusable,
                                                                                             *self.__detailedStatsXP.earnings.fines) + (
                   self.__detailedStatsXP.total,)
...
class PostbattleFieldsGetter(object):
    __slots__ = (
    '__detailedStatsCredits', '__detailedStatsCrystals', '__detailedStatsXP', '__teamStats', '__premiumBenefits')
    ...
    def getTeamStats(self, isSPG, isPersonal, isRoleExp):
        ...
        def filterFunc(field):
            return (field not in spgFields or isSPG) and (field not in personalFields or isPersonal) and (
                        field not in nonPersonalFields or not isPersonal) and (field not in roleExpFields or isRoleExp)
