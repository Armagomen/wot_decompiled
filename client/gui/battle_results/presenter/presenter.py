...
from gui.battle_results.presenter.event import setWidgets
...
from gui.battle_results.presenter.tooltips.efficiency_tooltips import setPersonalEfficiencyTooltipData, \
    getTeamEfficiencyTooltipData, TeamEfficiencyTooltipData, getTotalEfficiencyTooltipData
...
from gui.battle_results.presenter.detail_stats import setDetailStatsData
...
class DataPresenter(object):
    ...
    def setDataToModel(self, model, tooltipData, arenaUniqueID):
        ...
        setDetailStatsData(model.details, self.__getter, *battleResult)
        setUserStatus(model.userStatus, *battleResult)
        setEventsData(model.events, tooltipData, *battleResult)
        setAccountType(model, *battleResult)
        setArenaType(model, *battleResult)
        setWidgets(model.widgets, *battleResult)
        return
    ...
    def __saveSettings(self, arenaUniqueID):
        if arenaUniqueID not in self.__cachedSettings:
            ...
            self.__cachedSettings[arenaUniqueID] = {
                'additionalBonusMultiplier': additionalBonusConfig.get('bonusFactor', 0)}
