...
from gui.battle_results.br_constants import PremiumXpBonusRestrictions, EfficiencyKeys, EfficiencyItems, \
    STAT_STUN_FIELD_NAMES
...
_EnemyInfo = namedtuple('EnemyInfo',
                        ('name', 'clanTag', 'tankName', 'shortVehicleName', 'vehicleType', 'value', 'isTeamKiller'))
...
def getBattleType(reusable):
    ...
    return R.strings.arenas.type.dyn(arenaType.getGamePlayName()).name() if arenaGuiType in (
    ARENA_GUI_TYPE.RANDOM, ARENA_GUI_TYPE.EPIC_RANDOM) else R.strings.menu.loading.battleTypes.num(arenaGuiType)()
...
def getArmorUsingDetails(rickochets, noDamage, damageBlocked, labels):
    values = [backport.getIntegralFormat(rickochets), backport.getIntegralFormat(noDamage),
              backport.getIntegralFormat(damageBlocked)]
    descriptions = [backport.text(labels[0]), backport.text(labels[1]),
                    backport.text(labels[2], vals=style.getTooltipParamsStyle())]
    return (values, descriptions)


def getAssistBlockDetails(damageAssistedRadio, damageAssistedTrack, damageAssisted, labels):
    damageAssistedValues = [backport.getIntegralFormat(damageAssistedRadio),
                            backport.getIntegralFormat(damageAssistedTrack), backport.getIntegralFormat(damageAssisted)]
...
def getStunBlockDetails(assisted, count, duration, labels):
    stunValues = [backport.getIntegralFormat(assisted), backport.getIntegralFormat(count),
                  backport.getFractionalFormat(duration)]
    stunNames = [backport.text(labels[0], vals=style.getTooltipParamsStyle()), backport.text(labels[1]),
                 backport.text(labels[2],
                               vals=style.getTooltipParamsStyle(BATTLE_RESULTS.COMMON_TOOLTIP_PARAMS_VAL_SECONDS))]
