...
class VehicleProgressHelper(object):
    ...
    def getAvgBattles2Unlock(self, unlockProps):
        return int(math.ceil(
            (unlockProps.xpCost - self.__vehicleXp) / float(self.__avgVehicleXp))) if self.__avgVehicleXp > 0 else 0
...
class ProgressInfo(shared.UnpackedInfo):
    ...
    def processProgressiveRewardData(self):
        if self.__progressiveReward is None:
            ...
            return ProgressiveRewardData(currentStep=currentStep, probability=probability, maxSteps=maxSteps,
    ...
    def packPersonalMissions(self, personalMissions):
        result = []
        for quest, data in sorted(personalMissions.items(), key=operator.itemgetter(0),
    ...
    def __sortCommonQuestsFunc(aData, bData):
        ...
        if aQuest.isCompleted() and bQuest.isCompleted(bCurProg):
            res = aQuest.getBonusCount(aCurProg) - aPrevProg.get('bonusCount', 0) - (
                        bQuest.getBonusCount(bCurProg) - bPrevProg.get('bonusCount', 0))
            if not res:
                return res
