...
class PersonalInfo(shared.UnpackedInfo):
    __slots__ = (
    '__avatar', '__vehicles', '__lifeTimeInfo', '__isObserver', '_economicsRecords', '__rankInfo', '__isTeamKiller',
    '__premiumMask', '__isAddXPBonusApplied', '__c11nProgress', '__dogTags')
    itemsCache = dependency.descriptor(IItemsCache)
    ...
    def getAchievements(self, result):
        ...
        for intCD in self.__vehicles:
            ...
            for direction, achievement, achievementID in achievements:
                if direction == 1:
                    right.append(shared.AchievementSimpleData(achievementID=achievementID, achievement=achievement,
                                                              isUnique=True, isPersonal=True))
            ...
            if achievement is not None:
                left.append(shared.AchievementSimpleData(achievementID=UNKNOWN_ACHIEVEMENT_ID, achievement=achievement,
                                                         isUnique=False, isPersonal=True))
