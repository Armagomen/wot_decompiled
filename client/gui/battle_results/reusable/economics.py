...
class _CreditsReplayRecords(records.ReplayRecords):
    ...
    def __init__(self, replay, results, squadCreditsFactor=0):
        ...
        self._addRecord(ValueReplay.SUBCOEFF, 'originalCreditsToDrawSquad', results['originalCreditsToDrawSquad'],
                        results['originalCreditsToDrawSquad'] * self.getFactor('premSquadCreditsFactor100') * -1)
...
class _EconomicsRecordsChains(object):
    __slots__ = (
    '_baseCredits', '_premiumCredits', '_premiumPlusCredits', '_goldRecords', '_additionalRecords', '_baseXP',
    '_premiumXP', '_premiumPlusXP', '_baseXPAdd', '_premiumXPAdd', '_premiumPlusXPAdd', '_baseFreeXP', '_premiumFreeXP',
    '_premiumPlusFreeXP', '_baseFreeXPAdd', '_premiumFreeXPAdd', '_premiumPlusFreeXPAdd', '_crystal', '_crystalDetails')
    ...
    def getCreditsDiff(self):
        return self._premiumCredits.getRecord('credits', 'originalCreditsToDraw') - self._baseCredits.getRecord(
    ...
    def _addMoneyResults(self, connector, results):
        if 'creditsReplay' in results and results['creditsReplay'] is not None:
            ...
            self._premiumPlusCredits.addRecords(
                self.__buildCreditsReplayForPremType(PREMIUM_TYPE.PLUS, results, replay))
        else:
    ...
    def _addXPResults(self, connector, results):
        if 'xpReplay' in results and results['xpReplay'] is not None:
            ...
            isHighScope = RECORD_DB_IDS[('max15x15', 'maxXP')] in [recordID for recordID, _ in
                                                                   results.get('dossierPopUps', [])]
...
class EconomicsInfo(shared.UnpackedInfo):
    ...
    def canUpgradeToPremium(self, arenaBonusType):
        return self.__premiumState & PremiumState.BUY_ENABLED > 0 and self.__premiumState & PremiumState.HAS_ALREADY == 0 and not self.isPostBattlePremium and arenaBonusType in (
        ARENA_BONUS_TYPE.REGULAR, ARENA_BONUS_TYPE.EPIC_RANDOM, ARENA_BONUS_TYPE.EPIC_BATTLE)

    def canUpgradeToPremiumPlus(self, arenaBonusType):
        return self.__premiumPlusState & PremiumState.BUY_ENABLED > 0 and self.__premiumPlusState & PremiumState.HAS_ALREADY == 0 and not self.isPostBattlePremiumPlus and arenaBonusType in (
        ARENA_BONUS_TYPE.REGULAR, ARENA_BONUS_TYPE.EPIC_RANDOM, ARENA_BONUS_TYPE.EPIC_BATTLE)
