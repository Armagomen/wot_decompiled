...
def _fetchRecord(results, recordName):
    if recordName in results:
        ...
        if record is None:
            LOG_WARNING(
                'Record is not valid in the results. Perhaps, client and server versions of battle_results.g_config are different.',
                recordName)
        return record
    else:
...
def createReusableInfo(results):
    ...
    if not unpackedRecords:
        return ReusableInfo(arenaUniqueID, commonInfo, personalInfo, playersInfo, vehiclesInfo, avatarsInfo,
                            economicsInfo, progressInfo)
    else:
...
class ReusableInfo(object):
    __slots__ = (
    '__arenaUniqueID', '__clientIndex', '__squadFinder', '__economics', '__common', '__personal', '__players',
    '__vehicles', '__avatars', '__progress', '__battlePassProgress')
