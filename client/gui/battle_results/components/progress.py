...
from gui.Scaleform.daapi.view.lobby.customization.progression_helpers import getProgressionPostBattleInfo, parseEventID, \
    getC11nProgressionLinkBtnParams
...
def _makeTankmanVO(tman, avgBattles2NewSkill, itemsCache=None, lobbyContext=None):
    ...
    if avgBattles2NewSkill > 0:
        prediction = backport.text(R.strings.battle_results.common.newSkillPrediction(),
...
class VehicleProgressBlock(base.StatsBlock):
    ...
    def setRecord(self, result, reusable):
        for intCD, data in reusable.personal.getVehicleCDsIterator(result):
            ...
            progress = list(chain(ready2UnlockModulesVOs, ready2BuyModulesVOs, tankmenVOs, ready2UnlockVehiclesVOs,
                                  ready2BuyVehiclesVOs))
            helper.clear()
    ...
    def __getReady2UnlockItemsVOs(helper, vehicleBattleXp):
        ...
        for item, unlockProps, _ in ready2UnlockVehicles:
            ...
            if not helper.isEnoughXPToUnlock(unlockProps):
                0 < avgBattles2Unlock <= MIN_BATTLES_TO_SHOW_PROGRESS and ready2UnlockVehiclesVOs.append(
                    _makeUnlockVehicleVO(item, unlockProps, avgBattles2Unlock))

        ready2UnlockModulesVOs = [_makeUnlockModuleVO(item, unlockProps) for item, unlockProps, _ in
                                  ready2UnlockModules]
    ...
    def __getReady2BuyItemsVOs(helper, pureCreditsReceived):
        ...
        ready2BuyVehiclesVOs = [_makeVehiclePurchaseVO(item, unlockProps, price) for item, unlockProps, price in
                                ready2BuyVehicles]
        ready2BuyModulesVOs = [_makeModulePurchaseVO(item, unlockProps, price) for item, unlockProps, price in
                               ready2BuyModules]
