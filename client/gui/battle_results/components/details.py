    ...
    def setRecord(self, result, reusable):
        ...
        for records in reusable.economics.getBaseXPRecords():
            ...
            if factor == 1 and showSquadLabels and squadHasBonus:
                value = i18n.makeString(backport.text(R.strings.battle_results.common.details.xpTitleSquad()),
                                        img=icons.makeImageTag(
                                            backport.image(R.images.gui.maps.icons.library.prebattleInviteIcon_1())))
            else:
                value = backport.text(R.strings.battle_results.common.details.xpTitle())
            if factor > 1:
                value = ' '.join((value, icons.makeImageTag(
                    backport.image(R.images.gui.maps.icons.library.dyn('bonus_x{}'.format(factor))()), 46, 18)))
            self.addNextComponent(base.DirectStatsItem('', value))
...
class MoneyDetailsBlock(_EconomicsDetailsBlock):
    ...
    def __addTotalResults(self, baseCredits, premiumCredits, goldRecords, additionalRecords):
        ...
        columns = {
            'column1': style.makeCreditsLabel(baseCredits.getRecord(*self.__intermediateTotalRecords) + autoCredits,
                                              canBeFaded=baseCanBeFaded),
            'column3': style.makeCreditsLabel(premiumCredits.getRecord(*self.__intermediateTotalRecords) + autoCredits,
...
class PremiumBonusDetailsBlock(base.StatsBlock):
    __slots__ = (
    'description', 'bonusLeft', 'xpValue', 'statusBonusLabel', 'statusBonusTooltip', 'bonusIcon', '__isPersonalTeamWin',
    '__arenaUniqueID', '__arenaBonusType', '__xpFactor', '__vehicleCD', '__isActivePremiumPlus',
    '__applyAdditionalXPCount')
    ...
    def __updateStatus(self):
        ...
        PremiumXpBonusRestrictions.IS_XP_TO_TMEN_DISABLED, PremiumXpBonusRestrictions.IS_XP_TO_TMEN_ENABLED):
