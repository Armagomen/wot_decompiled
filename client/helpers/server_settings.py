...
from constants import IS_TUTORIAL_ENABLED, PremiumConfigs, DAILY_QUESTS_CONFIG, ClansConfig, MAGNETIC_AUTO_AIM_CONFIG, \
    Configs, DOG_TAGS_CONFIG, BATTLE_NOTIFIER_CONFIG, MISC_GUI_SETTINGS, PREM_BONUS_TYPES, PREMIUM_ENTITLEMENTS, \
    ENTITLEMENT_TO_PREM_TYPE, POSTBATTLE20_CONFIG
...
class _EventBattlesConfig(namedtuple('_EventBattlesConfig', ('isEnabled',
    ...
    def __new__(cls, **kwargs):
        defaults = dict(isEnabled=False, peripheryIDs={}, primeTimes={}, seasons={}, cycleTimes={}, progression=[],
                        exchange={}, hunterCollectionToken='', bossCollectionToken='', eventCollectionToken='',
                        ticketToken='', quickBossTicketToken='', quickHunterTicketToken='', ticketsToDraw=0,
                        lootBoxDailyPurchaseLimit=0, lootBoxCounterEntitlementID='')
        defaults.update(kwargs)
...
class ServerSettings(object):
    ...
    def set(self, serverSettings):
        ...
        if 'bday_calendar_config' in self.__serverSettings:
            self.__birthdayCalendar = makeTupleByDict(_BirthdayCalendarConfig,
                                                      self.__serverSettings['bday_calendar_config'])
        if 'epic_config' in self.__serverSettings:
            LOG_DEBUG('epic_config', self.__serverSettings['epic_config'])
            self.__epicMetaGameSettings = makeTupleByDict(_EpicMetaGameConfig,
        ...
        if 'unit_assembler_config' in self.__serverSettings:
            self.__unitAssemblerConfig = makeTupleByDict(_UnitAssemblerConfig,
                                                         self.__serverSettings['unit_assembler_config'])
    ...
    def getPremiumPlusCreditsBonus(self):
        battleBonuses = self.__getGlobalSetting('prem_battle_bonuses', {})
        return battleBonuses.get(PREM_BONUS_TYPES.CREDITS, {}).get(ENTITLEMENT_TO_PREM_TYPE[PREMIUM_ENTITLEMENTS.PLUS],
                                                                   0)

    def getPremiumPlusTmenXPBonus(self):
        battleBonuses = self.__getGlobalSetting('prem_battle_bonuses', {})
        return battleBonuses.get(PREM_BONUS_TYPES.TMEN_XP, {}).get(ENTITLEMENT_TO_PREM_TYPE[PREMIUM_ENTITLEMENTS.PLUS],
                                                                   0)
    ...
    def __updateEpic(self, targetSettings):
        self.__epicMetaGameSettings = self.__epicMetaGameSettings.replace(
            targetSettings['epic_config'].get('epicMetaGame', {}))
    ...
    def __updateBlueprintsConvertSaleConfig(self, targetSettings):
        if 'blueprints_convert_sale_config' in targetSettings:
            self.__blueprintsConvertSaleConfig = self.__blueprintsConvertSaleConfig.replace(
                targetSettings['blueprints_convert_sale_config'])

    def __updateVehiclePostProgressionConfig(self, serverSettingsDiff):
        self.__vehiclePostProgressionConfig = self.__vehiclePostProgressionConfig.replace(
            serverSettingsDiff[post_progression_common.SERVER_SETTINGS_KEY])
