...
class StageModifierManager(CGF.ComponentManager):
    stageQuery = CGF.QueryConfig(ArenaComponents.BattleStage)
...
class OnAppearPrefabSpawner(CGFComponent):
    prefab = ComponentProperty(type=CGFMetaTypes.STRING, editorName='prefab', value='',
                               annotations={'path': '*.prefab'})


class OnDisappearPrefabSpawner(CGFComponent):
    prefab = ComponentProperty(type=CGFMetaTypes.STRING, editorName='prefab', value='',
                               annotations={'path': '*.prefab'})
...
class IdleEffectComponentManager(CGF.ComponentManager):
    ...
    def __createGO(self, resource, activationDelay=0.0, deactivationDelay=None, parent=None):
        ...
        go.createComponent(GenericComponents.AnimatorComponent, resource, activationDelay, 1.0,
                           -1 if deactivationDelay is None else 1, True, False, '')
