...
if IS_CLIENT:
    ...
    from EntityInfo import CampComponent
...
def getBattleTimeLeft():
    ...
    return (
                       arena.periodEndTime - BigWorld.serverTime()) / _SECONDS_IN_MINUTE if arena is not None and arena.period == ARENA_PERIOD.BATTLE else 0.0


def getDestroyedGeneratorsCount():
    query = CGF.Query(BigWorld.player().spaceID, ArenaComponents.BattleStage)
...
def getCampCount():
    campsQuery = CGF.Query(BigWorld.player().spaceID, CampComponent)
    return max((camp.campIndex for camp in campsQuery)) if not campsQuery.empty() else 0
...
def isEventBattle():
    ...
    return bonusType in (ARENA_BONUS_TYPE.EVENT_BATTLES, ARENA_BONUS_TYPE.EVENT_BATTLES_2)
