...
class BombDeployManager(CGF.ComponentManager):
    ...
    def playerUI(self, addedActivation):
        genProgress = addedActivation.generatorGO.findComponentByType(ArenaComponents.GeneratorProgressComponent)
        for veh, activation, go in self.activations:
            if addedActivation.generatorGO.id == activation.generatorGO.id:
                if go.findComponentByType(PlayerVehicleTag):
                    ...
                    carry = veh.appearance.gameObject.findComponentByType(ArenaComponents.CarryingLootComponent)
                    if carry and self.feedbackCtrl:
                        self.feedbackCtrl.onVehicleFeedbackReceived(_EVENT_ID.WT_BOMB_DEPLOY, veh.id,
                                                                    genProgress.timeLeft)
...
class BombCarryManager(CGF.ComponentManager):
    ...
    @onAddedQuery(ArenaComponents.CarryingLootComponent, PlayerVehicleTag)
    ...
    @onRemovedQuery(ArenaComponents.CarryingLootComponent, PlayerVehicleTag)
