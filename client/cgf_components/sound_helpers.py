...
def _getSoundComponent(vehicle):
    ...
    return vehicle.appearance.findComponentByType(
        VehicleSoundComponent) if vehicle is not None and vehicle.appearance is not None else None
...
def hasVehicleSound(event, vehicle):
    soundComponent = _getSoundComponent(vehicle)
    return bool(findFirst(lambda soundObj: soundObj.name == event,
                          soundComponent.soundObjects)) if soundComponent is not None else None
