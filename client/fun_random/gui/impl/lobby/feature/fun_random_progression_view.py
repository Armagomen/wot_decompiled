...
class FunRandomProgressionView(ViewImpl, FunAssetPacksMixin, FunProgressionWatcher, FunSubModesWatcher):
    ...
    def __init__(self, *_, **__):
        settings = ViewSettings(layoutID=R.views.fun_random.mono.lobby.progression(), model=FunRandomProgressionViewModel())
        self.__tooltips = {}
...
class FunRandomProgressionWindow(WindowImpl):

    def __init__(self, layer, **kwargs):
        super(FunRandomProgressionWindow, self).__init__(content=FunRandomProgressionView(**kwargs), wndFlags=WindowFlags.WINDOW, layer=layer)