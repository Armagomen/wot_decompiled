...
from skeletons.gui.game_control import IAnonymizerController, IVehiclePostProgressionController
from skeletons.gui.lobby_context import ILobbyContext
_NEW_SETTING_COUNTER_VISIBILITY_VALIDATORS = {
    settings_constants.GAME.ANONYMIZER: lambda: dependency.instance(IAnonymizerController).isEnabled,
    'showQuestProgress': lambda: dependency.instance(ILobbyContext).getServerSettings().isPersonalMissionsEnabled(),
    settings_constants.GAME.GAMEPLAY_ONLY_10_MODE: lambda: dependency.instance(
        ILobbyContext).getServerSettings().isOnly10ModeEnabled(),
    settings_constants.GAME.SWITCH_SETUPS_IN_LOADING: lambda: dependency.instance(
        IVehiclePostProgressionController).isSwitchSetupFeatureEnabled()}
