...
class C11nMessageDecorator(LockButtonMessageDecorator):
    ...
    def __init__(self, entityID, entity=None, settings=None, model=None):
        ...
        g_eventBus.addListener(HangarSimpleEvent.DISPATCHER_ENTITY_WAS_UPDATED, self._updateButtons,
                               EVENT_BUS_SCOPE.LOBBY)

    def clear(self):
        super(C11nMessageDecorator, self).clear()
        g_clientUpdateManager.removeObjectCallbacks(self)
        g_eventBus.removeListener(HangarSimpleEvent.DISPATCHER_ENTITY_WAS_UPDATED, self._updateButtons,
                                  EVENT_BUS_SCOPE.LOBBY)
    ...
    def _updateButtons(self, _):
        ...
        if not isBRHangar and not isEventHangar and vehCustomizationIsEnabled:
            isLocked = self._entity.get('savedData', {}).get('toStyle', False) and not isVehicleCanBeCustomized(
