...
import logging
import BigWorld
from cgf_components import IsHighlighted, IsSelected
...
class EventPortal(ClientSelectableCameraObject):
    ...
    def setHighlight(self, show, fallback=False):
        if fallback:
            return super(EventPortal, self).setHighlight(show)
        elif self.entityGameObject.findComponentByType(IsSelected) is not None:
            return
        else:
            if show and self.entityGameObject.findComponentByType(IsHighlighted) is None:
                self.entityGameObject.createComponent(IsHighlighted)
            else:
    ...
    def onMouseClick(self):
        if self.isMouseSelectionLocked:
            ...
            g_eventBus.handleEvent(events.LobbySimpleEvent(events.HangarSimpleEvent.EVENT_PORTAL_SELECTED),
