...
class HWConsumablesPanel(ConsumablesPanelMeta):
    ...
    _HW_EQUIPMENT_START_IDX = None
    _HW_EQUIPMENT_END_IDX = None
    ...
    def _initIndexes(cls):
        cls._HW_EQUIPMENT_START_IDX, cls._HW_EQUIPMENT_END_IDX, cls._AMMO_START_IDX, cls._AMMO_END_IDX = _PRESETS_CUSTOM_INDEXES.get(getSettings(AccountSettingsKeys.AMMO_PANEL_PRESET), 'default')
    ...
    def _getEquipmentIcon(self, idx, item, icon):
        ...
        return backport.image(hwIconRes()) if hwIconRes.isValid() else super(HWConsumablesPanel, self)._getEquipmentIcon(idx, item, icon)
    ...
    def _buildEquipmentSlotTooltipText(self, item):
        ...
        if self._hasDuration(item):
            if int(variant.durationSeconds) > 0:
                body = '\n'.join((body, backport.text(R.strings.halloween_battle.consumablesPanel.tooltip.durationTime(), time=str(int(variant.durationSeconds)))))
