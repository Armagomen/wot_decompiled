...
# Embedded file name: halloween/scripts/client/halloween/gui/shared/gui_items/processors/processors.py
import BigWorld
...
class BuyBundleProcessor(Processor):
    ...
    def __pushSuccessMessage(self):
        ...
        messageData = {'title': backport.text(R.strings.halloween_system_messages.serviceChannelMessages.bundlePurchase.title()),
         ...
         'additionalText': backport.text(R.strings.halloween_system_messages.serviceChannelMessages.bundlePurchase.additionalText(), money=formatPrice(Money(**{self._price.currency: self._price.amount * self._count}), useStyle=True))}
...
class ExchangeCommandersProcessor(Processor):
    ...
    def __init__(self, controller, commandersData):
        super(ExchangeCommandersProcessor, self).__init__(plugins=(CheckTwitchCon(controller, commandersData),))
