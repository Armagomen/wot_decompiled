...
class BalanceWebApiMixin(object):
    ...
    def getBalance(self, cmd):
        ...
        balanceData = {Currency.currencyExternalName(currency): stats.actualMoney.get(currency, 0) for currency in
                       Currency.ALL}
    ...
    def getCollectionNations(self, cmd):
        return {nations.MAP[nationID]: self.itemsCache.items.stats.getMaxResearchedLevel(nationID) for nationID in
                nations.MAP if hasCollectibleVehicles(nationID)}
    ...
    def getInventoryEntitlements(self, cmd):
        ...
        yield {
            'success': result and self.__entitlementsController.isCacheInited and not self.__entitlementsController.isCodesWasFailedInLastRequest(
                cmd.codes),
