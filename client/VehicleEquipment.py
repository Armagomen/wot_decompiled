...
class VehicleEquipment(DynamicScriptComponent):
    ...
    def set_stage(self, prev=None):
        if self._context is None:
            ...
            LOG_DEBUG_DEV('stage', self.entity.id, self.compactDescr, self.quantity, self.stage, self.prevStage,
                          timeRemainig, self.totalTime)
            if BigWorld.player().guiSessionProvider.shared.equipments.hasEquipment(self.compactDescr):
                BigWorld.player().updateVehicleAmmo(self.entity.id, self.compactDescr, self.quantity, self.stage,
                                                    self.prevStage, timeRemainig, self.totalTime)
            return
