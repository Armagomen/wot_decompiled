...
class Comp7ShopController(IComp7ShopController):
    ...
    def hasNewDiscounts(self, newRank):
        config = self.__comp7Controller.getRanksConfig()
        prevRankDiscounts = {}
    ...
    def __sendNewProductsForRankMessage(self):
        pushMessage(type=SM_TYPE.Comp7ShopItemsAvailableForRank, text=backport.text(R.strings.comp7_ext.system_messages.shop.offer.newItems.body()), messageData={'header': backport.text(R.strings.comp7_ext.system_messages.shop.offer.newItems.title())})

    def __sendNewDiscountsForRankMessage(self):
        pushMessage(type=SM_TYPE.Comp7ShopItemsAvailableForRank, text=backport.text(R.strings.comp7_ext.system_messages.shop.offer.newDiscounts.body()), messageData={'header': backport.text(R.strings.comp7_ext.system_messages.shop.offer.newDiscounts.title())})