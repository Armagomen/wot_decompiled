...
class EventVehicle(ClientSelectableCameraVehicle):
    ...
    def onMouseClick(self):
        g_eventBus.handleEvent(events.LobbySimpleEvent(events.HangarSimpleEvent.EVENT_VEHICLE_SELECTED),
    ...
    def _isPortalSelected(self):
        ...
        return portalEntity.entityGameObject.findComponentByType(
            IsSelected) is not None if portalEntity is not None else False
    ...
    def _chooseVehicle(self):
        ...
        if self.eventType == VEHICLE_TAGS.EVENT_BOSS:
            vehicles = self._itemsCache.items.getVehicles(
                REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS([VEHICLE_TAGS.EVENT_SPECIAL_BOSS]))
            if vehicles:
        ...
        if self.eventType == VEHICLE_TAGS.EVENT_HUNTER:
            pass
        vehicles = self._itemsCache.items.getVehicles(
            REQ_CRITERIA.VEHICLE.HAS_TAGS([self.eventType]) | REQ_CRITERIA.INVENTORY)
        if vehicles:
            if self.eventType == VEHICLE_TAGS.EVENT_BOSS:
                vehicle = findFirst(lambda veh: VEHICLE_TAGS.EVENT_SPECIAL_BOSS in veh.tags, vehicles.values(),
                                    vehicles.values()[0])
    ...
    def _createAppearance(self):
        return _EventBossAppearance(self.spaceID,
                                    self) if self.eventType == VEHICLE_TAGS.EVENT_BOSS else _EventVehicleAppearance(
            self.spaceID, self)
