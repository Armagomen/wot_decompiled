...
from last_stand.gui.ls_account_settings import clearNewStatusUnlockLevel
from last_stand.gui.ls_gui_constants import DIFFICULTY_TOKEN_TO_LEVEL
...
class LSAccountComponent(BaseAccountExtensionComponent):
    ...
    def lockToken(self, token, callback=None):
        if callback is not None:
            ...
            proxy = None
        diffLvl = DIFFICULTY_TOKEN_TO_LEVEL.get(token)
        if diffLvl is not None:
            clearNewStatusUnlockLevel(diffLvl)
