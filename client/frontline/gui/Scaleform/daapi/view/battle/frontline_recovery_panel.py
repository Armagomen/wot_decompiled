...
class FrontlineRecoveryPanel(RecoveryPanelMeta):
    sessionProvider = dependency.descriptor(IBattleSessionProvider)

    def __init__(self):
        super(FrontlineRecoveryPanel, self).__init__()
    ...
    def _populate(self):
        super(FrontlineRecoveryPanel, self)._populate()
        ctrl = self.sessionProvider.shared.feedback
        if ctrl is not None:
            ctrl.onVehicleFeedbackReceived += self.__onVehicleFeedbackReceived
        g_playerEvents.onRoundFinished += self.__onRoundFinished
        CommandMapping.g_instance.onMappingChanged += self.__onMappingChanged
        self.as_setupTextsS(str(i18n.makeString(INGAME_GUI.RECOVERY_HINT1)), str(i18n.makeString(INGAME_GUI.RECOVERY_HINT2)), getReadableKey(CommandMapping.CMD_REQUEST_RECOVERY))
    ...
    def _dispose(self):
        super(FrontlineRecoveryPanel, self)._dispose()
        self.__nextActionTime = -1
    ...
    def __tick(self):
        if self.__nextActionTime > 0 and self.__recoveryActivated:
            ...
            if diffTime > 0:
                ...
                self.as_updateTimerS(str(i18n.makeString(INGAME_GUI.RECOVERY_COOLDOWN, cooldown=timerText)))
            else:
                self.__nextActionTime = -1
                self.as_updateTimerS(str(i18n.makeString(INGAME_GUI.RECOVERY_COOLDOWN, cooldown=timerText)))
                self.__cooldownTimerCallback = None
