...
from frontline.gui.prb_control.entities.epic_battle_training.limits import EpicBattleTrainingLimits
from frontline.gui.prb_control.entities.epic_battle_training.permissions import EpicBattleTrainingPermissions, EpicBattleTrainingIntroPermissions
from frontline.gui.prb_control.entities.epic_battle_training.requester import EpicBattleTrainingListRequester
...
from gui.prb_control.entities.training.legacy.ctx import SetPlayerObserverStateCtx
...
class EpicBattleTrainingEntryPoint(LegacyEntryPoint):
    ...
    def create(self, ctx, callback=None):
        if not isinstance(ctx, EpicTrainingSettingsCtx):
            ...
            player.FLAccountComponent.createEpicTrainingPrebattle(ctx.getArenaTypeID(), ctx.getRoundLen(), ctx.isOpened(), ctx.getComment())
