...
class PrebattleVehicle(IPrebattleVehicle):
    ...
    def selectAny(self):
        g_eventBus.handleEvent(events.LobbySimpleEvent(events.HangarSimpleEvent.UPDATE_CAROUSEL_VEHICLE_STATES),
                               scope=EVENT_BUS_SCOPE.LOBBY)
        bosses = self.itemsCache.items.getVehicles(
            REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS({VEHICLE_TAGS.EVENT_BOSS}))
        hunters = self.itemsCache.items.getVehicles(
            REQ_CRITERIA.INVENTORY | REQ_CRITERIA.VEHICLE.HAS_TAGS({VEHICLE_TAGS.EVENT_HUNTER}))
    ...
    def isSquad(self):
        if self.prbDispatcher:
            ...
            isSquad = state.isInUnit(PREBATTLE_TYPE.SQUAD) or state.isInUnit(PREBATTLE_TYPE.EVENT) or state.isInUnit(
                PREBATTLE_TYPE.EPIC)
            return isSquad
        return False
