...
from constants import ACTION_LABEL_TO_TYPE, ROLE_LABEL_TO_TYPE, ROLE_TYPE, DamageAbsorptionLabelToType, ROLE_LEVELS, \
    ROLE_TYPE_TO_LABEL
...
from math_common import ceilTo
from post_progression_common import POST_PROGRESSION_ALL_PRICES, ALLOWED_CURRENCIES_FOR_TREE_STEP, \
    ALLOWED_CURRENCIES_FOR_BUY_MODIFICATION_STEP, ALLOWED_CURRENCIES_FOR_CUSTOM_ROLE_SLOT_CHANGE, \
    POST_PROGRESSION_UNLOCK_MODIFICATIONS_PRICES, CUSTOM_ROLE_SLOT_CHANGE_PRICE, \
    POST_PROGRESSION_BUY_MODIFICATIONS_PRICES
from soft_exception import SoftException
from string import upper
from typing import Any, TYPE_CHECKING, Union
from constants import SHELL_MECHANICS_TYPE, TrackBreakMode
...
def _makeExtraNames(tankmanNames):
    retVal = {}
    extraSuffix = 'Health'
    edgeCases = {'track': ['leftTrack0' + extraSuffix, 'rightTrack0' + extraSuffix],
...
class VehicleDescriptor(object):
    ...
    __slots__ = (
    'enhancements', 'turret', 'gun', 'hull', 'engine', 'fuelTank', 'radio', 'chassis', 'turrets', 'optionalDevices',
    'shot', 'supplySlots', 'camouflages', 'playerEmblems', 'playerInscriptions', 'type', 'name', 'level', 'extras',
    'extrasDict', 'miscAttrs', 'physics', 'visibilityCheckPoints', 'observerPosOnChassis', 'observerPosOnTurret',
    '_customRoleSlotTypeId', '_modifications', '_optDevSlotsMap', '_maxHealth', '__activeTurretPos',
    '__activeGunShotIdx', '__activeGunShotPosition', '__boundingRadius')
    ...
    def __applyExternalData(self, extData):
        ...
        def getValue(key):
            if isinstance(extData, dict):
                return extData.get(key, None)
            else:
                return getattr(extData, key, None)
                return None

        self._customRoleSlotTypeId = 0
        self._modifications = []
        value = getValue('customRoleSlotTypeId') or 0
        self.installCustomRoleSlot(value, False)
        value = getValue('vehPostProgression') or []
...
class VehicleType(object):
    ...
    def retrieveSectionToSave(self, mainXmlPath, useSharedSections=True):
        ...
        if useSharedSections:
            for componentId in ITEM_TYPES.values():
                if componentId in Cache.NATION_ITEM_SOURCE:
                    compsXmlPath = '{vehcilePath}{nationName}{componentsPath}{componentSource}'.format(
                        vehcilePath=_VEHICLE_TYPE_XML_PATH, nationName=nationName,
                        componentsPath=Cache.NATION_COMPONENTS_SECTION,
                        componentSource=Cache.NATION_ITEM_SOURCE[componentId])
        ...
        _writeInstallableComponents(self.chassis, mainSection, 'chassis', _writeChassis, g_cache.chassisIDs(nationID),
                                    sharedSections, materialData=materialData['chassis'])
        defHull = self.hulls[0]
        for n in xrange(len(defHull.turretPositions)):
            _writeInstallableComponents(self.turrets[n], mainSection, 'turrets' + repr(n), _writeTurret,
                                        g_cache.turretIDs(nationID), sharedSections, materialData=materialData)
        ...
        for turrets in self.turrets:
            for turret in turrets:
                ...
                for gun in turret.guns:
                    ...
                    if repeatingTags:
                        LOG_ERROR('repeating matching tags between hull, turret and gun : %s for %s' % (repeatingTags, self.name))
...
def _writeInstallableComponents(components, section, subsectionName, writer, cachedIDs, sharedSections, **kwargs):
    ...
    for component in components:
        ...
        writer(component, combinedSection, sharedSections, kwargs['materialData'],
               parentName=kwargs['parentName'] if 'parentName' in kwargs else '')
...
def _writeTurret(item, section, sharedSections, materialData, *args, **kwargs):
    ...
    _writeCamouflageSettings(section, 'camouflage', item.camouflage)
    _writeArmor(item.materials, None, section, 'armor', materialData=materialData['turret'].get(item.name, None),
                precedingSectionName='guns')
    slots = item.emblemSlots + item.slotsAnchors
    shared_writers.writeCustomizationSlots(slots, section, 'customizationSlots')
    _writeCustomizableAreas(item.customizableVehicleAreas, section)
    shared_writers.writeModelsSets(item.modelsSets, section['models'])
    ...
    _writeInstallableComponents(item.guns, section, 'guns', _writeGun, g_cache.gunIDs(nationID), sharedSections,
                                materialData=materialData['gun'], parentName=item.name)
...
def _writeGun(item, section, sharedSections, materialData, *args, **kwargs):
    ...
    _writeArmor(item.materials, None, section, 'armor', optional=True,
                materialData=materialData.get(item.name + kwargs['parentName'], None))
    slots = item.emblemSlots + item.slotsAnchors
    shared_writers.writeCustomizationSlots(slots, section, 'customizationSlots')
    _writeCustomizableAreas(item.customizableVehicleAreas, section)
    shared_writers.writeModelsSets(item.modelsSets, section['models'])
...
def _readShell(xmlCtx, section, name, nationID, shellTypeID, icons):
    ...
    shell.damage = (_readDamageValue(xmlCtx, section, 'armor', isModernHighExplosive),
                    _readDamageValue(xmlCtx, section, 'devices', isModernHighExplosive))
    ...
    if kind == 'HIGH_EXPLOSIVE':
        ...
        if isModernHighExplosive:
            shellType.obstaclePenetration = _xml.readBool(xmlCtx, section, 'obstaclePenetration',
                                                          component_constants.DEFAULT_MODERN_HE_OBSTACLE_PENETRATION)
            shellType.shieldPenetration = _xml.readBool(xmlCtx, section, 'shieldPenetration',
                                                        component_constants.DEFAULT_MODERN_HE_SHIELD_PENETRATION)
            ...
            if not (blastWave.isActive or shellFragments.isActive or armorSpalls.isActive):
                _xml.raiseWrongXml(xmlCtx, '',
                                   'Modern high explosive shell must contain at least one damage mechanics: blastWave, shellFragments, armorSpalls')
            shellType.blastWave = blastWave
            shellType.shellFragments = shellFragments
            shellType.armorSpalls = armorSpalls
            shellType.maxDamage = max(shellFragments.damages[0], shellFragments.damages[1], armorSpalls.damages[0],
                                      armorSpalls.damages[1], blastWave.damages[0], blastWave.damages[1])
...
def _readDamageValue(xmlCtx, section, damageTypeName, isModernHighExplosive):
    ...
    return _xml.readNonNegativeFloat(xmlCtx, section,
                                     subsectionName) if isModernHighExplosive else _xml.readPositiveFloat(xmlCtx,
                                                                                                          section,
                                                                                                          subsectionName)
...
def _readGunShotDispersionFactors(xmlCtx, section, subsectionName):
    res = {
        'turretRotation': _xml.readNonNegativeFloat(xmlCtx, section, subsectionName + '/turretRotation') / radians(1.0),
...
def _writeArmor(armor, xmlCtx, section, subsectionName, optional=False, index=0, materialData=None,
                precedingSectionName=None):
    if not armor and optional:
        ...
        for matKind, matInfo in armor.items():
            ...
            hasChanges = matInfo.armor != 0 or matInfo.vehicleDamageFactor != defMatInfo[
                'vehicleDamageFactor'] or matInfo.chanceToHitByProjectile != defMatInfo[
                             'chanceToHitByProjectile'] or matInfo.chanceToHitByExplosion != defMatInfo[
                             'chanceToHitByExplosion'] or materialData is not None and matKind in materialData and material_kinds.needToWriteZeroMaterial(
                matKind)
            if hasChanges:
                _xml.rewriteFloat(armorSection, matKindName, matInfo.armor)
            _xml.rewriteFloat(armorSection, matKindName + '/vehicleDamageFactor', matInfo.vehicleDamageFactor,
                              defMatInfo['vehicleDamageFactor'])
            _xml.rewriteFloat(armorSection, matKindName + '/chanceToHitByProjectile', matInfo.chanceToHitByProjectile,
                              defMatInfo['chanceToHitByProjectile'])
            _xml.rewriteFloat(armorSection, matKindName + '/chanceToHitByExplosion', matInfo.chanceToHitByExplosion,
                              defMatInfo['chanceToHitByExplosion'])
...
def _readCamouflageTilings(xmlCtx, section, sectionName, defNationID):
    ...
    return (res, nameMap) if IS_UE_EDITOR else res
...
def _readVehicleEffects(xmlCtx, section, subsectionName, defaultEffects=None, useOverride=False):
    ...
    return (res, damagedStateGroupName) if IS_UE_EDITOR else res
...
def _readImpactParams(xmlCtx, section, paramName):
    ...
    if subsection is None:
        ...
        if paramName == 'armorSpalls':
            ...
            params.piercingSpalls = _xml.readBool(subXmlCtx, subsection, 'piercingSpalls',
                                                  component_constants.DEFAULT_PIERCING_SPALLS)
...
def _readBrokenTrackLosses(xmlCtx, section):
    if not section.has_key('brokenTrackLosses'):
        ...
        def readLoss(paramName):
            return None if subsection[paramName] is None else _xml.readTupleOfFloats(ctx, subsection, paramName)
    ...
    if vehType.isWheeledVehicle:
        ...
        for chassis in vehType.chassis:
            ...
            for lossName, losses in brokenTrackLosses.iteritems():
                ...
                if len(losses) != maxLossesCount:
                    _xml.raiseWrongXml((chassisCtx, 'brokenTrackLosses'), lossName, '%d floats expected' % maxLossesCount)
...
def _readBrokenWheelLosses(xmlCtx, section, axleIsLeading, axleCanBeRised, wheelRiseHeight):
    ...
    def readLoss(paramName, minCount):
        return (0.0,) * minCount if section[paramName] is None else _xml.readTupleOfFloats(xmlCtx, section, paramName, count=minCount)
...
def _readPostProgressionPricesOverrides(xmlCtx, section):
    if section is None:
        ...
        for sname, _ in section.items():
            ...
            priceConstants = [(POST_PROGRESSION_UNLOCK_MODIFICATIONS_PRICES, ALLOWED_CURRENCIES_FOR_TREE_STEP), (POST_PROGRESSION_BUY_MODIFICATIONS_PRICES, ALLOWED_CURRENCIES_FOR_BUY_MODIFICATION_STEP), ((CUSTOM_ROLE_SLOT_CHANGE_PRICE,), ALLOWED_CURRENCIES_FOR_CUSTOM_ROLE_SLOT_CHANGE)]
