from __future__ import unicode_literals
import inspect, sys, math, numbers
from future.utils import PY2, PY3, exec_
if PY2:
    from collections import Mapping
else:
    from collections.abc import Mapping
if PY3:
    import builtins
    from collections.abc import Mapping

    def apply(f, *args, **kw):
        return f(*args, **kw)


    from past.builtins import str as oldstr

    def chr(i):
        return oldstr(bytes((i,)))


    def cmp(x, y):
        if isinstance(x, set) and isinstance(y, set):
            raise TypeError(b'cannot compare sets using cmp()')
        try:
            if isinstance(x, numbers.Number) and math.isnan(x):
                if not isinstance(y, numbers.Number):
                    raise TypeError((b'cannot compare float("nan"), {type_y} with cmp').format(type_y=type(y)))
                if isinstance(y, int):
                    return 1
                return -1
            if isinstance(y, numbers.Number) and math.isnan(y):
                if not isinstance(x, numbers.Number):
                    raise TypeError((b'cannot compare {type_x}, float("nan") with cmp').format(type_x=type(x)))
                if isinstance(x, int):
                    return -1
                return 1
            return (x > y) - (x < y)
        except TypeError:
            if x == y:
                return 0
            type_order = [type(None),
             numbers.Number,
             dict, list,
             set,
             (
              str, bytes)]
            x_type_index = y_type_index = None
            for i, type_match in enumerate(type_order):
                if isinstance(x, type_match):
                    x_type_index = i
                if isinstance(y, type_match):
                    y_type_index = i

            if cmp(x_type_index, y_type_index) == 0:
                if isinstance(x, bytes) and isinstance(y, str):
                    return cmp(x.decode(b'ascii'), y)
                if isinstance(y, bytes) and isinstance(x, str):
                    return cmp(x, y.decode(b'ascii'))
                if isinstance(x, list):
                    for x_elem, y_elem in zip(x, y):
                        elem_cmp_val = cmp(x_elem, y_elem)
                        if elem_cmp_val != 0:
                            return elem_cmp_val

                    return 0
                if isinstance(x, dict):
                    if len(x) != len(y):
                        return cmp(len(x), len(y))
                    else:
                        x_key = min(a for a in x if a not in y or x[a] != y[a])
                        y_key = min(b for b in y if b not in x or x[b] != y[b])
                        if x_key != y_key:
                            return cmp(x_key, y_key)
                        return cmp(x[x_key], y[y_key])

            return cmp(x_type_index, y_type_index)

        return


    from sys import intern

    def oct(number):
        return b'0' + builtins.oct(number)[2:]


    raw_input = input
    if sys.version_info >= (3, 6):
        from importlib import reload
    else:
        from imp import reload
    unicode = str
    unichr = chr
    xrange = range
else:
    import __builtin__
    from collections import Mapping
    apply = __builtin__.apply
    chr = __builtin__.chr
    cmp = __builtin__.cmp
    execfile = __builtin__.execfile
    intern = __builtin__.intern
    oct = __builtin__.oct
    raw_input = __builtin__.raw_input
    reload = __builtin__.reload
    unicode = __builtin__.unicode
    unichr = __builtin__.unichr
    xrange = __builtin__.xrange
if PY3:

    def execfile(filename, myglobals=None, mylocals=None):
        if myglobals is None:
            caller_frame = inspect.stack()[1]
            myglobals = caller_frame[0].f_globals
            mylocals = caller_frame[0].f_locals
        elif mylocals is None:
            mylocals = myglobals
        if not isinstance(myglobals, Mapping):
            raise TypeError(b'globals must be a mapping')
        if not isinstance(mylocals, Mapping):
            raise TypeError(b'locals must be a mapping')
        with open(filename, b'rb') as (fin):
            source = fin.read()
        code = compile(source, filename, b'exec')
        exec_(code, myglobals, mylocals)
        return


if PY3:
    __all__ = [
     b'apply', b'chr', b'cmp', b'execfile', b'intern', b'raw_input',
     b'reload', b'unichr', b'unicode', b'xrange']
else:
    __all__ = []