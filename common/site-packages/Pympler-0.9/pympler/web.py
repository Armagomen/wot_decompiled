...
def tracker_index():
    ...
    if stats and stats.snapshots:
        ...
        if stats.snapshots[0].system_total.data_segment:
            series = [ s.system_total.data_segment - s.tracked_total - s.overhead for s in stats.snapshots
                     ]
            ...
            series = [ s.total - s.tracked_total - s.overhead for s in stats.snapshots
                     ]
            timeseries.append(('Other', series))
        timeseries = [ dict(label=label, data=list(enumerate(data))) for label, data in timeseries ]
