...
import types as Types, warnings, weakref as Weakref
__all__ = [
...
_builtin_modules = (
 int.__module__, 'types', Exception.__module__)
...
_all_refs = (
 None, _cell_refs, _class_refs, _co_refs, _dict_refs, _enum_refs,
...
_all_lens = (
 None, _len, _len_bytearray, _len_code, _len_dict,
...
try:
    ...
    class _Instkey(object):
        ...
        def __str__(self):
            t = (
             _moduleof(self._obj), self._obj.__name__, _old_style)
    ...
    def _keytuple(obj):
        ...
        if t is _Types_InstanceType:
            t = obj.__class__
            return (
             _claskey(t, _old_style), _instkey(t))
        else:
...
class _Typedef(object):
    ...
    def args(self):
        return (
         self.base, self.item, self.leng, self.refs,
...
_dict_classes = {'UserDict': ('IterableUserDict', 'UserDict'), 'weakref': (
...
class Asized(object):
    ...
    def format(self, format='%(name)s size=%(size)d flat=%(flat)d', depth=-1, order_by='size', indent=''):
        ...
        if depth and self.refs:
            rs = sorted(self.refs, key=lambda x: getattr(x, order_by), reverse=order_by in ('size',
                                                                                            'flat'))
...
class Asizer(object):
    ...
    def reset(self, above=1024, align=8, clip=80, code=False, cutoff=10, derive=False, detail=0, frames=False, ignored=True, infer=False, limit=100, stats=0, stream=None, **extra):
        if extra:
            t = (
             _plural(len(extra)), _kwdstr(**extra))
