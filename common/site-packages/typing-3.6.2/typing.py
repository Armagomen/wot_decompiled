...
# Compiled at: 2053-05-04 15:05:04
...
class TypingMeta(type):
    u"""Metaclass for most types defined in typing module
    ...
    def _eval_type(self, globalns, localns):
        u"""Override this in subclasses to interpret forward references.
...
class _TypingBase(object):
    u"""Internal indicator of special typing constructs."""
...
class _FinalTypingBase(_TypingBase):
    u"""Internal mix-in class to prevent instantiation.
...
class _TypeAlias(_TypingBase):
    u"""Internal helper class for defining generic variants of concrete types.
...
def _type_check(arg, msg):
    u"""Check that the argument is a type, and return it (internal helper).
    ...
    if arg is None:
        ...
        if isinstance(arg, basestring):
            arg = _ForwardRef(arg)
        if isinstance(arg, _TypingBase) and type(arg).__name__ == u'_ClassVar' or not isinstance(arg, (
        type, _TypingBase)) and not callable(arg):
            raise TypeError(msg + u' Got %.100r.' % (arg,))
        if type(arg).__name__ in (u'_Union', u'_Optional') and not getattr(arg, u'__origin__', None) or isinstance(arg,
                                                                                                                   TypingMeta) and _gorg(
...
class _ClassVar(_FinalTypingBase):
    u"""Special type construct to mark class variables.
...
class _Any(_FinalTypingBase):
    u"""Special type indicating an unconstrained type.
...
class _NoReturn(_FinalTypingBase):
    u"""Special type indicating functions that never return.
    Example::
...
def _replace_arg(arg, tvars, args):
    u"""An internal helper function: replace arg if it is a type variable
...
def _subs_tree(cls, tvars=None, args=None):
    u"""An internal helper function: calculate substitution tree
...
def _remove_dups_flatten(parameters):
    u"""An internal helper for Union creation and substitution: flatten Union's
...
class _Union(_FinalTypingBase):
    u"""Union type; Union[X, Y] means either X or Y.
...
def _gorg(a):
    u"""Return the farthest origin of a generic class (internal helper)."""
...
def _geqv(a, b):
    u"""Return whether two generic classes are equivalent (internal helper).
...
def _make_subclasshook(cls):
    u"""Construct a __subclasshook__ callable that incorporates
...
class GenericMeta(TypingMeta, abc.ABCMeta):
    ...
    * if Generic[...] is present, then it should list all type variables
    ...
    def __new__(cls, name, bases, namespace, tvars=None, args=None, origin=None, extra=None, orig_bases=None):
        u"""Create a new generic class. GenericMeta.__new__ accepts
...
class Generic(object):
    u"""Abstract base class for generic types.
...
class _TypingEmpty(object):
    u"""Internal placeholder for () or []. Used by TupleMeta and CallableMeta
...
class _TypingEllipsis(object):
    u"""Internal placeholder for ... (ellipsis)."""
    pass


class TupleMeta(GenericMeta):
    u"""Metaclass for Tuple (internal)."""
...
class Tuple(tuple):
    u"""Tuple type; Tuple[X, Y] is the cross-product type of X and Y.
...
class CallableMeta(GenericMeta):
    ...
    def __getitem__(self, parameters):
        u"""A thin wrapper around __getitem_inner__ to provide the latter
...
class Callable(object):
    u"""Callable type; Callable[[int], str] is a function of (int) -> str.
...
def _get_defaults(func):
    u"""Internal helper to extract the default arguments, by name."""
    code = func.__code__
...
def get_type_hints(obj, globalns=None, localns=None):
    u"""In Python 2 this is not supported and always returns None."""
    return None


def no_type_check(arg):
    u"""Decorator to indicate that annotations are not type hints.
...
def no_type_check_decorator(decorator):
    u"""Decorator to give another decorator the @no_type_check effect.
...
def _overload_dummy(*args, **kwds):
    u"""Helper for @overload to raise when called."""
...
def overload(func):
    u"""Decorator for overloaded functions/methods.
...
class _ProtocolMeta(GenericMeta):
    u"""Internal metaclass for _Protocol.
...
class _Protocol(object):
    u"""Internal base class for protocol classes.
...
class Type(Generic[CT_co]):
    u"""A special construct usable to annotate class objects.
...
def NewType(name, tp):
    u"""NewType creates simple unique types with almost zero
...
class IO(Generic[AnyStr]):
    u"""Generic base class for TextIO and BinaryIO.
...
class BinaryIO(IO[bytes]):
    u"""Typed version of the return of open() in binary mode."""
    __slots__ = ()
...
class TextIO(IO[unicode]):
    u"""Typed version of the return of open() in text mode."""
    __slots__ = ()
...
class io(object):
    u"""Wrapper namespace for IO generic classes."""
