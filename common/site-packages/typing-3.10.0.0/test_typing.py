...
import collections, contextlib, os, pickle, re, subprocess, sys, abc, types
...
import typing, weakref, collections

class BaseTestCase(TestCase):

    def assertIsSubclass(self, cls, class_or_tuple, msg=None):
        if not issubclass(cls, class_or_tuple):
            message = b'%r is not a subclass of %r' % (cls, class_or_tuple)
            if msg is not None:
                message += b' : %s' % msg
    ...
    def assertNotIsSubclass(self, cls, class_or_tuple, msg=None):
        if issubclass(cls, class_or_tuple):
            message = b'%r is a subclass of %r' % (cls, class_or_tuple)
            if msg is not None:
                message += b' : %s' % msg
            raise self.failureException(message)
        return
...
class AnyTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(Any), b'typing.Any')
...
class NoReturnTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(NoReturn), b'typing.NoReturn')
...
class TypeVarTests(BaseTestCase):
    ...
    def test_union_constrained(self):
        A = TypeVar(b'A', str, bytes)
        self.assertNotEqual(Union[(A, str)], Union[A])

    def test_repr(self):
        self.assertEqual(repr(T), b'~T')
        self.assertEqual(repr(KT), b'~KT')
        self.assertEqual(repr(VT), b'~VT')
        self.assertEqual(repr(AnyStr), b'~AnyStr')
        T_co = TypeVar(b'T_co', covariant=True)
        self.assertEqual(repr(T_co), b'+T_co')
        T_contra = TypeVar(b'T_contra', contravariant=True)
        self.assertEqual(repr(T_contra), b'-T_contra')

    def test_no_redefinition(self):
        self.assertNotEqual(TypeVar(b'T'), TypeVar(b'T'))
        self.assertNotEqual(TypeVar(b'T', int, str), TypeVar(b'T', int, str))
    ...
    def test_bound_errors(self):
        with self.assertRaises(TypeError):
            TypeVar(b'X', bound=42)
        with self.assertRaises(TypeError):
            TypeVar(b'X', str, float, bound=Employee)

    def test_no_bivariant(self):
        with self.assertRaises(ValueError):
            TypeVar(b'T', covariant=True, contravariant=True)
...
class UnionTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(Union), b'typing.Union')
        u = Union[(Employee, int)]
        self.assertEqual(repr(u), b'typing.Union[%s.Employee, int]' % __name__)
        u = Union[(int, Employee)]
        self.assertEqual(repr(u), b'typing.Union[int, %s.Employee]' % __name__)
        T = TypeVar(b'T')
        u = Union[(T, int)][int]
        self.assertEqual(repr(u), repr(int))
        u = Union[(List[int], int)]
        self.assertEqual(repr(u), b'typing.Union[typing.List[int], int]')
    ...
    def test_function_repr_union(self):
        ...
        self.assertEqual(repr(Union[(fun, int)]), b'typing.Union[fun, int]')
    ...
    def test_etree(self):
        try:
            ...
            raise SkipTest(b'cElementTree not found')
...
class TupleTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(Tuple), b'typing.Tuple')
        self.assertEqual(repr(Tuple[()]), b'typing.Tuple[()]')
        self.assertEqual(repr(Tuple[(int, float)]), b'typing.Tuple[int, float]')
        self.assertEqual(repr(Tuple[(int, ...)]), b'typing.Tuple[int, ...]')
...
class CallableTests(BaseTestCase):
    ...
    def test_repr(self):
        ct0 = Callable[([], bool)]
        self.assertEqual(repr(ct0), b'typing.Callable[[], bool]')
        ct2 = Callable[([str, float], int)]
        self.assertEqual(repr(ct2), b'typing.Callable[[str, float], int]')
        ctv = Callable[(..., str)]
        self.assertEqual(repr(ctv), b'typing.Callable[..., str]')

    def test_ellipsis_in_generic(self):
        typing.List[Callable[(..., str)]]


XK = TypeVar(b'XK', unicode, bytes)
XV = TypeVar(b'XV')
...
class ProtocolTests(BaseTestCase):
    ...
    def test_custom_subclasshook(self):
        ...
        class C(P):
            ...
            def __subclasshook__(cls, other):
                return other.__name__.startswith(b'OK')
    ...
    def test_init_called(self):
        ...
        self.assertEqual(C[int]().test, b'OK')
    ...
    def test_protocols_bad_subscripts(self):
        ...
        with self.assertRaises(TypeError):
            ...
            class P(Protocol[T], typing.Mapping[(T, S)]):
                pass
    ...
    def test_generic_protocols_repr(self):
        ...
        self.assertTrue(repr(P).endswith(b'P'))
        self.assertTrue(repr(P[(T, S)]).endswith(b'P[~T, ~S]'))
        self.assertTrue(repr(P[(int, str)]).endswith(b'P[int, str]'))
    ...
    def test_generic_protocols_eq(self):
        ...
        self.assertEqual(P[(T, T)][typing.Tuple[(T, S)]][(int, str)], P[(typing.Tuple[(int, str)], typing.Tuple[(int, str)])])
    ...
    def test_generic_protocols_special_from_protocol(self):
        ...
        self.assertEqual(P._get_protocol_attrs(), {b'meth'})
        self.assertEqual(PR._get_protocol_attrs(), {b'x'})
        self.assertEqual(frozenset(PG._get_protocol_attrs()), frozenset({b'x', b'meth'}))
        self.assertEqual(frozenset(PG[int]._get_protocol_attrs()), frozenset({b'x', b'meth'}))
    ...
    def test_protocols_pickleable(self):
        ...
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            ...
            self.assertEqual(x.bar, b'abc')
            self.assertEqual(x.x, 1)
            self.assertEqual(x.__dict__, {b'foo': 42, b'bar': b'abc'})
            s = pickle.dumps(P)
            D = pickle.loads(s)
    ...
    def test_protocol_instance_works(self):
        ...
        self.assertNotIsInstance(b'no', typing.SupportsAbs)
...
class GenericTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(SimpleMapping), __name__ + b'.' + b'SimpleMapping')
        self.assertEqual(repr(MySimpleMapping), __name__ + b'.' + b'MySimpleMapping')
    ...
    def test_chain_repr(self):
        ...
        self.assertTrue(str(Z).endswith(b'.C[typing.Tuple[str, int]]'))

    def test_new_repr(self):
        T = TypeVar(b'T')
        U = TypeVar(b'U', covariant=True)
        S = TypeVar(b'S')
        self.assertEqual(repr(List), b'typing.List')
        self.assertEqual(repr(List[T]), b'typing.List[~T]')
        self.assertEqual(repr(List[U]), b'typing.List[+U]')
        self.assertEqual(repr(List[S][T][int]), b'typing.List[int]')
        self.assertEqual(repr(List[int]), b'typing.List[int]')

    def test_new_repr_complex(self):
        T = TypeVar(b'T')
        TS = TypeVar(b'TS')
        self.assertEqual(repr(typing.Mapping[(T, TS)][(TS, T)]), b'typing.Mapping[~TS, ~T]')
        self.assertEqual(repr(List[Tuple[(T, TS)]][(int, T)]), b'typing.List[typing.Tuple[int, ~T]]')
        self.assertEqual(repr(List[Tuple[(T, T)]][List[int]]), b'typing.List[typing.Tuple[typing.List[int], typing.List[int]]]')

    def test_new_repr_bare(self):
        T = TypeVar(b'T')
        self.assertEqual(repr(Generic[T]), b'typing.Generic[~T]')
        self.assertEqual(repr(typing.Protocol[T]), b'typing.Protocol[~T]')
    ...
    def test_dict(self):
        ...
        b = B()
        b.foo = 42
        self.assertEqual(b.__dict__, {b'foo': 42})
        ...
        c = C()
        c.bar = b'abc'
        self.assertEqual(c.__dict__, {b'bar': b'abc'})
    ...
    def test_subscripted_generics_as_proxies(self):
        ...
        self.assertEqual(C[int].x, b'def')
        self.assertEqual(C[C[int]].x, b'def')
        C[C[int]].x = b'changed'
        self.assertEqual(C.x, b'changed')
        self.assertEqual(C[str].x, b'changed')
        C[List[str]].z = b'new'
        self.assertEqual(C.z, b'new')
        self.assertEqual(C[Tuple[int]].z, b'new')
        self.assertEqual(C().x, b'changed')
        self.assertEqual(C[Tuple[str]]().z, b'new')

        class D(C[T]):
            pass

        self.assertEqual(D[int].x, b'changed')
        self.assertEqual(D.z, b'new')
        D.z = b'from derived z'
        D[int].x = b'from derived x'
        self.assertEqual(C.x, b'changed')
        self.assertEqual(C[int].z, b'new')
        self.assertEqual(D.x, b'from derived x')
        self.assertEqual(D[str].z, b'from derived z')
    ...
    def test_naive_runtime_checks(self):
        ...
        self.assertTrue(naive_dict_check({b'x': 1}, typing.Dict[(typing.Text, int)]))
        self.assertFalse(naive_dict_check({1: b'x'}, typing.Dict[(typing.Text, int)]))
        with self.assertRaises(NotImplementedError):
            naive_dict_check({1: b'x'}, typing.Dict[(typing.Text, T)])

        def naive_generic_check(obj, tp):
            if not hasattr(obj, b'__orig_class__'):
                raise NotImplementedError
        ...
        self.assertFalse(naive_list_base_check([b'a', b'b'], C))
    ...
    def test_generic_hashes(self):
        ...
        self.assertTrue(repr(Tuple[A[str]]).endswith(b'test_typing.A[str]]'))
        self.assertTrue(repr(Tuple[mod_generics_cache.A[str]]).endswith(b'mod_generics_cache.A[str]]'))
    ...
    def test_extended_generic_rules_repr(self):
        T = TypeVar(b'T')
        self.assertEqual(repr(Union[(Tuple, Callable)]).replace(b'typing.', b''), b'Union[Tuple, Callable]')
        self.assertEqual(repr(Union[(Tuple, Tuple[int])]).replace(b'typing.', b''), b'Tuple')
        self.assertEqual(repr(Callable[(..., Optional[T])][int]).replace(b'typing.', b''), b'Callable[..., Union[int, NoneType]]')
        self.assertEqual(repr(Callable[([], List[T])][int]).replace(b'typing.', b''), b'Callable[[], List[int]]')
    ...
    def test_extended_generic_rules_subclassing(self):
        ...
        with self.assertRaises(TypeError):
            T2[(int, str)]
        self.assertEqual(repr(C1[int]).split(b'.')[(-1)], b'C1[int]')
    ...
    def test_all_repr_eq_any(self):
        ...
        for obj in objs:
            self.assertNotEqual(repr(obj), b'')
            self.assertEqual(obj, obj)
            if getattr(obj, b'__parameters__', None) and len(obj.__parameters__) == 1:
                self.assertEqual(obj[Any].__args__, (Any,))
            if isinstance(obj, type):
                for base in obj.__mro__:
                    self.assertNotEqual(repr(base), b'')
    ...
    def test_pickle(self):
        ...
        simples = [
         Any, Union, Tuple, Callable, ClassVar, List, typing.Iterable]
        for s in simples:
            for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    ...
    def test_weakref_all(self):
        ...
        things = [Any, Union[(T, int)], Callable[(..., T)], Tuple[(Any, Any)],
         ...
         typing.re.Match[bytes], typing.Iterable[b'whatever']]
        for t in things:
    ...
    def test_parameterized_slots(self):
        ...
        with self.assertRaises(AttributeError):
            c_int.tomato = 0
        self.assertEqual(typing._eval_type(C[b'C'], globals(), locals()), C[C])
        self.assertEqual(typing._eval_type(C[b'C'], globals(), locals()).__slots__, C.__slots__)
    ...
    def test_repr_2(self):
        ...
        if not PY32:
            self.assertEqual(C.__qualname__, b'GenericTests.test_repr_2.<locals>.C')
        self.assertEqual(repr(C).split(b'.')[(-1)], b'C')
        X = C[int]
        self.assertEqual(X.__module__, __name__)
        if not PY32:
            self.assertTrue(X.__qualname__.endswith(b'.<locals>.C'))
        self.assertEqual(repr(X).split(b'.')[(-1)], b'C[int]')
        ...
        if not PY32:
            self.assertEqual(Y.__qualname__, b'GenericTests.test_repr_2.<locals>.Y')
        self.assertEqual(repr(Y).split(b'.')[(-1)], b'Y')
    ...
    def test_new_with_args2(self):
        ...
        c = C(b'foo')
        self.assertEqual(c.from_a, b'foo')
        self.assertEqual(c.from_c, b'foo')
    ...
    def test_new_no_args(self):
        ...
        c = C(b'foo')
        self.assertEqual(c.arg, b'foo')
        self.assertEqual(c.from_b, b'b')
        self.assertEqual(c.from_c, b'c')
...
class ClassVarTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(ClassVar), b'typing.ClassVar')
        cv = ClassVar[int]
        self.assertEqual(repr(cv), b'typing.ClassVar[int]')
        cv = ClassVar[Employee]
        self.assertEqual(repr(cv), b'typing.ClassVar[%s.Employee]' % __name__)
...
class FinalTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(Final), b'typing.Final')
        cv = Final[int]
        self.assertEqual(repr(cv), b'typing.Final[int]')
        cv = Final[Employee]
        self.assertEqual(repr(cv), b'typing.Final[%s.Employee]' % __name__)
...
class LiteralTests(BaseTestCase):
    ...
    def test_illegal_parameters_do_not_raise_runtime_errors(self):
        ...
        Literal[(complex(2.0, 3.0), ..., ())]
        Literal[('foo', 'bar')]
        Literal[{b'foo': 3, b'bar': 4}]
        Literal[T]
    ...
    def test_repr(self):
        self.assertEqual(repr(Literal[1]), b'typing.Literal[1]')
        self.assertEqual(repr(Literal[(1, True, b'foo')]), b"typing.Literal[1, True, u'foo']")
        self.assertEqual(repr(Literal[int]), b'typing.Literal[int]')
        self.assertEqual(repr(Literal), b'typing.Literal')
        self.assertEqual(repr(Literal[None]), b'typing.Literal[None]')
        return
...
class ForwardRefTests(BaseTestCase):
    ...
    def test_forward_equality(self):
        fr = typing._ForwardRef(b'int')
        self.assertEqual(fr, typing._ForwardRef(b'int'))
        self.assertNotEqual(List[b'int'], List[int])

    def test_forward_repr(self):
        self.assertEqual(repr(List[b'int']), b'typing.List[_ForwardRef(%r)]' % b'int')
...
class CollectionsAbcTests(BaseTestCase):
    ...
    def test_subclassing(self):
        ...
        if hasattr(collections, b'Generator'):
    ...
    def test_collections_as_base(self):
        ...
        class It(collections.Iterable):
            pass

        self.assertIsSubclass(It, typing.Iterable)

        class A(collections.Mapping):
            pass
...
class NewTypeTests(BaseTestCase):
    ...
    def test_basic(self):
        UserId = NewType(b'UserId', int)
        UserName = NewType(b'UserName', str)
        self.assertIsInstance(UserId(5), int)
        self.assertIsInstance(UserName(b'Joe'), type(b'Joe'))
...
class NamedTupleTests(BaseTestCase):
    ...
    def test_basics(self):
        Emp = NamedTuple(b'Emp', [(b'name', str), (b'id', int)])
        self.assertIsSubclass(Emp, tuple)
        joe = Emp(b'Joe', 42)
        jim = Emp(name=b'Jim', id=1)
        self.assertIsInstance(joe, Emp)
        self.assertIsInstance(joe, tuple)
        self.assertEqual(joe.name, b'Joe')
        self.assertEqual(joe.id, 42)
        self.assertEqual(jim.name, b'Jim')
        self.assertEqual(jim.id, 1)
        self.assertEqual(Emp.__name__, b'Emp')
...
class TypedDictTests(BaseTestCase):
    ...
    def test_basics_iterable_syntax(self):
        Emp = TypedDict(b'Emp', {b'name': str, b'id': int})
        ...
        self.assertEqual(jim[b'name'], b'Jim')
        self.assertEqual(jim[b'id'], 1)
        self.assertEqual(Emp.__name__, b'Emp')
        self.assertEqual(Emp.__module__, __name__)
        self.assertEqual(Emp.__bases__, (dict,))
        self.assertEqual(Emp.__annotations__, {b'name': str, b'id': int})
    ...
    def test_typeddict_errors(self):
        Emp = TypedDict(b'Emp', {b'name': str, b'id': int})
        self.assertEqual(TypedDict.__module__, b'typing')
        jim = Emp(name=b'Jim', id=1)
        with self.assertRaises(TypeError):
            isinstance({}, Emp)
        with self.assertRaises(TypeError):
        ...
        with self.assertRaises(TypeError):
            TypedDict(b'Hi', x=1)
        with self.assertRaises(TypeError):
            TypedDict(b'Hi', [(b'x', int), ('y', 1)])
        with self.assertRaises(TypeError):
            TypedDict(b'Hi', [(b'x', int)], y=int)

    def test_pickle(self):
        global EmpD
        EmpD = TypedDict(b'EmpD', name=str, id=int)
        jane = EmpD({b'name': b'jane', b'id': 37})
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            z = pickle.dumps(jane, proto)
            jane2 = pickle.loads(z)
            self.assertEqual(jane2, jane)
            self.assertEqual(jane2, {b'name': b'jane', b'id': 37})
            ZZ = pickle.dumps(EmpD, proto)
            EmpDnew = pickle.loads(ZZ)
            self.assertEqual(EmpDnew({b'name': b'jane', b'id': 37}), jane)
    ...
    def test_total(self):
        D = TypedDict(b'D', {b'x': int}, total=False)
        self.assertEqual(D(), {})
        self.assertEqual(D(x=1), {b'x': 1})
...
class IOTests(BaseTestCase):
    ...
    def test_io_submodule(self):
        ...
        self.assertEqual(set(__all__), set([b'IO', b'TextIO', b'BinaryIO']))
        self.assertEqual(__name__, b'typing.io')


class RETests(BaseTestCase):

    def test_basics(self):
        pat = re.compile(b'[a-z]+', re.I)
        self.assertIsSubclass(pat.__class__, Pattern)
        self.assertIsSubclass(type(pat), Pattern)
        self.assertIsInstance(pat, Pattern)
        mat = pat.search(b'12345abcde.....')
    ...
    def test_repr(self):
        self.assertEqual(repr(Pattern), b'Pattern[~AnyStr]')
        self.assertEqual(repr(Pattern[unicode]), b'Pattern[unicode]')
        self.assertEqual(repr(Pattern[str]), b'Pattern[str]')
        self.assertEqual(repr(Match), b'Match[~AnyStr]')
        self.assertEqual(repr(Match[unicode]), b'Match[unicode]')
        self.assertEqual(repr(Match[str]), b'Match[str]')

    def test_re_submodule(self):
        from typing.re import Match, Pattern, __all__, __name__
        self.assertIs(Match, typing.Match)
        self.assertIs(Pattern, typing.Pattern)
        self.assertEqual(set(__all__), set([b'Match', b'Pattern']))
        self.assertEqual(__name__, b'typing.re')
    ...
    def test_cannot_subclass(self):
        ...
        self.assertEqual(str(ex.exception), b'Cannot subclass typing._TypeAlias')
...
class AllTests(BaseTestCase):
    ...
    def test_all(self):
        ...
        self.assertIn(b'AbstractSet', a)
        self.assertIn(b'ValuesView', a)
        self.assertIn(b'cast', a)
        self.assertIn(b'overload', a)
        self.assertNotIn(b'io', a)
        self.assertNotIn(b're', a)
        self.assertNotIn(b'os', a)
        self.assertNotIn(b'sys', a)
        self.assertIn(b'Text', a)
        self.assertIn(b'SupportsComplex', a)
    ...
    def test_typing_compiles_with_opt(self):
        file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), b'typing.py')
        try:
            subprocess.check_output([sys.executable, b'-OO', file_path], stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError:
            self.fail(b'Module does not compile with optimize=2 (-OO flag).')
