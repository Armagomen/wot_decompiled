...
# Embedded file name: scripts/common/site-packages/future-0.18.2/libpasteurize/fixes/fix_kwargs.py
...
def needs_fixing(raw_params, kwargs_default=_kwargs_default_name):
    ...
    for t in raw_params[2:]:
        if t.type == token.COMMA:
            continue
        if t.type == token.NAME and not found_kwargs:
            needs_fix = True
        ...
        if not new_kwargs:
            ...
            for name, default_value in gen_params(params_rawlist):
                if default_value is None:
                    ...
                    suite.insert_child(2, String(_assign_template % {u'name': name,
                     u'kwargs': new_kwargs}, prefix=ident))
                suite.insert_child(2, Newline())
                suite.insert_child(2, String(_else_template % {u'name': name,
                 u'default': default_value}, prefix=ident))
                suite.insert_child(2, Newline())
                suite.insert_child(2, String(_if_template % {u'assign': _assign_template % {u'name': name,
                             u'kwargs': new_kwargs},
                 u'name': name,
                 u'kwargs': new_kwargs}, prefix=ident))
