...
import numbers, copy, sys, re, binascii, mmap, os, struct, operator, array, io, collections
try:
...
class ConstByteStore(object):
    ...
    def _prependstore(self, store):
        ...
        if bit_offset:
            joinval = store.getbyte(-1) & (255 ^ 255 >> bit_offset) | self._rawarray[self.byteoffset] & 255 >> bit_offset
...
LITERAL_RE = re.compile('(?P<name>0([xob]))(?P<value>.+)', re.IGNORECASE)
STRUCT_PACK_RE = re.compile('(?P<endian>[<>@])?(?P<fmt>(?:\\d*[bBhHlLqQfd])+)$')
STRUCT_SPLIT_RE = re.compile('\\d*[bBhHlLqQfd]')
REPLACEMENTS_BE = {'b': 'intbe:8', 'B': 'uintbe:8', 'h': 'intbe:16', 
   'H': 'uintbe:16', 'l': 'intbe:32', 
   'L': 'uintbe:32', 'q': 'intbe:64', 
   'Q': 'uintbe:64', 'f': 'floatbe:32', 
   'd': 'floatbe:64'}
REPLACEMENTS_LE = {'b': 'intle:8', 'B': 'uintle:8', 'h': 'intle:16', 
   'H': 'uintle:16', 'l': 'intle:32', 
   'L': 'uintle:32', 'q': 'intle:64', 
   'Q': 'uintle:64', 'f': 'floatle:32', 
   'd': 'floatle:64'}
PACK_CODE_SIZE = {'b': 1, 'B': 1, 'h': 2, 'H': 2, 'l': 4, 'L': 4, 'q': 8, 
   'Q': 8, 'f': 4, 'd': 8}
_tokenname_to_initialiser = {'hex': 'hex', '0x': 'hex', '0X': 'hex', 'oct': 'oct', '0o': 'oct', 
   '0O': 'oct', 'bin': 'bin', '0b': 'bin', '0B': 'bin', 
   'bits': 'auto', 'bytes': 'bytes', 'pad': 'pad'}
...
class Bits(object):
    ...
    def __repr__(self):
        ...
        if isinstance(self._datastore._rawarray, MmapByteArray):
            ...
            return ("{0}(filename='{1}'{2}{3}{4})").format(self.__class__.__name__, self._datastore._rawarray.source.name, lengthstring, offsetstring, pos_string)
        else:
            s = self.__str__()
            lengthstring = ''
            if s.endswith('...'):
    ...
    def _init_with_token(cls, name, token_length, value):
        ...
        if token_length == 0:
            ...
            try:
                ...
                elif name in ('uint', 'int', 'uintbe', 'intbe', 'uintle', 'intle',
                              'uintne', 'intne'):
...
class BitArray(Bits):
    ...
    def __new__(cls, auto=None, length=None, offset=None, **kwargs):
        ...
        x._datastore = ByteStore(y._datastore._rawarray[:], y._datastore.bitlength, y._datastore.offset)
        return x
...
def _switch_lsb0_methods(lsb0):
    ...
    if lsb0:
        ...
        BitArray._prepend = BitArray._append_msb0
    ...
    name_to_read = {'uint': Bits._readuint, 'uintle': Bits._readuintle, 
...
set_msb0()
init_with_length_and_offset = {'bytes': Bits._setbytes_safe, 'filename': Bits._setfile}
init_with_length_only = {'uint': Bits._setuint, 'int': Bits._setint, 
...
init_without_length_or_offset = {'bin': Bits._setbin_safe, 'hex': Bits._sethex, 
   'oct': Bits._setoct, 
   'ue': Bits._setue, 
   'se': Bits._setse, 
   'uie': Bits._setuie, 
